...
fileprivate let dataLockModelMain:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "SHBDropMenuCell" :*/
fileprivate let appTodayMain:String = "string pool center backSHBDr"
fileprivate let dataTargetUser:String = "opMenelse origin"

/*: "title" :*/
fileprivate let dataIndexConst:[Character] = ["t","i","t","l","e"]
...
public class ObjectMenuView: UIView {
	var iconCurrentOn: Bool = true
	var toPingCount: Int = 17
	var windowBottomArray: [AnyHashable] = []
	var captureEnable: Bool = true
	var basicTotal: Int = 44
	var contentsArray: [AnyHashable] = []
	var equalDoing: Bool = true
	var hookupTotal: Int = 56
	var styleArray: [AnyHashable] = []

    //: var menuDelegate: DropDownMenuViewDelegate?
    var menuDelegate: CardViewDelegate?
    ...
    var menuBgColor: UIColor? {
        ...
        //: }
    } // = Config.ViewResourceConfig.menuBgColor
    // 线条颜色
    //: var lineColor = Config.MenuCellConfig.lineColor
    var lineColor = Config.ToConfig.lineColor
    // cell高度
    //: public var menuCellHeight = Config.MenuCellConfig.menuCellHeight
    public var menuCellHeight = Config.ToConfig.menuCellHeight
    /**
     *  table最大高度限制
     *  默认：5 * cellHeight
     */
    //: var menuMaxHeight = Config.MenuCellConfig.menuMaxHeight
    var menuMaxHeight = Config.ToConfig.menuMaxHeight
    /**
     *  小三角高度
     *  45°等腰三角形
     */
    ...
    public var menuStyle = Config.initialMenuStyle ?? OfMenuStyle.MenuDarkStyle {
        //: didSet {
        didSet {
            //: switch menuStyle {
            switch menuStyle {
...
//        menuView.refreshUI()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataLockModelMain.reversed(), encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func aboveMake() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func giftByInstall() -> UITableView {
        ...
        //: return tableView
            ...
            if (contentView.center.x == 10.23) && (!contentView.autoresizesSubviews) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let showText = PlayerView()
            showText.subSwitch = adjustPullDown

            
            showText.currentOff = { [self] doingMakeOff in
            self.iconCurrentOn = doingMakeOff
            
            var showText = adjustPullDown
                showText = true
                showText = false
            if showText != self.iconCurrentOn {
                self.iconCurrentOn = showText
            }
            
            return self.iconCurrentOn
            }
            showText.easyLaySum = { [self] tableNameTotal in
            self.toPingCount = tableNameTotal
            
            return self.toPingCount
            }
            showText.stopArray = { [self] endArray in
            self.windowBottomArray = endArray
            
            guard var value = self.windowBottomArray as? [String] else {
                return nil
            }
            return value
            }
                contentView.addSubview(showText)
            }
    ...
    private func catScan() {
        ...
        mTable.register(WhiteMenuCell.self, forCellReuseIdentifier: (String(appTodayMain.suffix(5)) + String(dataTargetUser.prefix(5)) + "uCell"))
        //: contentView.addSubview(mTable)
        contentView.addSubview(mTable)
    }

    //: private func refreshUI() {
    private func slowing() {
        //: drawmTableFrame()
        superior()
        //: mTable.reloadData()
        mTable.reloadData()
    
            if (mTable.clipsToBounds) && (mTable.convert(CGRect(x: CGFloat(327.41), y: 0, width: 0, height: 0), to: mTable.superview).size.height == 94.30) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let sectionBack = PlayerView(frame: mTable.frame.insetBy(dx: CGFloat(457.07), dy: CGFloat(51)))
            sectionBack.subSwitch = self.adjustPullDown

            
            sectionBack.currentOff = { [self] doingMakeOff in
            self.equalDoing = doingMakeOff
            
            var sectionBack = self.adjustPullDown
                sectionBack = true
                sectionBack = !sectionBack
            if sectionBack != self.equalDoing {
                self.equalDoing = sectionBack
            }
            
            return self.equalDoing
            }
            sectionBack.easyLaySum = { [self] tableNameTotal in
            self.hookupTotal = tableNameTotal
            
            return self.hookupTotal
            }
            sectionBack.stopArray = { [self] endArray in
            self.styleArray = endArray
    ...
    private func straddleAnimateRemove() {
        ...
        UIView.animate(withDuration: 0.5) {
            ...
            if (contentView.clipsToBounds) && (contentView.convert(CGRect(x: CGFloat(327.41), y: 0, width: 0, height: 0), to: contentView.superview).size.height == 94.30) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let sectionBack = PlayerView(frame: contentView.frame.insetBy(dx: CGFloat(457.07), dy: CGFloat(51)))
            sectionBack.subSwitch = self.adjustPullDown

            
            sectionBack.currentOff = { [self] doingMakeOff in
            self.captureEnable = doingMakeOff
            
            var sectionBack = self.adjustPullDown
                sectionBack = true
                sectionBack = !sectionBack
            if sectionBack != self.captureEnable {
                self.captureEnable = sectionBack
            }
            
            return self.captureEnable
            }
            sectionBack.easyLaySum = { [self] tableNameTotal in
            self.basicTotal = tableNameTotal
            
            return self.basicTotal
            }
            sectionBack.stopArray = { [self] endArray in
            self.contentsArray = endArray
            
            guard var value = self.contentsArray as? [String] else {
                return nil
            }
            return value
            }
                contentView.addSubview(sectionBack)
            }

	}

    //: private func cacuateCellWidth() -> CGFloat {
    private func visualisation() -> CGFloat {
    ...
    override public func touchesBegan(_: Set<UITouch>, with _: UIEvent?) {
        //: animateRemoveView()
        straddleAnimateRemove()
    }
}

//: extension DropDownMenuView: UITableViewDelegate, UITableViewDataSource {
extension ObjectMenuView: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(appTodayMain.suffix(5)) + String(dataTargetUser.prefix(5)) + "uCell"), for: indexPath) as! WhiteMenuCell
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        selectionAction?(indexPath.row, cellModel.title ?? (String(dataIndexConst)))
        //: animateRemoveView()
        straddleAnimateRemove()

        //: menuDelegate?.didClickSelectedRow(index: indexPath.row, title: cellModel.title ?? "")
        menuDelegate?.act(index: indexPath.row, title: cellModel.title ?? "")
    }
}
