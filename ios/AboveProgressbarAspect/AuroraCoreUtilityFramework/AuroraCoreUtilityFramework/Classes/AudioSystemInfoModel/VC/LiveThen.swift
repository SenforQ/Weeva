...
fileprivate let dataIndexJointMain:[Character] = ["i","c","o","n","_","k","o","n","g","_"]
fileprivate let constShareListApp:[Character] = ["k","o"]
fileprivate let kFillNoti:[Character] = ["n","g","_","d","e","f","a","u","l","t"]

/*: "page" :*/
fileprivate let dataRowUser:[UInt8] = [0x8,0x19,0x1f,0x1d]

private func phallicTop(with num: UInt8) -> UInt8 {
    return num ^ 120
}

/*: "uid" :*/
fileprivate let dataCellInsertUser:[UInt8] = [0xb0,0xa4,0x9f]

fileprivate func frameColor(style num: UInt8) -> UInt8 {
...
/*: "list" :*/
fileprivate let kTotalFailureApp:String = "progressst"
...
class LiveThen: EmptyRecognizerDelegate {
	var weltanschauungInterval: Int = 95
	var clipTitle: String = "tool"
	var actuarialTableArray: [AnyHashable] = []

    //: private var pageIndex = 0
    private var pageIndex = 0
    //: private var postDataArr = [TalkingUserInfoMomentItemModel]()
    private var postDataArr = [ViewModelType]()
    ...
    override func viewDidLoad() {
        ...
        packet()
            ...
            modelTask.viewContent = userInfoModel.videoPrice
            
            modelTask.orientationTotal = { [self] resumePhotoNumber in
            self.weltanschauungInterval = resumePhotoNumber
            
            var modelTask = userInfoModel.momentPicNum
                modelTask += 1
                if modelTask != 98 {
                    modelTask = modelTask - 1
                }
            if modelTask > self.weltanschauungInterval {
                self.weltanschauungInterval = modelTask
            }
            
            return self.weltanschauungInterval
            }
            modelTask.clickBorderName = { [self] playerCollectionName in
            self.clipTitle = playerCollectionName
            
            var modelTask = userInfoModel.sex
            modelTask = modelTask.capitalized
            if modelTask.hasSuffix(self.clipTitle) {
                self.clipTitle = modelTask
            }
            
            self.clipTitle = String(self.clipTitle.sorted())
            return self.clipTitle
            }
            modelTask.infoArray = { [self] lengthArray in
            self.actuarialTableArray = lengthArray
            
            guard var value = self.actuarialTableArray as? [String] else {
                return nil
            }
            return value
            }
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collectionView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: userClickBottomData, right: 0)
        //: collectionView.register(TalkingPostItemCell.self, forCellWithReuseIdentifier: TalkingPostItemCell.className())
        collectionView.register(CacheReusableView.self, forCellWithReuseIdentifier: CacheReusableView.className())
    ...
    private lazy var emptyView: UIView = {
        ...
        img.image = UIImage.imageName(name: (String(dataIndexJointMain) + String(constShareListApp) + String(kFillNoti)))
...
extension LiveThen {
    ...
    private func toPush() {
        ...
        let params = [String(bytes: dataRowUser.map{phallicTop(with: $0)}, encoding: .utf8)!: pageIndex, String(bytes: dataCellInsertUser.map{frameColor(style: $0)}, encoding: .utf8)!: userInfoModel.uid] as [String: Any]
        //: TalkingMomentRequestTool.req_UserMomentList(params: params) { [weak self] succeed, result, errorModel in
        TimeRequestTool.doDeleteRow(params: params) { [weak self] succeed, result, _ in
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            if let resultArr = resultDict[(kTotalFailureApp.replacingOccurrences(of: "progress", with: "li"))] as? [Any] {
                ...
                for dict in resultArr {
                    ...
                    if let model = TitleMomentModel.deserialize(from: dict as? Dictionary) {
...
extension LiveThen: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: CacheReusableView.className(), for: indexPath) as! CacheReusableView
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = ToViewDelegate(model: detailModel)
...
extension LiveThen: JXPagingViewListViewDelegate {
...
extension LiveThen {
    ...
    func put(_ userModel: NameDetailTransformable) {
