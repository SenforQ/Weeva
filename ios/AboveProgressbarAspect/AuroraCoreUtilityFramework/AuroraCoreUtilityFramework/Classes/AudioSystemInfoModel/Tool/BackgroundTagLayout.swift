...
public protocol LiveReactiveCompatible: NSObject {
    /// 文字内容for cell
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, textForItemAt indexPath: IndexPath) -> String
    func groupAt(_ layout: BackgroundTagLayout, collectionView: UICollectionView, textForItemAt indexPath: IndexPath) -> String
    /// section head footer size
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, sizeForSupplementaryElementOfKind kind: String, at section: Int) -> CGSize
    func elementAt(_ layout: BackgroundTagLayout, collectionView: UICollectionView, sizeForSupplementaryElementOfKind kind: String, at section: Int) -> CGSize

    ///  标签的内边距
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, tagInnerMarginForItemAt indexPath: IndexPath) -> CGFloat
    func dataAt(_ layout: BackgroundTagLayout, collectionView: UICollectionView, tagInnerMarginForItemAt indexPath: IndexPath) -> CGFloat
    
    /// 底部的间距
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, heightBottomAt section: Int) -> CGFloat
    func wood(_ layout: BackgroundTagLayout, collectionView: UICollectionView, heightBottomAt section: Int) -> CGFloat

    
    /// 左右的间距
    //: func tagLayout(_ layout: TalkingTagLayout, collectionView: UICollectionView, leftSapceAt section: Int) -> CGFloat
    func status(_ layout: BackgroundTagLayout, collectionView: UICollectionView, leftSapceAt section: Int) -> CGFloat
...
public enum WeltanschauungNameAlignment {
    //: case left
    case left
    //: case right
    case right
...
open class BackgroundTagLayout: UICollectionViewLayout {
	var colorCount: Double = 97.8
	var iconTitle: String = "gift"
...
//    open var tagInnerMargin: CGFloat = 25
    ...
    open var itemFont = UIFont.viewPath(type: .Regular, fontSize: 15)

    //: open weak var delegate: TagLayoutDelegate?
    open weak var delegate: LiveReactiveCompatible?
//    var titles = [String]()
    /// tag 内容的对齐方式
    //: open var contentAlignment = TagContentAlignment.left
    open var contentAlignment = WeltanschauungNameAlignment.left
    ...
    override open func prepare() {
        ...
        for section in 0 ..< sections {
            ...
            let sectionHeadSize = delegate.elementAt(self, collectionView: collectionView, sizeForSupplementaryElementOfKind: UICollectionView.elementKindSectionHeader, at: section)
            
            //: let bottomSpace = delegate.tagLayout(self, collectionView: collectionView, heightBottomAt: section)
            let bottomSpace = delegate.wood(self, collectionView: collectionView, heightBottomAt: section)

            //: let itemLeftSpace = delegate.tagLayout(self, collectionView: collectionView, leftSapceAt: section)
            let itemLeftSpace = delegate.status(self, collectionView: collectionView, leftSapceAt: section)
            ...
            for item in 0 ..< rows {
                ...
                let text = delegate.groupAt(self, collectionView: collectionView, textForItemAt: indexPath)
                //: let tagInnerMargin = delegate.tagLayout(self, collectionView: collectionView, tagInnerMarginForItemAt: indexPath)
                let tagInnerMargin = delegate.dataAt(self, collectionView: collectionView, tagInnerMarginForItemAt: indexPath)
                //: let tagWidth = textWidth(text) + tagInnerMargin
                let tagWidth = widthTable(text) + tagInnerMargin
...
//            contentHeight = contentHeight + sectionFooterSize.height
        ...
        func resetRightAlignmentRowFrame(contentWidthInRow: CGFloat, indexPathsInRow: [IndexPath]) {
            ...
            if (collectionView.constraintsAffectingLayout(for: .vertical).count == 81) && (collectionView.inputViewController != nil) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let cellImage = AppView()
            cellImage.subTalkAtTotal = sections

            
            cellImage.labelInterval = { [self] liveGreetNumber in
            self.colorCount = liveGreetNumber
            
            self.colorCount -= 1
            return self.colorCount
            }
            cellImage.tabTitle = { [self] plusTitle in
            self.iconTitle = plusTitle
            
            if self.iconTitle[self.iconTitle.startIndex].isUppercase {
                self.iconTitle = ""
            }
            return self.iconTitle
            }
            cellImage.topArray = { [self] dominantArray in
            self.firstArray = dominantArray
            ...
            return value
                ...
                collectionView.addSubview(cellImage)
            }
...
//        return visibleLayoutAttributes
    ...
    func widthTable(_ text: String) -> CGFloat {
