...
fileprivate let userToCommentShow:[UInt8] = [0x79,0x72,0x74,0x65,0x72,0x20,0x64,0x6e,0x61,0x20,0x74,0x69,0x78,0x65,0x20,0x65,0x73,0x61,0x65,0x6c,0x70,0x20,0x2c,0x64,0x65,0x67,0x6e,0x61,0x68,0x63,0x20,0x73,0x75,0x74,0x61,0x74,0x53]

/*: "GET" :*/
fileprivate let showTextUser:[Character] = ["G","E","T"]
...
protocol CapacityIncomeThen: NSObject {
    /// 播放完毕的回调
    //: func playerStatusChange(status: AudioPlayerStatus)
    func totalo(status: TabMultiplierTarget)
}

//: class TalkingAudioPlayTool: NSObject {
class SpeedBackstopThen: NSObject {
    //: var player: AVAudioPlayer?
    var player: AVAudioPlayer?
    //: open weak var delegate: TalkingAudioPlayToolDelegate?
    open weak var delegate: CapacityIncomeThen?

    // singleton
    //: static let shared = TalkingAudioPlayTool()
    static let shared = SpeedBackstopThen()
    ...
    override private init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(search(notification:)), name: AVAudioSession.interruptionNotification, object: nil)
    }

    //: func play(urlStr: String) {
    func somePath(urlStr: String) {
        ...
        sound(audioUrl: urlStr) { data in
            ...
            do {
                ...
                self.exceptAdd(showMsg: String(bytes: userToCommentShow.reversed(), encoding: .utf8)!.localized)
                //: self.delegate?.playerStatusChange(status: .PlayerFinish)
                self.delegate?.totalo(status: .PlayerFinish)
...
extension SpeedBackstopThen {
    //: func downloadAudioWith(audioUrl: String, completion: @escaping AudioCompletionblock) {
    func sound(audioUrl: String, completion: @escaping AudioCompletionblock) {
        //: var urlOk = true
        var urlOk = true
        ...
        request.httpMethod = (String(showTextUser))
...
extension SpeedBackstopThen: AVAudioPlayerDelegate {
    /// 播放过程中中断
    ...
    @objc func search(notification: NSNotification) {
        ...
        if changeReason?.intValue ?? 0 == AVAudioSession.InterruptionType.began.rawValue {
            ...
            self.delegate?.totalo(status: .PlayerInterruption)
        }
    }
