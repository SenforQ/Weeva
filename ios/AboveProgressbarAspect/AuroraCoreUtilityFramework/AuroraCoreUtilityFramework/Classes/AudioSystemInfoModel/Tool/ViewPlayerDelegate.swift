...
fileprivate let kModelMain:[UInt8] = [0xce,0xf2,0xf3,0xe9,0xba,0xfc,0xff,0xfb,0xee,0xef,0xe8,0xff,0xba,0xf3,0xe9,0xba,0xf4,0xf5,0xee,0xba,0xfb,0xec,0xfb,0xf3,0xf6,0xfb,0xf8,0xf6,0xff,0xba,0xfe,0xef,0xe8,0xf3,0xf4,0xfd,0xba,0xfb,0xba,0xf9,0xfb,0xf6,0xf6]

private func touchEqual(cell num: UInt8) -> UInt8 {
    return num ^ 154
}

/*: "wav" :*/
fileprivate let notiEndData:String = "wascale"
...
protocol ManagerToolDelegate: NSObject {
    /// 录音时间
...
class ViewPlayerDelegate: NSObject {
    //: open weak var delegate: TalkingAudioToolDelegate?
    open weak var delegate: ManagerToolDelegate?
    ...
    static let shared = ViewPlayerDelegate()
    ...
    override private init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(mistakeNotification(notification:)), name: AVAudioSession.interruptionNotification, object: nil)
    }

    //: func getVoiceRecordLocalPathWithFileType(path: NSString) -> String {
    func recoup(path: NSString) -> String {
...
extension ViewPlayerDelegate: AVAudioRecorderDelegate, AVAudioPlayerDelegate {
    /// 录制、播放过程中中断
    //: @objc func interruptionChange(notification: NSNotification) -> Void {
    @objc func mistakeNotification(notification: NSNotification) {
        ...
        if changeReason?.intValue ?? 0 == AVAudioSession.InterruptionType.began.rawValue {
            ...
            self.exceptAdd(showMsg: String(bytes: kModelMain.map{touchEqual(cell: $0)}, encoding: .utf8)!.localized)
            //: if self.isRecording {
            if self.isRecording {
                //: self.delegate?.func__recordFinishWithFile(filePath: self.filePath)
                self.delegate?.sum(filePath: self.filePath)
...
//                destroyRecordTimer()
                ...
                self.delegate?.need(status: .PlayerInterruption)
            }
        }
    }
    ...
    func audioPlayerDidFinishPlaying(_: AVAudioPlayer, successfully _: Bool) {
        ...
        self.delegate?.need(status: .PlayerFinish)
    }
}

//: extension TalkingAudioTool {
extension ViewPlayerDelegate {
    //: func BeginRecord() {
    func deadline() {
        //: deleteFile()
        untilUp()
        ...
        let path = recoup(path: (notiEndData.replacingOccurrences(of: "scale", with: "v")) as NSString)
    ...
    func usher() {
        ...
        recordTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(pathAddTime(time:)), userInfo: nil, repeats: true)
    ...
    @objc func pathAddTime(time _: Timer) {
        //: recorder?.updateMeters()
        recorder?.updateMeters()
        ...
        self.delegate?.volition(currentTime: interval)
    }
    ...
    func sex() {
        ...
        self.delegate?.deletePlace(currentTime: currentTime)
    }
    ...
    func render() {
        ...
        playTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(nail(time:)), userInfo: nil, repeats: true)
        //: if let curTime: Timer = recordTimer {
        if let curTime: Timer = recordTimer {
            //: RunLoop.main.add(curTime, forMode: .common)
            RunLoop.main.add(curTime, forMode: .common)
