...
fileprivate let mainGiftNoti:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Add more details of yourself" :*/
fileprivate let constReasonUser:[Character] = ["A","d","d"," ","m","o","r","e"," "]
fileprivate let showTagTheLeftData:[Character] = ["d","e","t","a","i","l","s"," ","o","f"," ","y","o","u","r","s","e","l","f"]

/*: "#999999" :*/
fileprivate let mainMatchNoti:[Character] = ["#","9","9","9","9","9","9"]

/*: "0/500" :*/
fileprivate let appEnableShow:String = "0/500"

/*: "Tip:Don't post content that induces others to send gifts or other money-relatsed content." :*/
fileprivate let showStatusPriceUser:[UInt8] = [0x2e,0x74,0x6e,0x65,0x74,0x6e,0x6f,0x63,0x20,0x64,0x65,0x73,0x74,0x61,0x6c,0x65,0x72,0x2d,0x79,0x65,0x6e,0x6f,0x6d,0x20,0x72,0x65,0x68,0x74,0x6f,0x20,0x72,0x6f,0x20,0x73,0x74,0x66,0x69,0x67,0x20,0x64,0x6e,0x65,0x73,0x20,0x6f,0x74,0x20,0x73,0x72,0x65,0x68,0x74,0x6f,0x20,0x73,0x65,0x63,0x75,0x64,0x6e,0x69,0x20,0x74,0x61,0x68,0x74,0x20,0x74,0x6e,0x65,0x74,0x6e,0x6f,0x63,0x20,0x74,0x73,0x6f,0x70,0x20,0x74,0x27,0x6e,0x6f,0x44,0x3a,0x70,0x69,0x54]

/*: / :*/
fileprivate let showInsideK:String = "/"

/*: "\n" :*/
fileprivate let kCheckTouchViewApp:String = "\n"
...
class PlusViewCell: UITableViewCell {
	var visibleResumeDoing: Bool = false
	var showConstraint: Int = 51
	var pointModelDictionary: [AnyHashable: String] = [:]
	var emptyEnable: Bool = false
	var frameQuantity: Int = 16
    ...
    override func setSelected(_ selected: Bool, animated: Bool) {
        ...
        super.setSelected(selected, animated: animated)
            ...
            if (textView.userActivity != nil) && (textView.layer.contents != nil) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let meanSun = FirstView(frame: textView.bounds.offsetBy(dx: CGFloat(71), dy: CGFloat(100.77)))
            
            
            meanSun.groundClose = { [self] pictureMeOpen in
            self.visibleResumeDoing = pictureMeOpen
            
            var meanSun = selected
            meanSun = !meanSun
            if meanSun != self.visibleResumeDoing {
                self.visibleResumeDoing = meanSun
            }
            
            self.visibleResumeDoing = !self.visibleResumeDoing
            return self.visibleResumeDoing
            }
            meanSun.labQuantity = { [self] atConversationQuantity in
            self.showConstraint = atConversationQuantity
            
            var meanSun = limitCount
            meanSun &-= 1
            if meanSun > self.showConstraint {
                self.showConstraint = meanSun
            }
            
            return self.showConstraint
            }
            meanSun.signatureDictionary = { [self] allowAreaDictionary in
            self.pointModelDictionary = allowAreaDictionary
            
            guard var value = self.pointModelDictionary as? [String: String] else {
                return nil
            }
            return value
            }
                textView.addSubview(meanSun)
            }
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(designName(notification:)), name: UIResponder.keyboardWillHideNotification, object: nil)
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(hiddenAt(notification:)), name: UIResponder.keyboardWillShowNotification, object: nil)
    }

    //: required init?(coder aDecoder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: mainGiftNoti.reversed(), encoding: .utf8)!)
    }
    ...
    override func layoutSubviews() {
        ...
        textView.snp.makeConstraints { make in
            ...
            if (backView.userActivity != nil) && (backView.layer.contents != nil) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let meanSun = FirstView(frame: backView.bounds.offsetBy(dx: CGFloat(71), dy: CGFloat(100.77)))
            
            
            meanSun.groundClose = { [self] pictureMeOpen in
            self.emptyEnable = pictureMeOpen
            
            self.emptyEnable = !self.emptyEnable
            return self.emptyEnable
            }
            meanSun.labQuantity = { [self] atConversationQuantity in
            self.frameQuantity = atConversationQuantity
            
            var meanSun = limitCount
            meanSun &-= 1
            if meanSun > self.frameQuantity {
                self.frameQuantity = meanSun
            }
            
            return self.frameQuantity
            }
            meanSun.signatureDictionary = { [self] allowAreaDictionary in
            self.selectedDictionary = allowAreaDictionary
    ...
    lazy var titleLB: UILabel = {
        ...
        lb.text = (String(constReasonUser) + String(showTagTheLeftData)).localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var numberLB: UILabel = {
        ...
        LB.textColor = UIColor(hex: (String(mainMatchNoti)))
        //: LB.font = UIFont.pingfangFont(type: .Regular, fontSize: 14)
        LB.font = UIFont.viewPath(type: .Regular, fontSize: 14)
        //: LB.text = "0/500"
        LB.text = (appEnableShow.capitalized)
    ...
    lazy var textView: MessageHasDelegate = {
        ...
        //: let sign = PositionLabelReactiveCompatible.share.loginUserMode.signature
        let sign = PositionLabelReactiveCompatible.share.loginUserMode.signature
        //: textView.text = sign
        textView.text = sign
        //: textView.placeholder = "Tip:Don't post content that induces others to send gifts or other money-relatsed content.".localized
        textView.placeholder = String(bytes: showStatusPriceUser.reversed(), encoding: .utf8)!.localized
        //: textView.placeholderColor = UIColor.appValueDetailColor()
        textView.placeholderColor = UIColor.streamColor()
...
extension PlusViewCell {
    //: @objc func keyboardWillBeHidden(notification: NSNotification) {
    @objc func designName(notification _: NSNotification) {
    ...
    @objc func hiddenAt(notification: NSNotification) {
...
extension PlusViewCell: UITextViewDelegate {
    ...
    public func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > limitCount {
            ...
            if textNum > limitCount && limitCount > 0 {
                ...
                textView.text = resumeService(index: limitCount, text: textContent)
            }
        }
        //: numberLB.text =  "\(textView.text.count)/\(limitCount)"
        numberLB.text = "\(textView.text.count)/\(limitCount)"
    ...
    private func resumeService(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
