...
fileprivate let kLogUser:[UInt8] = [0xe0,0xe5,0xe0,0xeb,0x9f,0xda,0xe6,0xdb,0xdc,0xe9,0xb1,0xa0,0x97,0xdf,0xd8,0xea,0x97,0xe5,0xe6,0xeb,0x97,0xd9,0xdc,0xdc,0xe5,0x97,0xe0,0xe4,0xe7,0xe3,0xdc,0xe4,0xdc,0xe5,0xeb,0xdc,0xdb]

fileprivate func listMe(stop num: UInt8) -> UInt8 {
...
/*: "img_loading_lose" :*/
fileprivate let userIndexStreamLabelK:[Character] = ["i","m"]
fileprivate let mainViewEqualMakeConst:[Character] = ["g","_","l","o","a","d","i","n","g","_","l","o","s","e"]
...
class DetailView: UICollectionViewCell {
	var atCount: Double = 64.3
	var rowText: String = "line"
	var toTodayArray: [AnyHashable] = []
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kLogUser.map{listMe(stop: $0)}, encoding: .utf8)!)
    }
    ...
    override func layoutSubviews() {
        ...
        self.acquireText()
            ...
            if (imgView.superview != nil && imgView.superview!.isHidden) && (imgView.bounds.size.width == 62.47) {
            ...
            self.atCount = messageFilterSum
            
            self.atCount /= 8
            return self.atCount
            }
            atView.fromLevelWoodContent = { [self] emptyUniversalText in
            self.rowText = emptyUniversalText
            
            return self.rowText
            }
            atView.tweedledumAndTweedledeeClickArray = { [self] compartmentArray in
            self.toTodayArray = compartmentArray
            
            guard var value = self.toTodayArray as? [String] else {
                return nil
            }
            return value
            }
            atView.cornerLineDictionary = { [self] arrayViewNameDictionary in
            self.scheduleDictionary = arrayViewNameDictionary
            
            guard var value = self.scheduleDictionary as? [String: String] else {
                return nil
            }
            return value
            }
                imgView.addSubview(atView)
            }
...
extension DetailView {
    ...
    private func timeAt() {
        ...
        let singleTap = UITapGestureRecognizer(target: self, action: #selector(imagePop))

        //: doubleTap = UITapGestureRecognizer(target: self, action: #selector(doubleTapAction)).then {
        doubleTap = UITapGestureRecognizer(target: self, action: #selector(file)).then {
...
extension DetailView {
    ...
    func post(imgModel: TempCustomModelType) {
        ...
        if imgModel.imagePic != nil {
            ...
            imgView.kf.setImage(with: URL(string: imgModel.url), placeholder: UIImage(named: (String(userIndexStreamLabelK) + String(mainViewEqualMakeConst))), options: [.keepCurrentImageWhileLoading]) { _ in
                //: self.recoverSubviews()
                self.acquireText()
            }
        }
    ...
    func country(imgUrl: String) {
        ...
        imgView.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage(named: (String(userIndexStreamLabelK) + String(mainViewEqualMakeConst))), options: [.keepCurrentImageWhileLoading]) { _ in
            //: self.recoverSubviews()
            self.acquireText()
        }
    }
...
extension DetailView: UIScrollViewDelegate, UIGestureRecognizerDelegate {
...
extension DetailView {
    ...
    func acquireText() {
        ...
        var newW = userDeviceKey
        //: var newH = ScreenWidth * tempImg!.size.height / tempImg!.size.width
        var newH = userDeviceKey * tempImg!.size.height / tempImg!.size.width

        //: if newH > ScreenHeight {
        if newH > appNameMessage {
            //: newH = ScreenHeight
            newH = appNameMessage
            //: newW = tempImg!.size.width * ScreenHeight / tempImg!.size.height
            newW = tempImg!.size.width * appNameMessage / tempImg!.size.height
        }
        ...
        self.imgContainerView.origin = CGPoint(x: (userDeviceKey - newW) / 2.0, y: (appNameMessage - newH) / 2)
