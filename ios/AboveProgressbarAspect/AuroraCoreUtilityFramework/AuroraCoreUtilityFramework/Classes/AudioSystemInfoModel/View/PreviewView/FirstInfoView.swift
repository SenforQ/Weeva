...
fileprivate let showColorK:[UInt8] = [0x54,0x53,0x54,0x49,0x15,0x5e,0x52,0x59,0x58,0x4f,0x7,0x14,0x1d,0x55,0x5c,0x4e,0x1d,0x53,0x52,0x49,0x1d,0x5f,0x58,0x58,0x53,0x1d,0x54,0x50,0x4d,0x51,0x58,0x50,0x58,0x53,0x49,0x58,0x59]

private func toHidden(max num: UInt8) -> UInt8 {
    return num ^ 61
}

/*: "icon_male_default" :*/
fileprivate let kComeConst:[UInt8] = [0x5f,0x55,0x59,0x58,0x69,0x5b,0x57,0x5a,0x53,0x69,0x52,0x53,0x50,0x57,0x43,0x5a,0x42]

private func answerName(model num: UInt8) -> UInt8 {
    return num ^ 54
}

/*: "icon_female_default" :*/
fileprivate let dataTitleMain:String = "icon_in color fatal image"
fileprivate let kSharedMain:String = "le_dname a key if"
fileprivate let constAddItemK:String = "efauleastern"

/*: "level_%@" :*/
fileprivate let kModelCellTextData:[Character] = ["l","e","v","e","l","_","%","@"]
...
class FirstInfoView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.fillInAddConstraint()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: showColorK.map{toHidden(max: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .viewPath(type: .Medium, fontSize: 17)
        //: $0.textColor = .white
        $0.textColor = .white
    }
    ...
    lazy var ageLabel = UILabel().then {
        ...
        $0.font = .viewPath(type: .Regular, fontSize: 10)
        //: $0.textColor = .white
        $0.textColor = .white
    }
...
extension FirstInfoView {
    //: func updateContentWithModel(model: TPreviewInfoModel) {
    func extra(model: AudioSystemInfoModel) {
        //: headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.placeImgSquare())
        headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.changeSquare())
        ...
        let imgStr = model.sex == PetitionKeyRepresentable.male.rawValue ? String(bytes: kComeConst.map{answerName(model: $0)}, encoding: .utf8)! : (String(dataTitleMain.prefix(5)) + "fema" + String(kSharedMain.prefix(4)) + constAddItemK.replacingOccurrences(of: "eastern", with: "t"))
        //: sexImgView.image = UIImage(named: imgStr)
        sexImgView.image = UIImage(named: imgStr)
        //: let levelStr = String(format: "level_%@", model.level)
        let levelStr = String(format: (String(kModelCellTextData)), model.level)
...
extension FirstInfoView {
    ...
    private func fillInAddConstraint() {
