...
fileprivate let showSourceApp:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Edit Title" :*/
fileprivate let showVideoUser:String = "make self table if colorEdit"
fileprivate let kAddMain:[Character] = ["e"]

/*: "0/ :*/
fileprivate let constDelayK:[Character] = ["0","/"]

/*: "Finish" :*/
fileprivate let kTableViewUser:[Character] = ["F","i","n","i","s","h"]

/*: "btn_delete" :*/
fileprivate let userListConst:[Character] = ["b","t","n","_","d","e","l","e","t","e"]

/*: / :*/
fileprivate let dataRequestNoti:[Character] = ["/"]

/*: "\n" :*/
fileprivate let userEqualConst:String = "\n"
...
class HomeThen: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(barCollection))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showSourceApp.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var titleLabel: UILabel = {
        ...
        lab.text = (String(showVideoUser.suffix(4)) + " Titl" + String(kAddMain)).localized
    ...
    lazy var inputTView: MessageHasDelegate = {
    ...
    private lazy var finishBtn: UIButton = {
        ...
        btn.setTitle((String(kTableViewUser)).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.analogDigitalConverterSize(colors: UIColor.animaBy(), size: CGSize(width: 267, height: 50)), for: .normal)
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.imageName(name: (String(userListConst))), for: .normal)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(checked), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension HomeThen: UITextViewDelegate {
    ...
    public func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > lengthLimit {
            ...
            if textNum > lengthLimit && lengthLimit > 0 {
                ...
                textView.text = immoderationName(index: lengthLimit, text: textContent)
            }
        }
    ...
    private func immoderationName(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
