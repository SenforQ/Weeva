...
fileprivate let dataAttentionK:[UInt8] = [0x19,0x1e,0x19,0x4,0x58,0x13,0x1f,0x14,0x15,0x2,0x4a,0x59,0x50,0x18,0x11,0x3,0x50,0x1e,0x1f,0x4,0x50,0x12,0x15,0x15,0x1e,0x50,0x19,0x1d,0x0,0x1c,0x15,0x1d,0x15,0x1e,0x4,0x15,0x14]

/*: "Add" :*/
fileprivate let showUserConst:[Character] = ["A","d","d"]

/*: "btn_me_edit" :*/
fileprivate let dataSharedUser:String = "effect execution interactionbtn_m"
fileprivate let mainUpConst:String = "e_editpath import catch"

/*: "Program" :*/
fileprivate let showAspectData:String = "top dataProgram"
...
class DetailViewCell: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataAttentionK.map{$0^112}, encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(plonkDown), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var tipsLab: UILabel = {
        ...
        lb.text = (String(showUserConst)).localized
        //: return lb
        return lb
        //: }()
    }()
    ...
    private lazy var editImg: UIImageView = {
        ...
        let imag = UIImageView(image: UIImage.imageName(name: (String(dataSharedUser.suffix(5)) + String(mainUpConst.prefix(6)))))
        //: imag.isUserInteractionEnabled = false
        imag.isUserInteractionEnabled = false
        //: return imag
        return imag
...
extension DetailViewCell {
    ...
    func worldView(title: String, icon: String) {
        //: iconImg.image = UIImage.BundleImageNamed(name: icon)
        iconImg.image = UIImage.imageName(name: icon)
        //: titleLab.text = title.localized
        titleLab.text = title.localized
        //: if title == "Program" {
        if title == (String(showAspectData.suffix(7))) {
    ...
    func someone() {
        ...
        backBtn.willRadii(width: userDeviceKey - 30, height: 50, corners: [.topLeft, .topRight], cornerRadii: .init(width: 8, height: 8))
    }
}
...
extension DetailViewCell {
    ...
    private func balefireConstraint() {
        //: backBtn.snp.remakeConstraints { make in
        backBtn.snp.remakeConstraints { make in
            //: make.leading.top.equalToSuperview().offset(15)
            make.leading.top.equalToSuperview().offset(15)
