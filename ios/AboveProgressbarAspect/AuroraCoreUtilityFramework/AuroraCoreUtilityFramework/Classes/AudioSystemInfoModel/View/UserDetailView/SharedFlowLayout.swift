...
protocol PlayerLayoutDelegate: NSObjectProtocol {
    //: func getCollectuonViewHeight(layoutH: CGFloat)
    func checkOff(layoutH: CGFloat)
}
...
class SharedFlowLayout: UICollectionViewFlowLayout {
    ...
    var delegate: PlayerLayoutDelegate?
    ...
    override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
        ...
        for i in 1..<attributes.count {
            ...
            if i == attributes.count-1 {
                ...
                self.delegate?.checkOff(layoutH: curAttr.frame.maxY)
            }
        }
