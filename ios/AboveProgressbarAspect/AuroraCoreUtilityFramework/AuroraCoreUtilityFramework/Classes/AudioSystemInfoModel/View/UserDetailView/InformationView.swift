...
fileprivate let appCurrentOkMain:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_userdetail_header_shadow" :*/
fileprivate let userThenDropK:String = "icgiftn"
fileprivate let showValueK:String = "etatablel"
fileprivate let appDataShareNoti:String = "time background position file letder_s"
fileprivate let userNeedMain:[Character] = ["h","a","d","o","w"]

/*: "icon_lounge" :*/
fileprivate let kFrameData:[Character] = ["i","c","o","n","_","l","o","u","n"]
fileprivate let userItemK:[Character] = ["g","e"]

/*: "Official" :*/
fileprivate let showMainK:[Character] = ["O","f","f","i","c","i","a"]
fileprivate let mainFinishApp:String = "top"

/*: "Reviewing" :*/
fileprivate let constTitleUser:[Character] = ["R","e","v","i","e","w","i","n","g"]

/*: "#FF506D" :*/
fileprivate let userRouteData:String = "direction view to next section#FF50"
fileprivate let kWhoNoti:String = "send line view to self6D"

/*: "icon_user_detail_intimate_header" :*/
fileprivate let dataCornerShow:[UInt8] = [0xbf,0xb5,0xb9,0xb8,0x89,0xa3,0xa5,0xb3,0xa4,0x89,0xb2,0xb3,0xa2,0xb7,0xbf,0xba,0x89,0xbf,0xb8,0xa2,0xbf,0xbb,0xb7,0xa2,0xb3,0x89,0xbe,0xb3,0xb7,0xb2,0xb3,0xa4]

/*: "?toUid= :*/
fileprivate let dataLeadingBlockShow:String = "?toUid=to in remove"

/*: "ErrorDetailUserItemCellView" :*/
fileprivate let mainCellUser:String = "follow lab for openTUser"
fileprivate let mainResideStreamShow:String = "lHeadernormal push leading"
fileprivate let mainLayerK:String = "age value gift toItemCell"

/*: "#FFFFFF" :*/
fileprivate let notiSharedData:String = "corner cur event#FFFFFF"

/*: "#BDB8B5" :*/
fileprivate let showBlockTopData:String = "#BDB8model add false angle"
fileprivate let showLabelConst:String = "Bface"

/*: "head_detail_bottom" :*/
fileprivate let userManagerRefreshLabApp:String = "path makehead_"
fileprivate let kLanguageShow:[Character] = ["b"]
fileprivate let constMakeNoti:String = "atttatm"

/*: "#2DF2FF" :*/
fileprivate let dataPanShow:String = "#2DF2string view to equal"
fileprivate let kPathData:[Character] = ["F","F"]

/*: "Live" :*/
fileprivate let showActionNoti:[Character] = ["L","i","v","e"]

/*: "#48FF1E" :*/
fileprivate let notiHeadApp:String = "status if target lab#48FF1"
fileprivate let userRawSizeK:[Character] = ["E"]

/*: "Online" :*/
fileprivate let userPackageNoti:[Character] = ["O","n","l","i","n","e"]

/*: "#FF4B7F" :*/
fileprivate let notiAppMain:[Character] = ["#","F","F","4","B","7","F"]

/*: "busy" :*/
fileprivate let userAtNoti:[Character] = ["b","u","s","y"]
...
let kBarRankData = (302.0 + showDomainUrl)
...
class InformationView: UIView {
    ...
    private var userInfoModel = NameDetailTransformable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appCurrentOkMain.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var bannerView: JXBanner = {
        ...
        view.placeholderImgView.image = UIImage.toBanner()
    ...
    private lazy var shadowBgView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.imageName(name: (userThenDropK.replacingOccurrences(of: "gift", with: "o") + "_userd" + showValueK.replacingOccurrences(of: "table", with: "i") + "_hea" + String(appDataShareNoti.suffix(5)) + String(userNeedMain))))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var vipBgView: UIImageView = {
        ...
        v.frame = CGRect(x: 0, y: kBarRankData - 15, width: userDeviceKey, height: 15)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.imageName(name: (String(kFrameData) + String(userItemK)))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var officalImgV: UIButton = {
        ...
        img.setTitle((String(showMainK) + mainFinishApp.replacingOccurrences(of: "top", with: "l")).localized, for: .normal)
    ...
    private lazy var onlineImgV: EmptyButton = {
    ...
    private lazy var liveView: LabelStreamsView = {
        //: let v = UserLivePullStreamsView()
        let v = LabelStreamsView()
        //: v.isHidden = true
        v.isHidden = true
        //: v.playEndBlock = {
        v.playEndBlock = {
        ...
        v.tapGestureBlock = { [weak self] in
            ...
            guard let self = self else { return }
            //: ViewPushManager.share.func_audiencePushToLiveRoomVC(uid: self.userInfoModel.uid, enterType: .userDetail)
            ViewPushManager.share.funcInsideBusinessCycle(uid: self.userInfoModel.uid, enterType: .userDetail)
        }
        //: return v
        return v
        //: }()
    ...
    private lazy var reViewingLB: UILabel = {
        ...
        lb.text = (String(constTitleUser)).localized
        //: lb.textColor = .white
        lb.textColor = .white
        //: lb.backgroundColor = UIColor.init(hex: "#FF506D")
        lb.backgroundColor = UIColor(hex: (String(userRouteData.suffix(5)) + String(kWhoNoti.suffix(2))))
    ...
    lazy var lastLoginLab: UILabel = {
        ...
        lb.font = .viewPath(type: .Medium, fontSize: 12)
        //: lb.textColor = .white
        lb.textColor = .white
        //: return lb
        return lb
    ...
    private lazy var intimateBtn: EmptyButton = {
        //: let btn = TalkingButton()
        let btn = EmptyButton()
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_user_detail_intimate_header"), for: .normal)
        btn.setImage(UIImage.imageName(name: String(bytes: dataCornerShow.map{$0^214}, encoding: .utf8)!), for: .normal)
        //: btn.isHidden = true
        btn.isHidden = true
...
extension InformationView {
    ...
    private func tool() {
        ...
        let vc = CoordinateViewController(videoPath: self.userInfoModel.videoUrl)
        //: if PositionLabelReactiveCompatible.share.loginUid != self.userInfoModel.uid {
        if PositionLabelReactiveCompatible.share.loginUid != self.userInfoModel.uid {
    ...
    private func contribution(_ index: Int) {
        ...
        let vc = PhotosViewDelegate(momentModel: currenModel, index: index, type: .normal)
        //: self.currentViewController()?.navigationController?.pushViewController(vc, animated: true)
        self.underTitle()?.navigationController?.pushViewController(vc, animated: true)
    }
    ...
    @objc func snapClick() {
        ...
        let url = ViewThen.rawDataInstance(urlType: .Intimate) + (String(dataLeadingBlockShow.prefix(7))) + "\(self.userInfoModel.uid)"
...
extension InformationView: JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: ErrorDetailUserItemCellView.self, reuseIdentifier: (String(mainCellUser.suffix(5)) + "Detai" + String(mainResideStreamShow.prefix(7)) + String(mainLayerK.suffix(8))))
    }

    //: func jxBanner(_ banner: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        //: let bannerCell = cell as! TUserDetailHeaderItemCell
        let bannerCell = cell as! ErrorDetailUserItemCellView
        //: let urlStr = self.dataArray[index]
        let urlStr = self.dataArray[index]

        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == PetitionKeyRepresentable.female.rawValue && userInfoModel.videoUrl.count > 1 { // 视频
            //: bannerCell.setVideoView()
            bannerCell.labelUp()
            //: bannerCell.configModel(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            bannerCell.viewStatus(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            ...
            bannerCell.heatherMixture()
            //: bannerCell.bgImgView.setUrlImage(urlStr: urlStr, placeImg: UIImage.placeSquareBigImgBanner())
            bannerCell.bgImgView.windowFinish(urlStr: urlStr, placeImg: UIImage.statuteText())
        }
    ...
    func jxBanner(_: JXBannerType, centerIndex: Int, centerCell _: UICollectionViewCell) {
        //: guard userInfoModel.uid == PositionLabelReactiveCompatible.share.loginUid else {
        guard userInfoModel.uid == PositionLabelReactiveCompatible.share.loginUid else {
            //: return
            return
        }
    ...
    func jxBanner(pageControl _: JXBannerType, numberOfPages _: Int, coverView: UIView, builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder {
        ...
        pageControl.activeColor = UIColor(hex: (String(notiSharedData.suffix(7))))!
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(showBlockTopData.prefix(5)) + showLabelConst.replacingOccurrences(of: "face", with: "5")))!
...
extension InformationView: JXBannerDelegate {
    ...
    public func jxBanner(_: JXBannerType, didSelectItemAt index: Int) {
        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == PetitionKeyRepresentable.female.rawValue && userInfoModel.videoUrl.count > 1 {
            //: goToVideo()
            tool()
            ...
            if userInfoModel.sex == PetitionKeyRepresentable.female.rawValue && userInfoModel.videoUrl.count > 1 && curren > 0 {
                //: curren -= 1
                curren -= 1
            }
            //: goToPhoto(curren)
...
extension InformationView {
    ...
    func source(_ userModel: NameDetailTransformable) {
        ...
        if userModel.sex == PetitionKeyRepresentable.female.rawValue && userModel.videoUrl.count > 1 {
        ...
        let normalImg = UIImage.imageName(name: (String(userManagerRefreshLabApp.suffix(5)) + "detail_" + String(kLanguageShow) + constMakeNoti.replacingOccurrences(of: "at", with: "o"))).withTintColor(.white)
        //: if userModel.loungePlus {
        if userModel.loungePlus {
            //: if let vipModel = PositionLabelReactiveCompatible.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userModel.vipSkinId}).first {

            if let vipModel = PositionLabelReactiveCompatible.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userModel.vipSkinId }).first {
                //: vipBgView.setUrlImage(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
                vipBgView.windowFinish(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
            }
            //: } else {
        } else {
        ...
        self.nicknameLab.textColor = userModel.loungePlus ? .endUserForColoration() : .white
        ...
        if userModel.intimateUrl.count > 0 {
            ...
            intimateImgV.degas(urlStr: userModel.intimateUrl)
        }
        // 在线状态
        ...
        if userModel.livePlayUrl.count > 0 {
            ...
            let image = UIImage.requestSize(color: UIColor(hex: (String(dataPanShow.prefix(5)) + String(kPathData)))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.on(), for: .normal)
            onlineImgV.setImage(image.on(), for: .normal)
            //: onlineImgV.setTitle("Live".localized, for: .normal)
            onlineImgV.setTitle((String(showActionNoti)).localized, for: .normal)
            ...
            let image = UIImage.requestSize(color: UIColor(hex: (String(notiHeadApp.suffix(6)) + String(userRawSizeK)))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.on(), for: .normal)
            onlineImgV.setImage(image.on(), for: .normal)
            //: onlineImgV.setTitle("Online".localized, for: .normal)
            onlineImgV.setTitle((String(userPackageNoti)).localized, for: .normal)
            ...
            let image = UIImage.requestSize(color: UIColor(hex: (String(notiAppMain)))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.on(), for: .normal)
            onlineImgV.setImage(image.on(), for: .normal)
            //: onlineImgV.setTitle("busy".localized, for: .normal)
            onlineImgV.setTitle((String(userAtNoti)).localized, for: .normal)
            //: } else {
        } else {
        ...
        // 直播
        //: if PositionLabelReactiveCompatible.share.loginUid != userModel.uid &&
        if PositionLabelReactiveCompatible.share.loginUid != userModel.uid,
        ...
        if liveView.isHidden == false {
            ...
            liveView.beforeNeighbour(liveUrl: userModel.livePlayUrl)
        }
    }
    ...
    private func noSub() {
        ...
        liveView.snp.makeConstraints { make in
            ...
            make.top.equalTo(showDomainUrl + 80)
        ...
        reViewingLB.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(showDomainUrl + 15)
