...
fileprivate let notiTitleValueMain:[UInt8] = [0x7,0x0,0x7,0x1a,0x46,0xd,0x1,0xa,0xb,0x1c,0x54,0x47,0x4e,0x6,0xf,0x1d,0x4e,0x0,0x1,0x1a,0x4e,0xc,0xb,0xb,0x0,0x4e,0x7,0x3,0x1e,0x2,0xb,0x3,0xb,0x0,0x1a,0xb,0xa]

private func sendNow(from num: UInt8) -> UInt8 {
    return num ^ 110
}
...
// __CLOSE_PRINT__
//
//  PercentageTextCurrentReactiveCompatible.swift
//  AbroadTalking
//
...
class PercentageTextCurrentReactiveCompatible: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiTitleValueMain.map{sendNow(from: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var progressView: GiftReactiveCompatible = {
        ...
        let v = GiftReactiveCompatible(frame: self.frame, progressBgColor: self.progressBgColor, progressPlayedColor: self.progressPlayedColor, progressCachedColor: self.progressCachedColor)
    ...
    private lazy var panGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(gestureGlobalHighlight(gesture:)))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(finish(gesture:)))
        //: return v
        return v
        //: }()
    }()
...
extension PercentageTextCurrentReactiveCompatible {
    //: @objc func panGesture(gesture: UIPanGestureRecognizer) {
    @objc func gestureGlobalHighlight(gesture: UIPanGestureRecognizer) {
        // 移动的距离
    ...
    @objc func finish(gesture: UIPanGestureRecognizer) {
...
extension PercentageTextCurrentReactiveCompatible {
    ...
    private func nameAppConstraint() {
