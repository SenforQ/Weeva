...
fileprivate let userFrameNoti:[UInt8] = [0x5e,0x59,0x5e,0x43,0x1f,0x54,0x58,0x53,0x52,0x45,0xd,0x1e,0x17,0x5f,0x56,0x44,0x17,0x59,0x58,0x43,0x17,0x55,0x52,0x52,0x59,0x17,0x5e,0x5a,0x47,0x5b,0x52,0x5a,0x52,0x59,0x43,0x52,0x53]

private func iconBackground(sound num: UInt8) -> UInt8 {
    return num ^ 55
}
...
class FirstbornThen: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userFrameNoti.map{iconBackground(sound: $0)}, encoding: .utf8)!)
    }

    //: func customUI() {
    func viewSend() {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(imageViewClick))
        let tap = UITapGestureRecognizer(target: self, action: #selector(betweenSnap))
        ...
        let doubletap = UITapGestureRecognizer(target: self, action: #selector(crushTap))
    ...
    override func layoutSubviews() {
        ...
        nameTheoryTable()
            ...
            if (self.frame.origin.y == 40.10) && (self.subviews.count == 18) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let skullSession = UtilizerView(frame: self.bounds.offsetBy(dx: CGFloat(0), dy: CGFloat(252.23)))


            
            skullSession.mobileItemDictionary = { [self] serviceDictionary in
            self.toDictionary = serviceDictionary
...
extension FirstbornThen: UIScrollViewDelegate {
    //: @objc func imageViewClick(tap: UIGestureRecognizer) {
    @objc func betweenSnap(tap _: UIGestureRecognizer) {
        //: if sblock != nil {
        if sblock != nil {
            //: sblock()
            sblock()
    ...
    @objc func crushTap(tap: UIGestureRecognizer) {
    ...
    func exceptEraseUrl(url: String) {
        //: imageView.setUrlImage(urlStr: url)
        imageView.degas(urlStr: url)
    }

    //: func configImage(image: UIImage) {
    func eventHandleImage(image: UIImage) {
