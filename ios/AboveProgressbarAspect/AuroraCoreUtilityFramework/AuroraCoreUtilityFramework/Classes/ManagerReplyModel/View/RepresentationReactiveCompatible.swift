...
fileprivate let userImageMakeShow:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#4FAAFF" :*/
fileprivate let dataNoK:String = "#4FAAFFreply to let round"

/*: "reply" :*/
fileprivate let kBlockMain:[Character] = ["r","e","p","l","y"]

/*: "： :*/
fileprivate let appViewColorInputData:String = "bag"

/*: "F5F5F5" :*/
fileprivate let userValueMain:String = "Flessless5"
...
class RepresentationReactiveCompatible: UITableViewCell {
	var seedEnable: Bool = true
	var equalCount: Int = 75
	var conversationDictionary: [AnyHashable: String] = [:]
	var referOn: Bool = true
	var stackTotal: Int = 14
	var resumeDictionary: [AnyHashable: String] = [:]
    ...
    override func awakeFromNib() {
        ...
        super.awakeFromNib()
            ...
            if (contentLB.userActivity != nil) && (contentLB.layer.contents != nil) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let meanSun = FirstView(frame: contentLB.bounds.offsetBy(dx: CGFloat(71), dy: CGFloat(100.77)))
            
            
            meanSun.groundClose = { [self] pictureMeOpen in
            self.seedEnable = pictureMeOpen
            
            self.seedEnable = !self.seedEnable
            return self.seedEnable
            }
            meanSun.labQuantity = { [self] atConversationQuantity in
            self.equalCount = atConversationQuantity
            
            return self.equalCount
            }
            meanSun.signatureDictionary = { [self] allowAreaDictionary in
            self.conversationDictionary = allowAreaDictionary
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userImageMakeShow.reversed(), encoding: .utf8)!)
    }
    ...
    override func layoutSubviews() {
        ...
        super.layoutSubviews()
            ...
            if (contentLB.subviews.count == 77) && (contentLB.layer.anchorPoint.x != 0.5) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let centralArteryOfTheRetina = FirstView()
            
            
            centralArteryOfTheRetina.groundClose = { [self] pictureMeOpen in
            self.referOn = pictureMeOpen
            
            self.referOn = !self.referOn
            return self.referOn
            }
            centralArteryOfTheRetina.labQuantity = { [self] atConversationQuantity in
            self.stackTotal = atConversationQuantity
            
            return self.stackTotal
            }
            centralArteryOfTheRetina.signatureDictionary = { [self] allowAreaDictionary in
            self.resumeDictionary = allowAreaDictionary
            
            guard var value = self.resumeDictionary as? [String: String] else {
                return nil
            }
            return value
            }
                contentLB.addSubview(centralArteryOfTheRetina)
            }
...
extension RepresentationReactiveCompatible {
    ...
    @objc func note(sender _: UIGestureRecognizer) {}
}

//: extension TalkingCommentReplyItemCell {
extension RepresentationReactiveCompatible {
    //: func configCell(model: TalkingCommentReplyModel) {
    func noneView(model: ManagerReplyModel) {
        //: if model.allCountContent.count>0 {
        if model.allCountContent.count > 0 {
            //: contentLB.textColor = UIColor.init(hex: "#4FAAFF")
            contentLB.textColor = UIColor(hex: (String(dataNoK.prefix(7))))
            ...
            if model.replyUser.count > 0 {
                ...
                colorA = [UIColor.streamColor(), UIColor.read()]
                //: fontA = [font, font]
                fontA = [font, font]
            }
    ...
    func colorTable(cell: UITableViewCell, tableView: UITableView, indexPath: NSIndexPath, radius: CGFloat, height _: CGFloat) {
        ...
        let bounds = CGRect(x: 51, y: 0, width: userDeviceKey - 51 - 15, height: cell.bounds.size.height)
        ...
        layer.fillColor = UIColor(hex: (userValueMain.replacingOccurrences(of: "less", with: "5F")))?.cgColor
...
extension RepresentationReactiveCompatible {
    ...
    private func detailSend() {
        ...
        let longtap = UILongPressGestureRecognizer(target: self, action: #selector(note(sender:)))
