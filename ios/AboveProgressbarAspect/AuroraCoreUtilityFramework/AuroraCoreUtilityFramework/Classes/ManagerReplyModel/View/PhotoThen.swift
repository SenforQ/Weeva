...
fileprivate let notiPartyK:[UInt8] = [0x4b,0x4c,0x4b,0x56,0xa,0x41,0x4d,0x46,0x47,0x50,0x18,0xb,0x2,0x4a,0x43,0x51,0x2,0x4c,0x4d,0x56,0x2,0x40,0x47,0x47,0x4c,0x2,0x4b,0x4f,0x52,0x4e,0x47,0x4f,0x47,0x4c,0x56,0x47,0x46]

private func stemCustomer(session num: UInt8) -> UInt8 {
    return num ^ 34
}

/*: "btn_dynamic_stop_nor" :*/
fileprivate let dataModelMain:String = "btn_dif view type sale self"
fileprivate let showBarInfoConst:String = "c_stoheight background name model"
fileprivate let userThreeConst:[Character] = ["p","_","n","o","r"]

/*: "btnwOblEh_tdj_oediv_stop" :*/
fileprivate let showCheckLook:[Character] = ["b","t","n","w","O","b","l","E","h","_","t","d","j","_","o","e"]
fileprivate let appUpMain:String = "make limit messagediv_stop"

/*: "btnCmjoS7_tdj_oediv_play" :*/
fileprivate let kBlockData:[Character] = ["b","t","n","C","m","j","o","S","7","_","t","d","j"]
fileprivate let kEveryApp:[Character] = ["_","o","e"]
fileprivate let constLabelK:String = "div_playmodel cell let var layer"

/*: "%@" :*/
fileprivate let appStartStatusConst:String = "%from"
...
protocol ModelViewDelegate: NSObject {
    ...
    func belowTap(view: PhotoThen, status: TabViewStatus)

    //: func func__interactionViewSlideValue(view: TalkingVideoPlayerInteractionView, value: CGFloat)
    func bottom(view: PhotoThen, value: CGFloat)
}

//: class TalkingVideoPlayerInteractionView: UIView {
class PhotoThen: UIView {
    //: open weak var delegate: InteractionViewDelegate?
    open weak var delegate: ModelViewDelegate?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiPartyK.map{stemCustomer(session: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var centerPlayButton: UIButton = {
        ...
        btn.setImage(UIImage.imageName(name: (String(dataModelMain.prefix(5)) + "ynami" + String(showBarInfoConst.prefix(5)) + String(userThreeConst))), for: .normal)
        //: btn.addTarget(self, action: #selector(centerButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(wait(sender:)), for: .touchUpInside)
        //: btn.isHidden = true
        btn.isHidden = true
        //: return btn
        return btn
    ...
    private lazy var playButton: UIButton = {
        ...
        btn.setImage(UIImage.imageName(name: (String(showCheckLook) + String(appUpMain.suffix(8)))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btnCmjoS7_tdj_oediv_play"), for: .selected)
        btn.setImage(UIImage.imageName(name: (String(kBlockData) + String(kEveryApp) + String(constLabelK.prefix(8)))), for: .selected)
        //: btn.addTarget(self, action: #selector(playButtonClickAction(sender:)), for: .touchUpInside)
        btn.addTarget(self, action: #selector(clickDownWith(sender:)), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var failImageView: UIImageView = {
        ...
        imag.image = UIImage.imageName(name: "")
        //: imag.isHidden = true
        imag.isHidden = true
    ...
    private lazy var slider: PercentageTextCurrentReactiveCompatible = {
        ...
        let slid = PercentageTextCurrentReactiveCompatible(frame: CGRect(x: 100, y: 0, width: userDeviceKey - 167, height: 40), sliderWidth: 12, sliderColor: UIColor.white, progressHeight: 2, progressBgColor: UIColor(red: 255 / 255.0, green: 255 / 255.0, blue: 255 / 255.0, alpha: 0.3), progressPlayedColor: UIColor.white, progressCachedColor: UIColor.clear, isShowCorner: true)
...
extension PhotoThen {
    ...
    private func month() {
        ...
        self.slider.panBeginBlock = { [weak self] in
            ...
            self.licenseContent(status: .StartSeek)
        }
        ...
        self.slider.getValueBlock = { [weak self] value in
            ...
            self.delegate?.bottom(view: self, value: value)
        }
        ...
        self.slider.panEndHandler = { [weak self] _ in
            ...
            self.licenseContent(status: .StopSeek)
        }
    }
}
