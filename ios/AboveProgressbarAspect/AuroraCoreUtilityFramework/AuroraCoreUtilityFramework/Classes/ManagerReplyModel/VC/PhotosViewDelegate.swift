...
fileprivate let dataContainerBeyondMain:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "FirstbornThen" :*/
fileprivate let showSucceedUser:[Character] = ["T","a","l","k","i","n","g","M","o","m","e","n","t","P","h","o","t","o","s","P","r","e","v","i","e","w","C","e","l","l"]

/*: "Save successfully" :*/
fileprivate let showAddMain:String = "label gift letSave "
fileprivate let showUpMain:[Character] = ["s","u","c","c","e","s","s","f","u","l","l","y"]

/*: / :*/
fileprivate let notiMakeConst:[Character] = ["/"]
...
class PhotosViewDelegate: ShareReactiveCompatible {
	var effectComponentArray: [AnyHashable] = []
    ...
    init(momentModel: TitleMomentModel, index: Int, type: GiftFirstPhotoType) {
        ...
        if ManagerReactiveCompatible.shared.direction == .rightToLeft {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                ...
                self.collecView.contentOffset = CGPoint(x: curren * Int(userDeviceKey), y: 0)
    ...
    init(images: [UIImage], index: Int, type: GiftFirstPhotoType) {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: myType = type
        myType = type
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataContainerBeyondMain.reversed(), encoding: .utf8)!)
    }
...
//        }
    ...
    override func sendPlayerClick() {
        //: super.publishDeleteButtonClick()
        super.sendPlayerClick()
        ...
        since()
            ...
            blockText.shouldNameTextDoing = isHidebottomView
            blockText.nameInterval = pageIndex

            
                self.navigationController?.present(blockText.self, animated: false) { [self] in
            self.effectComponentArray = []
                }
            }
    ...
    lazy var collecView: UICollectionView = {
        ...
        flayout.itemSize = CGSize(width: userDeviceKey, height: appNameMessage)
        ...
        view.frame = CGRect(x: 0, y: 0, width: userDeviceKey, height: appNameMessage)
        ...
        view.contentOffset = CGPoint(x: pageIndex * Int(userDeviceKey), y: 0)
        //: if myType == .normal {
        if myType == .normal {
            //: view.contentSize = CGSize(width: (currenModel.pic.count )*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (currenModel.pic.count) * Int(userDeviceKey), height: 0)
            //: } else {
        } else {
            //: view.contentSize = CGSize(width: (photos.count)*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (photos.count) * Int(userDeviceKey), height: 0)
        }
        //: view.register(TalkingMomentPhotosPreviewCell.self, forCellWithReuseIdentifier: "TalkingMomentPhotosPreviewCell")
        view.register(FirstbornThen.self, forCellWithReuseIdentifier: (String(showSucceedUser)))
...
//    lazy var downloadBtn: EmptyButton = {
//        let btn = EmptyButton.init()
...
//        TimingThen.func__openAlbumServiceWithBlock(true) { [weak self] (isOpen: Bool) in
//            if isOpen {
//                guard let self = self else {return}
//                guard  self.pageIndex >= 0 && self.pageIndex < self.currenModel.pic.count  else { return }
//                let model: PlayerTransformable = self.currenModel.pic[self.pageIndex]
...
//    }
    ...
    func libraryArray(image: UIImage) {
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if isSuccess {
                ...
                DispatchQueue.main.async {
                    ...
                    self.barPrice(showMsg: (String(showAddMain.suffix(5)) + String(showUpMain)).localized)
                }
                //: } else {
            } else {
...
extension PhotosViewDelegate: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    //: func hideTopView() {
    func sortEqualView() {
...
//        downloadBtn.isHidden = isHideTopView
    ...
    func nameHide() {
        //: if  currenModel.sex==PositionLabelReactiveCompatible.share.loginUserMode.sex {
        if currenModel.sex == PositionLabelReactiveCompatible.share.loginUserMode.sex {
            //: return
            return
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: FirstbornThen = collectionView.dequeueReusableCell(withReuseIdentifier: (String(showSucceedUser)), for: indexPath) as! FirstbornThen
        //: if myType == .normal {
        if myType == .normal {
            //: let model: TalkingMomentPhotoModel = currenModel.pic[indexPath.row]
            let model: PlayerTransformable = currenModel.pic[indexPath.row]
            //: if model.imgUrl?.count ?? 0 > 1 {
            if model.imgUrl?.count ?? 0 > 1 {
                //: cell.configUrl(url: model.imgUrl! )
                cell.exceptEraseUrl(url: model.imgUrl!)
                //: }else {
            } else {
                //: cell.configImage(image: model.imagePic ?? UIImage.init())
                cell.eventHandleImage(image: model.imagePic ?? UIImage())
            }

            //: } else {
        } else {
            //: cell.configImage(image: photos[indexPath.row])
            cell.eventHandleImage(image: photos[indexPath.row])
        }
    ...
    func collectionView(_: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt _: IndexPath) {
        ...
        let tcell: FirstbornThen = cell as! FirstbornThen
        //: tcell.recoverSubviews()
        tcell.nameTheoryTable()
    }
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(offSetWidth / userDeviceKey)
        //: if myType == .normal {
        if myType == .normal {
