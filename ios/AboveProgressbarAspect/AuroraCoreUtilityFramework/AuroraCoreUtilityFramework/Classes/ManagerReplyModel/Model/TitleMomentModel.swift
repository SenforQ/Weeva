...
/*: "id" :*/
fileprivate let kNeedUser:String = "isize"
...
struct TitleMomentModel: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
    ...
    var pic = [PlayerTransformable]()
    //: var momentType: String?
    var momentType: String?
    //: var videoInfo: TalkingMomentVideoModel?
    var videoInfo: ImageTransformable?
...
struct PlayerTransformable: HandyJSON {
    //: var thumbUrl: String?
    var thumbUrl: String?
    //: var imgUrl: String?
    var imgUrl: String?
...
enum ManagerNameConvertible: String {
    //: case nones  = "0"
    case nones = "0"
    //: case Photo = "1"
    case Photo = "1"
...
struct ImageTransformable: HandyJSON {
    ...
    var status: ListExpressionConvertible?
}

///
//: enum MomentVideoStatus: String {
enum ListExpressionConvertible: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension TitleMomentModel {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func match(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(TelecastingThen.soap(text: content!, size: CGSize(width: userDeviceKey - 54, height: CGFloat(MAXFLOAT)), font: UIFont.viewPath(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == ManagerNameConvertible.Photo.rawValue || momentType == ManagerNameConvertible.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
