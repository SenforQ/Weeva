...
/*: "id" :*/
fileprivate let appEffectShow:String = "ID"

/*: "%@ reply %@：%@" :*/
fileprivate let constTabShow:String = "%@ reequal not make to"
fileprivate let constBackUser:String = "model：%model"

/*: " %@：%@" :*/
fileprivate let notiSpaceShow:[Character] = [" ","%","@","：","%","@"]
...
struct ManagerReplyModel: HandyJSON {
    //: var cid: String?
...
extension ManagerReplyModel {
    //: mutating func caculateReplyItemHeight() {
    mutating func nameure() {
        //: var tempContent = ""
        var tempContent = ""
        //: if replyUser.count>0 {
        if replyUser.count > 0 {
            //: tempContent = String(format: "%@ reply %@：%@", nickname, replyUser, content)
            tempContent = String(format: (String(constTabShow.prefix(5)) + "ply %" + constBackUser.replacingOccurrences(of: "model", with: "@")), nickname, replyUser, content)
            //: } else {
        } else {
        ...
        let sizeheight = Int(TelecastingThen.soap(text: tempContent, size: CGSize(width: userDeviceKey - 82, height: CGFloat(MAXFLOAT)), font: UIFont.viewPath(type: .Regular, fontSize: 14), lineNumber: 0).height)
