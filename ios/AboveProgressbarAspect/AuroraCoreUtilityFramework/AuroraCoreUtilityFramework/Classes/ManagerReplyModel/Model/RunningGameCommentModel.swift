...
/*: "id" :*/
fileprivate let userExamineedApp:String = "idate"

/*: "reply" :*/
fileprivate let userAppK:String = "REPLY"

/*: :  :*/
fileprivate let constSearchUser:String = ": "

/*:  : :*/
fileprivate let appListConst:[Character] = [" ",":"]
...
struct RunningGameCommentModel: HandyJSON {
    //: var cid: String?
...
extension RunningGameCommentModel {
    ...
    mutating func have() {
        ...
        if replyUser?.count ?? 0 > 0 {
            ...
            if ManagerReactiveCompatible.shared.direction == .leftToRight {
                ...
                tempContent = (userAppK.lowercased()).localized + "\(replyUser!): \(content!)"
                //: } else {
            } else {
                //: tempContent = "\(content!) :\(replyUser!)" + "reply".localized
                tempContent = "\(content!) :\(replyUser!)" + (userAppK.lowercased()).localized
            }
        ...
        let sizeheight = Int(TelecastingThen.soap(text: tempContent, size: CGSize(width: userDeviceKey - 70, height: CGFloat(MAXFLOAT)), font: UIFont.viewPath(type: .Regular, fontSize: 16), lineNumber: 0).height)
