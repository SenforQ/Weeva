...
/*: "%@%@" :*/
fileprivate let notiTimeMain:String = "%@%@"

/*: "/TXCache" :*/
fileprivate let mainSexNoti:String = "/TXCachetitle strength height manager"
...
protocol EscapeVelocityReactiveCompatible: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func corner(player: PlayerManager, status: MenuPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func groupTo(player: PlayerManager, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func smart(player: PlayerManager, progress: CGFloat)
}
...
class PlayerManager: NSObject {
    ...
    open weak var delegate: EscapeVelocityReactiveCompatible?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: MenuPlayerStatus?
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(mainSexNoti.prefix(8)))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(eraseEmpty), userInfo: nil, repeats: true)
...
extension PlayerManager {
    ...
    func streetwise(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: mainRequestFormat.name6decrypt(), with: appFormalStr)
    ...
    @objc func eraseEmpty() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.smart(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func superposeSendMode(renderMode: AggregationRenderMode) {
    ...
    func indueMuteSet(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isShared == true {
        if CurrentSocketManager.shared.isShared == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func utmost() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.corner(player: self, status: self.playStatus!)
        }
    }

    //: func isPlayings() -> Bool {
    func rectangleView() -> Bool {
...
extension PlayerManager: TXVodPlayListener {
    ...
    func onPlayEvent(_: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.groupTo(player: self, duration: Int(duration), currentTime: Int(currentTime))
