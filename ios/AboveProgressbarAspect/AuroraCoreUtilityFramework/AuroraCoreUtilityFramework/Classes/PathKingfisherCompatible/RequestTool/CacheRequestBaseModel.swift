...
/*: "http://stat. :*/
fileprivate let notiToUser:String = "make index imagehttp:"
fileprivate let showNumbImageApp:String = "manager path tap edit//stat."

/*: .com" :*/
fileprivate let notiMenuMNoti:[Character] = [".","c","o","m"]

/*: "http://" :*/
fileprivate let appModelK:String = "http://data to info equal title"

/*: "https://" :*/
fileprivate let mainLabUser:String = "https:/top reject"
fileprivate let mainVideoConst:String = "/"

/*: "http://m. :*/
fileprivate let kSharedShow:String = "next search up size viewhttp:/"
fileprivate let showDateK:[Character] = ["/","m","."]

/*: "http:// :*/
fileprivate let notiEqualIconK:String = "true view make as viewhttp:"
fileprivate let kModeDarkUser:[Character] = ["/","/"]

/*: "-1" :*/
fileprivate let showManagerUser:[Character] = ["-","1"]

/*: "3,90" :*/
fileprivate let notiCreateSendApp:String = "3,data"

/*: "1990-01-01" :*/
fileprivate let showViewToolK:[Character] = ["1"]
fileprivate let notiRestoreMain:[Character] = ["9","9","0","-","0","1","-","0","1"]

/*: "MM-dd-yyyy" :*/
fileprivate let kToTimeApp:[Character] = ["M","M","-","d","d","-"]
fileprivate let notiSharedApp:String = "livelive"

/*: "id" :*/
fileprivate let userToNoti:String = "name"
...
class CurrentThen: NSObject {
    ...
    var requestType: RitualRequestType?
    ...
    override init() {
        ...
        self.requestServer = UpToDatenessReactiveCompatible.share.serverUrlStr
...
struct ManagerGainBaseResponse: HandyJSON {
    //: var errno: Int!
...
public struct GiftErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct SpeedHandyJSON: HandyJSON {
    ...
    var CSConfig = MainGiftMeasurable() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (String(notiToUser.suffix(5)) + String(showNumbImageApp.suffix(7))) + "\(appFormalStr)" + (String(notiMenuMNoti))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(appModelK.prefix(7)))) && !statDomain.hasPrefix((String(mainLabUser.prefix(7)) + mainVideoConst.capitalized)) {
                //: return "http://".appending(statDomain)
                return (String(appModelK.prefix(7))).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (String(kSharedShow.suffix(6)) + String(showDateK)) + "\(appFormalStr)" + (String(notiMenuMNoti))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (String(notiEqualIconK.suffix(5)) + String(kModeDarkUser)) + "\(urlDomain)"
        }
    }
...
public class BulkTransformable: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [TopModelType] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [TopModelType] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [TopModelType] = [] // 语音收费设置
    ...
    var vipSetting = [PickModelType]() // vip皮肤
    ...
    var tabPageBanner = [ErrorMeasurable]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = TimeMeasurable() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class RowViewHandyJSON: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [MinDetailHandyJSON]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [MinDetailHandyJSON]?
    //: var partner: String?
    var partner: String?
    //: var full: Int?
    var full: Int?
    ...
    var gallery: [DeleteMainMeasurable]?
    //: var status: Int = -1
    ...
    var callPopupSetting = (notiCreateSendApp.replacingOccurrences(of: "data", with: "90")) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.cornerDoinge(self.birthday ?? (String(showViewToolK) + String(notiRestoreMain)), dateFormat: (String(kToTimeApp) + notiSharedApp.replacingOccurrences(of: "live", with: "yy")))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.barPosition().year else { return "0" }
...
struct DeleteMainMeasurable: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct MinDetailHandyJSON: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func upNow(compareTo: MinDetailHandyJSON) -> Bool {
        //: return
        return
...
struct ReportTransformable: HandyJSON {
    //: var aboutMe: Array<UserTagTypeModel>!
    var aboutMe: [ALaCarteMeasurable]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [ALaCarteMeasurable]!
}

//: struct UserTagTypeModel: HandyJSON {
struct ALaCarteMeasurable: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [MinDetailHandyJSON]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct MainGiftMeasurable: HandyJSON {
...
struct PickModelType: HandyJSON {
...
struct TimeMeasurable: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
