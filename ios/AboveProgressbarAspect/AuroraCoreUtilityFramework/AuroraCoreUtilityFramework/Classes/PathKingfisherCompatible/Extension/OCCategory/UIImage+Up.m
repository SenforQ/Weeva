...
- (UIImage *)since:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self cityCenter:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self status:size];
...
- (CGSize)cityCenter:(Boolean)isSession {
...
+ (NSData *)dataOfOrientation:(UIImage *)image littleStatus:(NSInteger)fileSize{
    ...
    if (shortSize>1080) {//压缩到最短边不大于1080,也避免了shortSize==0的情况
        ...
        image = [image conversation:targetSize];
    }

    //: UIImage *thumIMG = image;
    UIImage *thumIMG = image;
    //: NSData *outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
    NSData *outIMGData = [thumIMG activeFile:(fileSize*1024)];
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG gift:scalesize dateTitle:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG activeFile:(fileSize*1024)];
    }
...
- (UIImage*)conversation:(CGSize)targetSize{
...
- (UIImage *)gift:(CGSize)targetSize dateTitle:(BOOL)isScale{
...
- (NSData *)activeFile:(NSInteger)fileSize{
...
+ (UIImage *)duringSend:(UIImage*)orginImage license:(NSUInteger)level {
    // 获取BitmapData
