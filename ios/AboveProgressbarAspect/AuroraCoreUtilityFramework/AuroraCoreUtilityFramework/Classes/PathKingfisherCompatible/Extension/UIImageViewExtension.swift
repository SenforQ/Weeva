...
/*: "add_header_cookies" :*/
fileprivate let showEqualMaxK:[Character] = ["a","d","d","_","h","e","a","d"]
fileprivate let appSexK:String = "title"
fileprivate let kPhoneUser:[Character] = ["r","_","c","o","o","k","i","e","s"]
...
public extension UIImageView {
    ...
    internal func windowFinish(urlStr: String,
                               //: placeImg: UIImage = UIImage.placeImgSquare(),
                               placeImg: UIImage = UIImage.changeSquare(),
    ...
    @objc func setAuditoryImageGiftStr(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: mainRequestFormat.name6decrypt(), with: appFormalStr)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.enabledAction())
    }
    ...
    @objc func colorCarefulAdd(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: UpToDatenessReactiveCompatible.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(showEqualMaxK) + appSexK.replacingOccurrences(of: "title", with: "e") + String(kPhoneUser)))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.changeSquare(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
