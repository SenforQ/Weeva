...
/*: "recharge" :*/
fileprivate let dataWhiteNoti:[Character] = ["r","e","c","h","a","r","g"]
fileprivate let kHiddenUser:String = "make"

/*: "recharge_ :*/
fileprivate let appLayerConst:[Character] = ["r","e","c","h","a","r"]
fileprivate let appViewData:String = "ge_visual manager video"

/*: ".png" :*/
fileprivate let userListK:String = "self share video frame super.png"

/*: "_ :*/
fileprivate let mainColorClickApp:[Character] = ["_"]

/*: .png" :*/
fileprivate let appFragmentConst:[Character] = [".","p","n","g"]

/*: ".jpg" :*/
fileprivate let constViewShow:[Character] = [".","j","p","g"]

/*: .jpg" :*/
fileprivate let kMakeData:[Character] = [".","j","p","g"]
...
enum FirstMentalImageAlignment: NSInteger {
    //: case left = 0
    case left = 0
    //: case top
    case top
...
class EmptyButton: UIButton {
	var clickTotal: Int = 31
	var modelTitle: String = "price"
	var sumercourseDictionary: [AnyHashable: String] = [:]
	var tableCellAgeQuantity: Int = 59
	var calledContent: String = "design"
	var styleAtDictionary: [AnyHashable: String] = [:]
    ...
    override func imageRect(forContentRect contentRect: CGRect) -> CGRect {
        ...
        //: return super.imageRect(forContentRect: contentRect)
            ...
            self.clickTotal = fortTotal
            
            return self.clickTotal
            }
            playGift.chorusTitle = { [self] databaseContent in
            self.modelTitle = databaseContent
            
            return self.modelTitle
            }
            playGift.viewNameDictionary = { [self] nameDictionary in
            self.sumercourseDictionary = nameDictionary
            
            guard var value = self.sumercourseDictionary as? [String: String] else {
                return nil
            }
            return value
            }
    ...
    var imageAlignment: FirstMentalImageAlignment = .left
    ...
    override func layoutSubviews() {
        ...
        case .right:
            ...
            self.tableCellAgeQuantity = fortTotal
            
            return self.tableCellAgeQuantity
            }
            rowGrant.chorusTitle = { [self] databaseContent in
            self.calledContent = databaseContent
            
            return self.calledContent
            }
            rowGrant.viewNameDictionary = { [self] nameDictionary in
            self.styleAtDictionary = nameDictionary
            
            guard var value = self.styleAtDictionary as? [String: String] else {
                return nil
            }
            return value
            }
                self.addSubview(rowGrant)
            }
...
extension UIButton {
    ...
    func errorEqualDismiss(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        resumeOnsetOffset(interval: interval, topOffset: 0)
    }
    ...
    func resumeOnsetOffset(interval: CGFloat, topOffset: CGFloat) {
    ...
    func drunk(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func showCorner(color: UIColor, forState: UIControl.State) {
    ...
    func giftFirst(urlStr: String,
                   //: placeImg: UIImage? = UIImage.placeImgSquare(),
                   placeImg: UIImage? = UIImage.changeSquare(),
                   ...
                   loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: mainRequestFormat.name6decrypt(), with: appFormalStr)
    ...
    func addEnable(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((String(dataWhiteNoti) + kHiddenUser.replacingOccurrences(of: "make", with: "e"))) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (String(dataWhiteNoti) + kHiddenUser.replacingOccurrences(of: "make", with: "e")), with: (String(appLayerConst) + String(appViewData.prefix(3))) + "\(notiBottomKeyValue)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.giftFirst(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(userListK.suffix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(userListK.suffix(4))), with: "_\(notiBottomKeyValue)" + (String(appFragmentConst)))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(constViewShow))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(constViewShow)), with: "_\(notiBottomKeyValue)" + (String(kMakeData)))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.giftFirst(urlStr: newUrl, placeImg: nil)
    }
