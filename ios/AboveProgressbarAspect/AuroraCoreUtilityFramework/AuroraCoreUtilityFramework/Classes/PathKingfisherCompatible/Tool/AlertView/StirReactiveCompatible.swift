...
fileprivate let mainPhotoHiddenK:[Character] = ["b","t","n","_","s","h"]
fileprivate let dataFarK:String = "self by extra let falseow_ale"
fileprivate let showEqualData:[Character] = ["r","t","B","a","c","k","g","r","o","u","n","d"]

/*: "btn_alertBackground_false" :*/
fileprivate let kSizeApp:[Character] = ["b","t","n","_","a","l","e","r","t","B","a","c","k","g","r","o","u","n","d","_","f","a","l","s","e"]
...
public class StirReactiveCompatible: NSObject {
    /// 通用回调
...
public extension StirReactiveCompatible {
    /// 适配器回调,用于给适配器参数赋值
    //: public typealias ConfigAlert = ((_ config: ShowAlertConfig) -> Void)
    typealias ConfigAlert = (_ config: ShapeAlertConfig) -> Void
    ...
    internal class func leftConfig(title: String? = nil,
                                   //: message: String?  = nil,
                                   message: String? = nil,
    ...
    @objc internal class func messageLicense(title: String? = nil,
    ...
    class func claim(title: String? = nil,
                     ...
                     config: ShapeAlertConfig? = nil)
    {
        ...
        let model = config ?? ShapeAlertConfig()

        //: let alertView = TalkingAlertView(title: title,
        let alertView = SharedReactiveCompatible(title: title,
        ...
        //: RowMacroDefine.getWindow().addSubview(alertView)
        RowMacroDefine.apply().addSubview(alertView)
    ...
    class func hidePush() {
        //: RowMacroDefine.getWindow().subviews.forEach { (view) in
        RowMacroDefine.apply().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: SharedReactiveCompatible.self) {
    ...
    class func strengthState(title: String, isTouch: Bool) {
        //: RowMacroDefine.getWindow().subviews.forEach { (view) in
        RowMacroDefine.apply().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: SharedReactiveCompatible.self) {
                //: let cunrenView: TalkingAlertView = view as! TalkingAlertView
                let cunrenView: SharedReactiveCompatible = view as! SharedReactiveCompatible
                //: cunrenView.changeRightBtn(title: title, isTouch: isTouch)
                cunrenView.videoChange(title: title, isTouch: isTouch)
            }
        }
    }
}
...
public class ShapeAlertConfig {
    ...
    public var maskType: EmptyMaskType = .color
    /// 阴影
    ...
    public var titleFont: UIFont = .viewPath(type: .Medium, fontSize: 18)
    /// alert标题字体颜色
    //: public var titleColor  = UIColor.appTitleColor()
    public var titleColor = UIColor.read()
    /// alert信息字体
    //: public var textFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var textFont: UIFont = .viewPath(type: .Medium, fontSize: 16)
    //: public var textColor  = UIColor.appTitleColor()
    public var textColor = UIColor.read()
    ...
    public var buttonFont: UIFont = .viewPath(type: .Medium, fontSize: 16)
    /// alert按钮字体颜色
    //: public var leftColor  = UIColor.appThemeColor()
    public var leftColor = UIColor.requestCardTitle()
    //: public var rightColor = UIColor.white
    public var rightColor = UIColor.white
    //: public var rightBackgroundImage = "btn_show_alertBackground"
    public var rightBackgroundImage = (String(mainPhotoHiddenK) + String(dataFarK.suffix(6)) + String(showEqualData))

    //: public var rightBackgroundImagefalse = "btn_alertBackground_false"
    public var rightBackgroundImagefalse = (String(kSizeApp))
    //: public var rightFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var rightFont: UIFont = .viewPath(type: .Medium, fontSize: 16)
