...
/*: "Select a date" :*/
fileprivate let kSeriesConst:String = "Selecuser var"
fileprivate let kLiveNorNoti:[Character] = ["t"," ","a"," ","d","a","t","e"]

/*: M" :*/
fileprivate let notiDeleteMain:String = "image"

/*: D" :*/
fileprivate let dataVisibleMain:[Character] = ["D"]

/*: Y" :*/
fileprivate let userValueK:[Character] = ["Y"]
...
protocol ImportantObjectProtocol: NSObjectProtocol {
...
class IncomeCapacityDateView: BaseViewPickReactiveCompatible, UIPickerViewDataSource, UIPickerViewDelegate {
	var managerQuantity: Int = 92
	var upQuantity: Int = 1
	var constraint: Int = 3
    ...
    weak var delegate: ImportantObjectProtocol?
    ...
    override func pickMessage() {
        ...
        titleString = (String(kSeriesConst.prefix(5)) + String(kLiveNorNoti)).localized
        ...
        ofBlock(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func ofBlock(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
		...
		if var appValue = defaultMonth { 
			if var voiceOverValue = selectMonth { 
		            if (self.layer.contents != nil) && (self.frame.size.height == 102.14) {
		                //: SWIFT_CUSTOM_DANGER_File_Call
		                let needAfter = PlayView()
		            needAfter.sizeFromTotal = voiceOverValue
		            
		            needAfter.workTitleCount = { [self] logQuantity in
		            self.managerQuantity = logQuantity
		            
		            var needAfter = appValue
		            needAfter |= needAfter | 3
		            if needAfter < self.managerQuantity {
		                self.managerQuantity = needAfter
		            }
    ...
    override func everyToPath(button: UIButton) {
        ...
        if let day = selectDay {
            ...
            delegate?.upDay(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.upDay(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
    ...
    func numberOfComponents(in _: UIPickerView) -> Int {
		...
		if var sectionValue = selectDay { 
	            if (self.layer.isDoubleSided != true) && (self.tintAdjustmentMode == .dimmed) {
	                //: SWIFT_CUSTOM_DANGER_File_Call
	                let stemRow = PlayView(frame: self.frame.integral)
	            stemRow.sizeFromTotal = ShowMonth
	            
	            stemRow.workTitleCount = { [self] logQuantity in
	            self.constraint = logQuantity
	            
	            var stemRow = sectionValue
	            stemRow &-= 1
	            if stemRow > self.constraint {
	                self.constraint = stemRow
	            }
	            
	            return self.constraint
	            }
	                self.addSubview(stemRow)
	            }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return iconMonth(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func iconMonth(year: Int, month: Int) -> Int {
		...
		if var viewValue = defaultYear { 
			if var colorationValue = selectDay { 
		            if (self.layer.contents != nil) && (self.frame.size.height == 102.14) {
		                //: SWIFT_CUSTOM_DANGER_File_Call
		                let needAfter = PlayView()
		            needAfter.sizeFromTotal = colorationValue
		            
		            needAfter.workTitleCount = { [self] logQuantity in
		            self.upQuantity = logQuantity
		            
		            var needAfter = viewValue
		            needAfter |= needAfter | 3
		            if needAfter < self.upQuantity {
		                self.upQuantity = needAfter
		            }
