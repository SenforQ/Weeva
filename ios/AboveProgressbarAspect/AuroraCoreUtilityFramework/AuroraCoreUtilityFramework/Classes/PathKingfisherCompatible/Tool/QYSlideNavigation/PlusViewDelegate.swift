...
protocol PhotoVideoNavigationDelegate: AnyObject {
...
class PlusViewDelegate: UIViewController {
	var demonstrateOff: Bool = false
	var queryCount: Int = 49
    ...
    weak var delegate: PhotoVideoNavigationDelegate?
    ...
    override func viewDidLoad() {
        ...
        view.addSubview(controllerScrollView)
            ...
            if (self.bottomLine.clipsToBounds) && (self.bottomLine.convert(CGRect(x: CGFloat(327.41), y: 0, width: 0, height: 0), to: self.bottomLine.superview).size.height == 94.30) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let sectionBack = PlayerView(frame: self.bottomLine.frame.insetBy(dx: CGFloat(457.07), dy: CGFloat(51)))
            sectionBack.subSwitch = self.isBisect

            
            sectionBack.currentOff = { [self] doingMakeOff in
            self.demonstrateOff = doingMakeOff
            
            var sectionBack = isBisect
                sectionBack = true
                sectionBack = !sectionBack
            if sectionBack != self.demonstrateOff {
                self.demonstrateOff = sectionBack
            }
            
            return self.demonstrateOff
            }
            sectionBack.easyLaySum = { [self] tableNameTotal in
            self.queryCount = tableNameTotal
            ...
            return value
                ...
                self.bottomLine.addSubview(sectionBack)
            }
    ...
    private lazy var controllerScrollView: UIScrollView = {
        ...
        let count = min(self.controllerItems.count, self.titleItems.count)
        //: var height = RowMacroDefine.getScreenHeight() - (topDistance + kTitle_H)
        var height = RowMacroDefine.subjectMatter() - (topDistance + kTitle_H)
        //: if self.hasBttomTabBar {
        if self.hasBttomTabBar {
            //: height -= TabBarViewHeight
            height -= constBeginScreenHeightKey
        }
        ...
        //: let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: RowMacroDefine.getScreenWidth(), height: height)
        let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: RowMacroDefine.customAbout(), height: height)
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        //: scrollView.contentSize = CGSize(width: count * Int(RowMacroDefine.getScreenWidth()), height: 0)
        scrollView.contentSize = CGSize(width: count * Int(RowMacroDefine.customAbout()), height: 0)
        ...
        scrollView.semanticContentAttribute = ManagerReactiveCompatible.shared.direction == .leftToRight ? .forceLeftToRight:.forceRightToLeft
        //: return scrollView
        return scrollView
    //: }()
    }()
    ...
    private lazy var titleScrollView: UIScrollView = {
        //: var rect = CGRect(x: 15, y: topDistance, width: RowMacroDefine.getScreenWidth()-rightSpace, height: kTitle_H)
        var rect = CGRect(x: 15, y: topDistance, width: RowMacroDefine.customAbout()-rightSpace, height: kTitle_H)
        //: if isBisect {
        if isBisect {
            //: rect = CGRect(x: 0, y: topDistance, width: RowMacroDefine.getScreenWidth()-rightSpace, height: kTitle_H)
            rect = CGRect(x: 0, y: topDistance, width: RowMacroDefine.customAbout()-rightSpace, height: kTitle_H)
        }
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        ...
        var originX: CGFloat = ManagerReactiveCompatible.shared.direction == .leftToRight ? 0:25
        ...
        self.titleItems.enumerated().forEach { [weak self] index, obj in
            ...
            titleBtn.addTarget(self, action: #selector(landmarkPlay), for: .touchDown)
            //: var titleWidth = 0.0
            var titleWidth = 0.0
            //: if isBisect {
            if isBisect {
                //: titleWidth = RowMacroDefine.getScreenWidth() / Double(self.titleItems.count)
                titleWidth = RowMacroDefine.customAbout() / Double(self.titleItems.count)
            //: } else {
            } else {
                //: titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
                titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
            }
            //: let x = LanguageManager.shared.direction == .leftToRight ? originX:(RowMacroDefine.getScreenWidth()-originX-titleWidth)
            let x = ManagerReactiveCompatible.shared.direction == .leftToRight ? originX:(RowMacroDefine.customAbout()-originX-titleWidth)
...
extension PlusViewDelegate {
    //: @objc private func titleButtonClick(_ sender: UIButton) {
    @objc private func landmarkPlay(_ sender: UIButton) {
        //: currentSelectedTitleButton(sender)
        technocrat(sender)
        //: let index = sender.tag - 1000
        let index = sender.tag - 1000
        //: let offset_x = CGFloat(index) * RowMacroDefine.getScreenWidth()
        let offset_x = CGFloat(index) * RowMacroDefine.customAbout()
        //: controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        //: addControllerView(withIndex: index)
        sunnaIndex(withIndex: index)
        //: seletedViewController(withIndex: index)
        tapTo(withIndex: index)
    }
    
    //: private func addControllerView(withIndex index: Int) {
    private func sunnaIndex(withIndex index: Int) {
        //: if index >= children.count {
        if index >= children.count {
            //: return
            return
        }
        //: let origin_x = CGFloat(index) * RowMacroDefine.getScreenWidth()
        let origin_x = CGFloat(index) * RowMacroDefine.customAbout()
        ...
        //: vc.view.frame = CGRect(x: origin_x, y: 0, width: RowMacroDefine.getScreenWidth(), height: controllerScrollView.frame.size.height)
        vc.view.frame = CGRect(x: origin_x, y: 0, width: RowMacroDefine.customAbout(), height: controllerScrollView.frame.size.height)
        //: controllerScrollView.addSubview(vc.view)
        controllerScrollView.addSubview(vc.view)
    }
    
    //: private func currentSelectedTitleButton(_ button: UIButton) {
    private func technocrat(_ button: UIButton) {
    ...
    func centerFinish(_ sender: UIButton) {
        //: if titleScrollView.contentSize.width <= RowMacroDefine.getScreenWidth() {
        if titleScrollView.contentSize.width <= RowMacroDefine.customAbout() {
            //: return
            return
        }
        //: var offset_x = sender.center.x - RowMacroDefine.getScreenWidth() * 0.5
        var offset_x = sender.center.x - RowMacroDefine.customAbout() * 0.5
        //: if offset_x < 0 {
        if offset_x < 0 {
            //: offset_x = 0
            offset_x = 0
        }
        //: let maxOffset = titleScrollView.contentSize.width - RowMacroDefine.getScreenWidth()
        let maxOffset = titleScrollView.contentSize.width - RowMacroDefine.customAbout()
...
extension PlusViewDelegate {
    ...
    func tapTo(withIndex index: Int) {
...
extension PlusViewDelegate: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: let index = Int(controllerScrollView.contentOffset.x / RowMacroDefine.getScreenWidth())
        let index = Int(controllerScrollView.contentOffset.x / RowMacroDefine.customAbout())
        //: if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
        if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
            //: currentSelectedTitleButton(selectedBtn)
            technocrat(selectedBtn)
        }
        //: addControllerView(withIndex: index)
        sunnaIndex(withIndex: index)
        //: seletedViewController(withIndex: index)
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        //: let index = Int(scrollView.contentOffset.x / RowMacroDefine.getScreenWidth())
        let index = Int(scrollView.contentOffset.x / RowMacroDefine.customAbout())
        ...
        //: let scaleR = scrollView.contentOffset.x / RowMacroDefine.getScreenWidth() - CGFloat(index)
        let scaleR = scrollView.contentOffset.x / RowMacroDefine.customAbout() - CGFloat(index)
        //: let scaleL = 1 - scaleR
        let scaleL = 1 - scaleR
