...
fileprivate let dataTableLabShow:[Character] = ["A","p","p"]
fileprivate let dataBorderShow:String = "party main none selection animaResou"
fileprivate let showIconUser:String = "ertargetio"

/*: ".zip" :*/
fileprivate let showCircleRequestData:String = ".ziparrow model extension"

/*: "pic" :*/
fileprivate let showCenterUser:[Character] = ["p","i","c"]

/*: "nor" :*/
fileprivate let kGiftApp:String = "norecord"

/*: "svga" :*/
fileprivate let notiPageConst:String = "swindowga"

/*: "mp3" :*/
fileprivate let constPartyData:String = "MP3"

/*: "model" :*/
fileprivate let userOffPathApp:String = "meetingodel"

/*: "language" :*/
fileprivate let constShowK:String = "lanloaduaload"
fileprivate let dataAtLabProgressMain:String = "color"

/*: "@2x.png" :*/
fileprivate let userToolData:[Character] = ["@","2","x",".","p","n","g"]

/*: "png" :*/
fileprivate let notiModeImageUser:[Character] = ["p","n","g"]
...
private let notiSucceedItemCaptureKey = (String(dataTableLabShow) + String(dataBorderShow.suffix(5)) + "rcesV" + showIconUser.replacingOccurrences(of: "target", with: "s") + "nKey")

//: enum SVGAType: String {
enum ReportFrameSystemSendable: String {
...
class ShareImageThen: NSObject {
    //: static let `default` = SVGAEffectTool()
    static let `default` = ShareImageThen()
    ...
    private lazy var AppResPath: String = {
        ...
        let res_Dir = (docuPath as NSString).appendingPathComponent(constLimitFailureKey)
        //: let version = Defaults.string(forKey: AppResourcesVersionKey) ?? ""
        let version = constSizeFormat.string(forKey: notiSucceedItemCaptureKey) ?? ""
        //: if version == AppVersion, FileManager.default.fileExists(atPath: res_Dir) {
        if version == kImageTitle, FileManager.default.fileExists(atPath: res_Dir) {
            //: return res_Dir
            return res_Dir
        }
        //: let path = PodspecBundle.bundle.path(forResource: PodspecName, ofType: ".zip") ?? ""
        let path = EmptyPodspecBundle.bundle.path(forResource: constLimitFailureKey, ofType: (String(showCircleRequestData.prefix(4)))) ?? ""
        ...
        let zipSuc = SSZipArchive.unzipFile(atPath: path,
                                            ...
                                            password: constLimitFailureKey,
                                            //: progressHandler: nil)
                                            progressHandler: nil)
        //: if zipSuc {
        if zipSuc {
            //: Defaults.set(AppVersion, forKey: AppResourcesVersionKey)
            constSizeFormat.set(kImageTitle, forKey: notiSucceedItemCaptureKey)
            //: return res_Dir
            return res_Dir
        }
        //: return ""
    ...
    private lazy var imageResPath: String = //: return (AppResPath as NSString).appendingPathComponent("pic")
        (AppResPath as NSString).appendingPathComponent((String(showCenterUser)))
    //: }()

    //: private lazy var languageImageResPath: String = {
    private lazy var languageImageResPath: String = {
        //: let language = LanguageManager.shared.currLanguage
        let language = ManagerReactiveCompatible.shared.currLanguage
    ...
    private lazy var norImageResPath: String = {
        ...
        return (imageResPath as NSString).appendingPathComponent((kGiftApp.replacingOccurrences(of: "record", with: "r")))
        //: }()
    }()
    ...
    private lazy var svgaResPath: String = //: return (AppResPath as NSString).appendingPathComponent("svga")
        (AppResPath as NSString).appendingPathComponent((notiPageConst.replacingOccurrences(of: "window", with: "v")))
    //: }()
    ...
    private lazy var mp3ResPath: String = //: return (AppResPath as NSString).appendingPathComponent("mp3")
        (AppResPath as NSString).appendingPathComponent((constPartyData.lowercased()))
    //: }()
    ...
    public lazy var modelResPath: String = //: return (AppResPath as NSString).appendingPathComponent("model")
        (AppResPath as NSString).appendingPathComponent((userOffPathApp.replacingOccurrences(of: "meeting", with: "m")))
    //: }()
    ...
    public lazy var languagePath: String = //: return (AppResPath as NSString).appendingPathComponent("language")
        (AppResPath as NSString).appendingPathComponent((constShowK.replacingOccurrences(of: "load", with: "g") + dataAtLabProgressMain.replacingOccurrences(of: "color", with: "e")))
    //: }()
}

//: extension SVGAEffectTool {
extension ShareImageThen {
    /// 直接获取图片
    //: func getImage(name: String) -> UIImage? {
    func finishThoughtImage(name: String) -> UIImage? {
        //: let languagePath = (languageImageResPath as NSString).appendingPathComponent(name).appending("@2x.png")
        let languagePath = (languageImageResPath as NSString).appendingPathComponent(name).appending((String(userToolData)))
        ...
        let path = (norImageResPath as NSString).appendingPathComponent(name).appending((String(userToolData)))
    ...
    func comeMagnitudeo(type: ReportFrameSystemSendable) -> URL {
        //: var path = (svgaResPath as NSString).appendingPathComponent(type.rawValue)
        var path = (svgaResPath as NSString).appendingPathComponent(type.rawValue)
        //: path = (path as NSString).appendingPathExtension("svga")!
        path = (path as NSString).appendingPathExtension((notiPageConst.replacingOccurrences(of: "window", with: "v")))!
    ...
    func livePoint(name: String, type: String = (constPartyData.lowercased())) -> String {
