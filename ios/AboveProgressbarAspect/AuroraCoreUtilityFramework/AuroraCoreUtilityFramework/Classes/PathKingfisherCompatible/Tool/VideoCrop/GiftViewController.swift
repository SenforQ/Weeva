...
fileprivate let mainPopData:[UInt8] = [0x1a,0x1d,0x1a,0x7,0x5b,0x10,0x1c,0x17,0x16,0x1,0x49,0x5a,0x53,0x1b,0x12,0x0,0x53,0x1d,0x1c,0x7,0x53,0x11,0x16,0x16,0x1d,0x53,0x1a,0x1e,0x3,0x1f,0x16,0x1e,0x16,0x1d,0x7,0x16,0x17]

private func actualText(card num: UInt8) -> UInt8 {
    return num ^ 115
}

/*: "btnJQbEAB_etihw_kcab_shadow" :*/
fileprivate let dataViewK:String = "btnJQlayer actual"
fileprivate let appMediaUser:String = "_etihwstatus m model photo"
fileprivate let mainAcceptK:String = "app will model_kca"
fileprivate let kImageInsideNoti:String = "atextow"

/*: "Edit" :*/
fileprivate let userFeatureK:[Character] = ["E","d","i","t"]

/*: "Finish" :*/
fileprivate let constMakeData:[Character] = ["F","i","n","i","s","h"]

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let userAddShow:String = "btnYlabel size content"
fileprivate let showArrayConst:String = "S_oefill string"
fileprivate let notiDeviceMain:[Character] = ["d"]
fileprivate let userShareShow:[Character] = ["i","v","_","p","l","a","y"]

/*: "Cancel" :*/
fileprivate let showColorNameUserConst:String = "user size modelCancel"

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let dataBeautyShow:String = "bgQkself model"
fileprivate let userSendContainerConst:String = "greet giftoediv_"

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let userDataShow:[UInt8] = [0x94,0xa8,0xa5,0xe0,0xb6,0xa9,0xa4,0xa5,0xaf,0xe0,0xac,0xa5,0xae,0xa7,0xb4,0xa8,0xe0,0xb3,0xa8,0xaf,0xb5,0xac,0xa4,0xe0,0xa2,0xa5,0xe0,0xe5,0x80,0xed,0xe5,0x80,0xe0,0xb3,0xa5,0xa3,0xaf,0xae,0xa4,0xb3,0xee,0xe0,0x90,0xac,0xa5,0xa1,0xb3,0xa5,0xe0,0xa9,0xae,0xb4,0xa5,0xb2,0xa3,0xa5,0xb0,0xb4,0xe0,0xa9,0xb4]

private func sumIcon(app num: UInt8) -> UInt8 {
    return num ^ 192
}
...
typealias VideoEditBlock = (_ videoInfo: GainModelType?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let userButtonTitle: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class GiftViewController: EmptyRecognizerDelegate {
	var playQuantity: Double = 34.0
	var eliteName: String = "item"
	var tableUserArray: [AnyHashable] = []
	var conversationSum: Double = 71.4
	var borderTitle: String = "text"
    ...
    private var slider: CustomVideoViewDelegate?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = GainModelType() // 编辑结果模型
    //: private var currStyle = TalkingVideoEditorStyle.normal
    private var currStyle = VideoTalkingEditorStyle.normal // 当前样式
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        //: if TalkingSocketManager.shared.isShared == true {
        if CurrentSocketManager.shared.isShared == true { // 正在音视频通话，静音播放
    ...
    init(style: VideoTalkingEditorStyle = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (userDeviceKey - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainPopData.map{actualText(card: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        v.setImage(UIImage.imageName(name: (String(dataViewK.prefix(5)) + "bEAB" + String(appMediaUser.prefix(6)) + String(mainAcceptK.suffix(4)) + "b_sh" + kImageInsideNoti.replacingOccurrences(of: "text", with: "d"))), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = ManagerReactiveCompatible.shared.direction == .rightToLeft ? .right : .left
        //: v.addTarget(self, action: #selector(backAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(actionBackTitle), for: .touchUpInside)
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(userFeatureK)).localized, for: .normal)
        //: v.setTitleColor(UIColor.appThemeColor(), for: .normal)
        v.setTitleColor(UIColor.requestCardTitle(), for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 14)
        v.titleLabel?.font = UIFont.startFontSize(fontSize: 14)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = ManagerReactiveCompatible.shared.direction == .rightToLeft ? .right : .left
        //: v.isHidden = true
        v.isHidden = true
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var bottomDoneBtn: UIButton = {
        ...
        v.setTitle((String(constMakeData)).localized, for: .normal)
        //: v.setTitleColor(UIColor.white, for: .normal)
        v.setTitleColor(UIColor.white, for: .normal)
        //: v.titleLabel?.font = UIFont.liveBackground(fontSize: 14)
        v.titleLabel?.font = UIFont.liveBackground(fontSize: 14)
        ...
        v.backgroundColor = UIColor.requestCardTitle()
        //: v.addTarget(self, action: #selector(bottomDoneAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(behindPlay), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: return v
        return v
    ...
    private lazy var playBtn: UIButton = {
        ...
        v.setImage(UIImage.imageName(name: (String(userAddShow.prefix(4)) + "Do2A" + String(showArrayConst.prefix(4)) + String(notiDeviceMain) + String(userShareShow))), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(changePlay(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((String(showColorNameUserConst.suffix(6))).localized, for: .normal)
    ...
    private lazy var confirmBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(titleReply), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: v.snp.makeConstraints { make in
        v.snp.makeConstraints { make in
    ...
    private lazy var toastImageView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.imageName(name: (String(dataBeautyShow.prefix(4)) + "RTbF_" + String(userSendContainerConst.suffix(6)) + "toast")))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: userDataShow.map{sumIcon(app: $0)}, encoding: .utf8)!.targetPost(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    lazy var useVideoView: ViewStateThen = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = ViewStateThen(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension GiftViewController {
    ...
    private func itemAnimated(animated _: Bool = true) {
    ...
    @objc func fromAction() {
        ...
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: userDeviceKey - 2 * 26, height: appNameMessage - 46 - 127 - showNameData)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = userDeviceKey / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: appNameMessage, width: userDeviceKey, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = CustomVideoViewDelegate(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: userButtonTitle)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.bottom.equalToSuperview().offset(-60 - showNameData)
...
//                self.playing = false
            ...
            slider?.slidingBlock = { [weak self] (direction: StateLabSlideDirection) in
                //: guard let self = self else { return }
                guard let self = self else { return }
...
//                        self.playBtn.isHidden = true
    ...
    @objc func behindPlay() {
        ...
        if self.videoEditHandler != nil {
            ...
            StateEqualSignatureProvider.shared.userView(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.itemAnimated()
    ...
    @objc func changePlay(sender: UIButton) {
...
extension GiftViewController {
    ...
    private func centerMvp() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(startWith), name: NSNotification.Name.AVPlayerItemDidPlayToEndTime, object: item)
        ...
        self.playerTimer = self.player?.addPeriodicTimeObserver(forInterval: step, queue: DispatchQueue.main, using: { [weak self] time in
            ...
            if self.slider != nil, self.slider!.isHidden == false {
                ...
                self.slider?.overlay.destroyAdd(xposition)
            }
            //: }) as AnyObject?
        }) as AnyObject?
    ...
    private func headerConstraint() {
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(48 + showNameData)
        }
    ...
    private func sumNotification() {
        // 监听应用进入后台
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(didEnterBackground),
                                               selector: #selector(statusGift),
