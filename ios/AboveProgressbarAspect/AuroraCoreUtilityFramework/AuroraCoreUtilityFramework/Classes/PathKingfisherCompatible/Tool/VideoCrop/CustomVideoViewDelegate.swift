...
fileprivate let kViewConst:[UInt8] = [0x41,0x46,0x41,0x4c,0x0,0x3b,0x47,0x3c,0x3d,0x4a,0x12,0x1,0xf8,0x40,0x39,0x4b,0xf8,0x46,0x47,0x4c,0xf8,0x3a,0x3d,0x3d,0x46,0xf8,0x41,0x45,0x48,0x44,0x3d,0x45,0x3d,0x46,0x4c,0x3d,0x3c]

fileprivate func splitIcon(kit num: UInt8) -> UInt8 {
...
typealias SlidingHandler = (_ direction: StateLabSlideDirection) -> Void
...
class CustomVideoViewDelegate: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (mainErrTitle - self.contentView.contentOffset.x)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kViewConst.map{splitIcon(kit: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.everyBackground(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: PlusCancelReactiveCompatible = {
        ...
        let overlay = PlusCancelReactiveCompatible(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension CustomVideoViewDelegate {
    /// 重置位置
    //: func resetPostion() {
    func chapiter() {
    ...
    private func everyBackground(point: CGFloat) -> UIImage {
...
extension CustomVideoViewDelegate: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = mainErrTitle - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.blockFill(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.blockFill(isShow: true)
    }
...
extension CustomVideoViewDelegate {
    ...
    private func indexIn() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(mainErrTitle)
