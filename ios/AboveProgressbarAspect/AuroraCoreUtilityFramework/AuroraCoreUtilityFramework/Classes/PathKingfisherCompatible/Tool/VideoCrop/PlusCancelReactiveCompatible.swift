...
fileprivate let appButtonConst:[UInt8] = [0xe3,0xe4,0xe3,0xfe,0xa2,0xe9,0xe5,0xee,0xef,0xf8,0xb0,0xa3,0xaa,0xe2,0xeb,0xf9,0xaa,0xe4,0xe5,0xfe,0xaa,0xe8,0xef,0xef,0xe4,0xaa,0xe3,0xe7,0xfa,0xe6,0xef,0xe7,0xef,0xe4,0xfe,0xef,0xee]

private func nolPros(birth num: UInt8) -> UInt8 {
    return num ^ 138
}

/*: "FFD855" :*/
fileprivate let constGroupMain:String = "effectD855"
...
// __CLOSE_PRINT__
//
//  PlusCancelReactiveCompatible.swift
//  AbroadTalking
//
...
class PlusCancelReactiveCompatible: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - userTextFormat
                //: self.rightSlider.frame = CGRect(x: minX,
                self.rightSlider.frame = CGRect(x: minX,
                                                //: y: self.rightSlider.frame.origin.y,
                                                y: self.rightSlider.frame.origin.y,
                                                //: width: kSliderWidth,
                                                width: userTextFormat,
                ...
                self.objectExecuteAll(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appButtonConst.map{nolPros(birth: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (constGroupMain.replacingOccurrences(of: "effect", with: "FF")))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: mainErrTitle, y: 0, width: userTextFormat, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - userTextFormat, y: 0, width: userTextFormat, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.eventWith(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: mainErrTitle, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.eventWith(0, 0, 0, 0.6)
...
extension PlusCancelReactiveCompatible {
    ...
    func objectExecuteAll(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - userTextFormat,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - userTextFormat)
    }
}
...
extension PlusCancelReactiveCompatible {
    ...
    @objc func nonsolidColor(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.objectExecuteAll(startX: videoStartX)
    ...
    @objc func blockWill(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension PlusCancelReactiveCompatible {
    ...
    private func setupResourceEqual() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(nonsolidColor(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(blockWill(gesture:)))
    ...
    func destroyAdd(_ xposition: CGFloat) {
