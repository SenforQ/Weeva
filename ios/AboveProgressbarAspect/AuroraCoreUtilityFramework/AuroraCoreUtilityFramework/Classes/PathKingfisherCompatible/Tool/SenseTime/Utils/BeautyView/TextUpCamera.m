...
NSString *StringFromDestroyData(Byte *data);


//: Width
Byte kPathText[] = {59, 5, 10, 14, 182, 83, 224, 98, 206, 201, 240, 140, 249, 99, 97, 115, 110, 126, 114, 68};


//: Height
Byte kHandlePitchText[] = {20, 6, 57, 6, 112, 95, 129, 158, 162, 160, 161, 173, 22};


//: 请打开相机权限
Byte k_sizeText[] = {62, 21, 25, 13, 10, 106, 120, 80, 108, 255, 22, 127, 67, 1, 200, 208, 255, 162, 172, 254, 213, 153, 0, 180, 209, 255, 181, 211, 255, 182, 156, 2, 178, 169, 231};
...
#import "TextUpCamera.h"
//: #import <UIKit/UIKit.h>
#import <UIKit/UIKit.h>

//: @interface STFilterCamera () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate>
@interface TextUpCamera () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate>
...
@implementation TextUpCamera

//: - (instancetype)initWithDevicePosition:(AVCaptureDevicePosition)iDevicePosition
- (instancetype)initWithCry:(AVCaptureDevicePosition)iDevicePosition
                        //: sessionPresset:(AVCaptureSessionPreset)sessionPreset
                        section:(AVCaptureSessionPreset)sessionPreset
                                   //: fps:(int)iFPS
                                   outputFor:(int)iFPS
                         //: needYuvOutput:(BOOL)needYuvOutput
                         startName:(BOOL)needYuvOutput
{
    //: self = [super init];
    self = [super init];
    //: if (self) {
    if (self) {
        ...
        self.videoDevice = [self root:iDevicePosition];
...
- (void)click:(CGPoint)point shareDate:(CGRect)frame
{
    ...
    [self name:AVCaptureFocusModeAutoFocus
           //: exposureMode:AVCaptureExposureModeContinuousAutoExposure
           with:AVCaptureExposureModeContinuousAutoExposure
                //: atPoint:CGPointMake(fX, fY)];
                sessionPoint:CGPointMake(fX, fY)];

}

//: - (void)focusWithMode:(AVCaptureFocusMode)focusMode exposureMode:(AVCaptureExposureMode)exposureMode atPoint:(CGPoint)point
- (void)name:(AVCaptureFocusMode)focusMode with:(AVCaptureExposureMode)exposureMode sessionPoint:(CGPoint)point
{

    //: [self changeDeviceProperty:^(AVCaptureDevice *captureDevice) {
    [self deviceStyle:^(AVCaptureDevice *captureDevice) {
...
- (void)deviceStyle:(void(^)(AVCaptureDevice *))propertyChange
{
...
- (void)setCellScaleValue:(float)value
{
...
- (void)setDevicePosition:(AVCaptureDevicePosition)devicePosition
{
    ...
    if (_devicePosition != devicePosition && devicePosition != AVCaptureDevicePositionUnspecified) {
        ...
        if (_session) {
            ...
            AVCaptureDevice *targetDevice = [self root:devicePosition];

            //: if (targetDevice && [self judgeCameraAuthorization]) {
            if (targetDevice && [self message]) {
...
- (void)setIExpectedFPS:(int)iExpectedFPS
{
    ...
    CGFloat fWidth = [[_dataOutput.videoSettings objectForKey:StringFromDestroyData(kPathText)] floatValue];
    //: CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:@"Height"] floatValue];
    CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:StringFromDestroyData(kHandlePitchText)] floatValue];
...
- (CGRect)row:(CGRect)rect imageGift:(BOOL)bScaleToFit
{
    //: CGRect rectRet = rect;
    CGRect rectRet = rect;
    ...
    if (self.dataOutput.videoSettings) {
        ...
        CGFloat fWidth = [[self.dataOutput.videoSettings objectForKey:StringFromDestroyData(kPathText)] floatValue];
        //: CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:@"Height"] floatValue];
        CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:StringFromDestroyData(kHandlePitchText)] floatValue];
...
- (BOOL)message
{
    ...
    if (authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            UIAlertController *alertView = [UIAlertController alertControllerWithTitle:@""
                                                                               //: message:@"请打开相机权限"
                                                                               message:StringFromDestroyData(k_sizeText)
...
- (AVCaptureDevice *)root:(AVCaptureDevicePosition)position
{
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (!self.bSessionPause) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(info:sunnaConnection:underContextFrom:)]) {
            //: [self.delegate captureOutput:captureOutput didOutputSampleBuffer:sampleBuffer fromConnection:connection];
            [self.delegate info:captureOutput sunnaConnection:sampleBuffer underContextFrom:connection];
        }
    }
...
Byte * DestroyDataToCache(Byte *data) {
    int raw = data[0];
    int countStyle = data[1];
    Byte blockRawPause = data[2];
    int exaggerate = data[3];
    if (!raw) return data + exaggerate;
    for (int i = exaggerate; i < exaggerate + countStyle; i++) {
        int value = data[i] - blockRawPause;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[exaggerate + countStyle] = 0;
    return data + exaggerate;
}

NSString *StringFromDestroyData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)DestroyDataToCache(data)];
}
