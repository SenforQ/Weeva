...
NSString *StringFromScreenData(ScreenData *data);


//: model
ScreenData k_constraintSessionTitle = (ScreenData){59, (Byte []){86, 84, 95, 94, 87, 197}, 5, false, 177, 137, 99};


//: key.Notification.Tap.To.Modify.Exposure.Point
ScreenData kIndexName = (ScreenData){205, (Byte []){166, 168, 180, 227, 131, 162, 185, 164, 171, 164, 174, 172, 185, 164, 162, 163, 227, 153, 172, 189, 227, 153, 162, 227, 128, 162, 169, 164, 171, 180, 227, 136, 181, 189, 162, 190, 184, 191, 168, 227, 157, 162, 164, 163, 185, 211}, 45, false, 198, 142, 152};


//: camera_focus_red
ScreenData k_videoName = (ScreenData){147, (Byte []){240, 242, 254, 246, 225, 242, 204, 245, 252, 240, 230, 224, 204, 225, 246, 247, 21}, 16, false, 4, 92, 18};


//: hasFace
ScreenData kSaveTitle = (ScreenData){166, (Byte []){206, 199, 213, 224, 199, 197, 195, 34}, 7, false, 198, 121, 214};


//: FaceActionEvent_Interval_12_frame
ScreenData kGroupContent = (ScreenData){190, (Byte []){248, 223, 221, 219, 255, 221, 202, 215, 209, 208, 251, 200, 219, 208, 202, 225, 247, 208, 202, 219, 204, 200, 223, 210, 225, 143, 140, 225, 216, 204, 223, 211, 219, 215}, 33, false, 125, 160, 81};


//: FaceActionEvent_Interval_1_second
ScreenData k_cornerPerformContent = (ScreenData){238, (Byte []){168, 143, 141, 139, 175, 141, 154, 135, 129, 128, 171, 152, 139, 128, 154, 177, 167, 128, 154, 139, 156, 152, 143, 130, 177, 223, 177, 157, 139, 141, 129, 128, 138, 175}, 33, false, 153, 116, 194};


//: M_SenseME_Face_Video_5.3.4
ScreenData k_rangeName = (ScreenData){39, (Byte []){106, 120, 116, 66, 73, 84, 66, 106, 98, 120, 97, 70, 68, 66, 120, 113, 78, 67, 66, 72, 120, 18, 9, 20, 9, 19, 242}, 26, false, 195, 50, 31};
...
//
//#import "SingleDownPhotoCostReactiveCompatible.h"
//#import "FiltrateLooksModel.h"
//ST_MOBILE
...
@interface ModeView ()<ComponentAd,UIGestureRecognizerDelegate>
{
...
//: @property (nonatomic, strong) TimeThen * lichelper;
@property (nonatomic, strong) TimeThen * lichelper;
//: @property (nonatomic, strong) ModelReactiveCompatible * ParamUtil;
@property (nonatomic, strong) ModelReactiveCompatible * ParamUtil;
...
- (instancetype)init {
    ...
    if (self) {
        ...
        //: self.lichelper = [TimeThen share];
        self.lichelper = [TimeThen share];
        //: self.ParamUtil = [[ModelReactiveCompatible alloc]init];
        self.ParamUtil = [[ModelReactiveCompatible alloc]init];
        //: _currentDropCount = 5;
        ...
        if ([self.lichelper selectName]) {
            ...
            //: [[SingleDownPhotoCostReactiveCompatible share] getLocalModel];
            [[SingleDownPhotoCostReactiveCompatible share] todayOn];
            ...
            [NSNotificationCenter.defaultCenter addObserver:self
                                                   ...
                                                   selector:@selector(need:)
                                                       //: name:@"key.Notification.Tap.To.Modify.Exposure.Point"
                                                       name:StringFromScreenData(&kIndexName)
                                                     //: object:nil];
                                                     object:nil];
            ...
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getThroughInstantiate:) name:self.ParamUtil.kNotificationSelectBeautyFilterStr object:nil];
            ...
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(named:) name:self.ParamUtil.kNotificationBeautyFilterValueChangeStr object:nil];
...
- (instancetype)initTitle:(NSString*)sessionPreset {
    ...
    if (self = [super init]) {
        ...
        if ([self.lichelper selectName]) {
            ...
            [self initIntervalimate:sessionPreset];

        //: } else {
        } else {
            //: [TimeThen.share checkRemoteLicInfoWithCompletion:^(BOOL x) {
            [TimeThen.share visibleWithCompletion:^(BOOL x) {
                //: if (x) {
                if (x) {
                    //: [self initFilterView:sessionPreset];
                    [self initIntervalimate:sessionPreset];
...
- (void)initIntervalimate:(NSString*)sessionPreset {
    //: self.layer.masksToBounds = YES;
    self.layer.masksToBounds = YES;
    //: self.clipsToBounds = YES;
    self.clipsToBounds = YES;
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getThroughInstantiate:)
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(named:)
...
- (void)send {
    ...
    //: if ([SingleDownPhotoCostReactiveCompatible share].filterSuite.filterModelPath.length > 0) {
    if ([SingleDownPhotoCostReactiveCompatible share].filterSuite.filterModelPath.length > 0) {
        //: FiltrateLooksModel* model = [[SingleDownPhotoCostReactiveCompatible share] getFilterDataModel];
        FiltrateLooksModel* model = [[SingleDownPhotoCostReactiveCompatible share] deadlineModel];
        //: if (model) {
        if (model) {
...
- (void)setCurrentSessionPreset:(NSString *)currentSessionPreset {
    ...
    [self.stCamera click:self.previewCenter shareDate:self.previewFrame];
}

//: - (void)setupPreviewFullScreenSize:(BOOL)isFullScreen {
- (void)effect:(BOOL)isFullScreen {
...
- (void)object {
    ...
    _stCamera = [[TextUpCamera alloc] initWithCry:AVCaptureDevicePositionFront
                                          //: sessionPresset:self.currentSessionPreset
                                          section:self.currentSessionPreset
                                                     //: fps:fps
                                                     outputFor:fps
                                           //: needYuvOutput:NO];
                                           startName:NO];
...
- (void)orientation:(CGFloat)width low:(CGFloat)height {
    //: CGRect previewRect = [self.stCamera getScaleRect:CGRectMake(0 , 0 , width, height)
    CGRect previewRect = [self.stCamera row:CGRectMake(0 , 0 , width, height)
                                                   //: scaleToFit:NO];
                                                   imageGift:NO];
    ...
    if (self.glPreview.superview == nil) {
        ...
        @synchronized (self) {
           ...
           _glPreview = [[LessView alloc] initWithIsPhoto:previewRect path:previewContext];
        }
...
- (void)lastFrame {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(indicatorActive) name:UIApplicationWillResignActiveNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appDidEnterBackground) name:UIApplicationDidEnterBackgroundNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(objectOutput) name:UIApplicationDidEnterBackgroundNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appWillEnterForeground) name:UIApplicationWillEnterForegroundNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(enterValue) name:UIApplicationWillEnterForegroundNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appDidBecomeActive) name:UIApplicationDidBecomeActiveNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(modeBy) name:UIApplicationDidBecomeActiveNotification object:nil];
}
...
- (void)welt:(CMAccelerometerData *)accelerometerData {
...
- (st_rotate_type)turnProperty {
    ...
    [self welt:self.motionManager.accelerometerData];
...
- (void)search {
    ...
    //初始化检测模块句柄
    //: NSString *strModelPath = [RowMacroDefine.getModelFileName stringByAppendingPathComponent:@"M_SenseME_Face_Video_5.3.4"];
    NSString *strModelPath = [RowMacroDefine.retrieveHideLastTitle stringByAppendingPathComponent:StringFromScreenData(&k_rangeName)];
    //: strModelPath = [strModelPath stringByAppendingPathExtension:@"model"];
    strModelPath = [strModelPath stringByAppendingPathExtension:StringFromScreenData(&k_constraintSessionTitle)];
...
- (BOOL)size:(CVPixelBufferRef)pixelBuffer {
...
- (void)need:(NSNotification *)sender {
...
- (void)to:(UITapGestureRecognizer *)tapGesture {
    ...
    [self.stCamera click:point shareDate:self.frame];
...
- (UIImageView *)focusImageView {
    ...
    if (!_focusImageView) {
        ...
        _focusImageView.image = [UIImage imageNamed:StringFromScreenData(&k_videoName)];
...
- (void)getThroughInstantiate:(NSNotification*) notice {
    //: if ([notice.object isKindOfClass:[FiltrateLooksModel class]]) {
    if ([notice.object isKindOfClass:[FiltrateLooksModel class]]) {
        //: FiltrateLooksModel* model = (FiltrateLooksModel*)notice.object;
        FiltrateLooksModel* model = (FiltrateLooksModel*)notice.object;
        //: [self handleFilterChanged:model];
        [self key:model];
    }
...
//: - (void)handleFilterChanged:(FiltrateLooksModel *)model {
- (void)key:(FiltrateLooksModel *)model {
...
- (void)named:(NSNotification*) notice {
    //: FiltrateLooksModel* model = (FiltrateLooksModel*)notice.object;
    FiltrateLooksModel* model = (FiltrateLooksModel*)notice.object;
    //: if (_hFilter) {
    if (_hFilter) {
...
- (void)compartment {
    ...
    //: PossibilityModel *beautyValueCache = SingleDownPhotoCostReactiveCompatible.share.currentBeautySuite;
    PossibilityModel *beautyValueCache = SingleDownPhotoCostReactiveCompatible.share.untilSuite;

    // 设置美白参数
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_WHITEN_STRENGTH value:beautyValueCache.fWhitenStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_WHITEN_STRENGTH value:beautyValueCache.fWhitenStrength];
    // 设置默认红润参数
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_REDDEN_STRENGTH value:beautyValueCache.fReddenStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_REDDEN_STRENGTH value:beautyValueCache.fReddenStrength];
    // 设置默认磨皮参数
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SMOOTH_STRENGTH value:beautyValueCache.fSmoothStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SMOOTH_STRENGTH value:beautyValueCache.fSmoothStrength];
    // 设置默认大眼参数
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ENLARGE_EYE_RATIO value:beautyValueCache.fEnlargeEyeStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ENLARGE_EYE_RATIO value:beautyValueCache.fEnlargeEyeStrength];
    // 设置默认瘦脸参数
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_FACE_RATIO value:beautyValueCache.fShrinkFaceStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_FACE_RATIO value:beautyValueCache.fShrinkFaceStrength];
    // 设置小脸参数
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_JAW_RATIO value:beautyValueCache.fShrinkJawStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_JAW_RATIO value:beautyValueCache.fShrinkJawStrength];

    //瘦脸型
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_THIN_FACE_SHAPE_RATIO value:beautyValueCache.fThinFaceShapeStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_THIN_FACE_SHAPE_RATIO value:beautyValueCache.fThinFaceShapeStrength];
    //窄脸
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_NARROW_FACE_STRENGTH value:beautyValueCache.fNarrowFaceStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_NARROW_FACE_STRENGTH value:beautyValueCache.fNarrowFaceStrength];
    //圆眼
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ROUND_EYE_RATIO value:beautyValueCache.fRoundEyeStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ROUND_EYE_RATIO value:beautyValueCache.fRoundEyeStrength];
    //下巴
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_CHIN_LENGTH_RATIO value:beautyValueCache.fChinStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_CHIN_LENGTH_RATIO value:beautyValueCache.fChinStrength];
    //额头
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_HAIRLINE_HEIGHT_RATIO value:beautyValueCache.fHairLineStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_HAIRLINE_HEIGHT_RATIO value:beautyValueCache.fHairLineStrength];
    //瘦鼻翼
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NARROW_NOSE_RATIO value:beautyValueCache.fNarrowNoseStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NARROW_NOSE_RATIO value:beautyValueCache.fNarrowNoseStrength];
    //长鼻
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NOSE_LENGTH_RATIO value:beautyValueCache.fLongNoseStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NOSE_LENGTH_RATIO value:beautyValueCache.fLongNoseStrength];
    //嘴形
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_MOUTH_SIZE_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fMouthStrength]];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_MOUTH_SIZE_RATIO value:[self asView:beautyValueCache.fMouthStrength]];
    //缩人中
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PHILTRUM_LENGTH_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fPhiltrumStrength]];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PHILTRUM_LENGTH_RATIO value:[self asView:beautyValueCache.fPhiltrumStrength]];

    //设置对比度参数
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_CONTRAST_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fContrastStrength]];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_CONTRAST_STRENGTH value:[self asView:beautyValueCache.fContrastStrength]];

    //设置饱和度参数
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SATURATION_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fSaturationStrength]];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SATURATION_STRENGTH value:[self asView:beautyValueCache.fSaturationStrength]];

    //锐化参数
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHARPEN_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fSharpenStrength]];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHARPEN_STRENGTH value:[self asView:beautyValueCache.fSharpenStrength]];

    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_APPLE_MUSLE_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fAppleMusleStrength]];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_APPLE_MUSLE_RATIO value:[self asView:beautyValueCache.fAppleMusleStrength]];

    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PROFILE_RHINOPLASTY_RATIO value:beautyValueCache.fProfileRhinoplastyStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PROFILE_RHINOPLASTY_RATIO value:beautyValueCache.fProfileRhinoplastyStrength];

    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_DISTANCE_RATIO value:beautyValueCache.fEyeDistanceStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_DISTANCE_RATIO value:beautyValueCache.fEyeDistanceStrength];

    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_ANGLE_RATIO value:beautyValueCache.fEyeAngleStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_ANGLE_RATIO value:beautyValueCache.fEyeAngleStrength];

    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_OPEN_CANTHUS_RATIO value:beautyValueCache.fOpenCanthusStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_OPEN_CANTHUS_RATIO value:beautyValueCache.fOpenCanthusStrength];

    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_BRIGHT_EYE_RATIO value:beautyValueCache.fBrightEyeStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_BRIGHT_EYE_RATIO value:beautyValueCache.fBrightEyeStrength];
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_DARK_CIRCLES_RATIO value:beautyValueCache.fRemoveDarkCirclesStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_DARK_CIRCLES_RATIO value:beautyValueCache.fRemoveDarkCirclesStrength];
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_NASOLABIAL_FOLDS_RATIO value:beautyValueCache.fRemoveNasolabialFoldsStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_NASOLABIAL_FOLDS_RATIO value:beautyValueCache.fRemoveNasolabialFoldsStrength];
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_WHITE_TEETH_RATIO value:beautyValueCache.fWhiteTeethStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_WHITE_TEETH_RATIO value:beautyValueCache.fWhiteTeethStrength];
    //: [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_CHEEKBONE_RATIO value:beautyValueCache.fShrinkCheekboneStrength];
    [self.ParamUtil addTextWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_CHEEKBONE_RATIO value:beautyValueCache.fShrinkCheekboneStrength];

}

//: - (float)rangeMappedValueFrom:(float) rawValue {
- (float)asView:(float) rawValue {
...
void dataRelease(st_mobile_animal_face_t *src, int faceCount, st_mobile_animal_face_t *dst) {
...
void makeKey(st_mobile_animal_face_t *src, int faceCount) {
    //: if (faceCount > 0) {
    if (faceCount > 0) {
        //: for (int i = 0; i < faceCount; ++i) {
        for (int i = 0; i < faceCount; ++i) {
...
- (void)file:(CMSampleBufferRef)originalSampleBuffer
                       //: resultPixelBuffer:(CVPixelBufferRef)resultPixelBufffer {
                       userDown:(CVPixelBufferRef)resultPixelBufffer {
    ...
    if ( originalSampleBuffer && resultPixelBufffer ) {
        ...
        if (tmpDelegate && [tmpDelegate respondsToSelector:@selector(fillContent:info:)]) {
            ...
            if (sampleBuffer) {
                ...
                [tmpDelegate fillContent:self info:sampleBuffer];
            }
...
- (CVPixelBufferRef)applier:(CVPixelBufferRef)pixelBuffer {
...
- (void)info:(AVCaptureOutput *)captureOutput sunnaConnection:(CMSampleBufferRef)sampleBuffer underContextFrom:(AVCaptureConnection *)connection {
    ...
    st_rotate_type stMobileRotate = [self turnProperty];
    ...
    if (_hDetector) {
        ...
        if (self.iCurrentAction > 0) {
            ...
            if (_faceFrameCount == 12) { // 第12帧时
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:StringFromScreenData(&kGroupContent) object:nil userInfo: @{StringFromScreenData(&kSaveTitle): @(hasFace)}];
            }
            ...
            if (_faceFrameCount%18 == 0) { // 每秒发送一次
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:StringFromScreenData(&k_cornerPerformContent) object:nil userInfo: @{StringFromScreenData(&kSaveTitle): @(hasFace)}];
            }
        }
    }
    ...
    if (faceCount > 0) {
        ...
        dataRelease(_detectResult1, faceCount, newDetectResult1);
    }
    ...
    dispatch_async(self.renderQueue, ^{
       ...
       BOOL isTextureOriginReady = [self size:pixelBuffer];
       ...
       // ST_MOBILE 以下为滤镜部分
        ...
        if (callBackFlag) [self file:sampleBuffer userDown:resultPixelBufffer];
        ...
        if (faceCount > 0) {
            ...
            makeKey(newDetectResult1, faceCount);
        }
        //: if (callBackFlag) {
        if (callBackFlag) {
            //: @synchronized (self) {
            @synchronized (self) {
                ...
                [self.glPreview viewTitleure:textureResult];
            }
        }
...
Byte *ScreenDataToByte(ScreenData *data) {
    if (data->property) return data->deep;
    for (int i = 0; i < data->infoConversation; i++) {
        data->deep[i] ^= data->style;
    }
    data->deep[data->infoConversation] = 0;
    data->property = true;
	if (data->infoConversation >= 3) {
		data->capacityMedia = data->deep[0];
		data->linkResult = data->deep[1];
		data->session = data->deep[2];
	}
    return data->deep;
}

NSString *StringFromScreenData(ScreenData *data) {
    return [NSString stringWithUTF8String:(char *)ScreenDataToByte(data)];
}
