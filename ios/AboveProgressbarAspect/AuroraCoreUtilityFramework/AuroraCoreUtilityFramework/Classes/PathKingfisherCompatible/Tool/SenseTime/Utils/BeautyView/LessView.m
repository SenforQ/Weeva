...
NSString *StringFromRelieveData(RelieveData *data);


//: STDisplayShader
RelieveData kStyleViewText = (RelieveData){182, (Byte []){229, 226, 242, 223, 197, 198, 218, 215, 207, 229, 222, 215, 210, 211, 196, 53}, 15, false, 107};
...
- (instancetype)initWithIsPhoto:(CGRect)frame path:(EAGLContext *)context
{
    ...
    if (self) {
        ...
        [self contentPrice:StringFromRelieveData(&kStyleViewText)
                //: fragmentShader:@"STDisplayShader"
                doing:StringFromRelieveData(&kStyleViewText)
                    //: forProgram:&stDisplayProgram];
                    brickbatImage:&stDisplayProgram];
...
- (BOOL)contentPrice:(NSString *)vertexShaderName
          //: fragmentShader:(NSString *)fragmentShaderName
          doing:(NSString *)fragmentShaderName
              //: forProgram:(GLuint *)programPointer;
              brickbatImage:(GLuint *)programPointer;
{
    ...
    *programPointer = glCreateProgram();
    ...
    if (![self stop:&vertexShader originCompile:0x8B31 indexText:userSpaceId]) {
    ...
    if (![self stop:&fragShader originCompile:0x8B30 indexText:userTagPath]) {
    ...
    if (![self relateProgram:*programPointer]) {
...
- (BOOL)stop:(GLuint *)shader originCompile:(GLenum)type indexText:(const char *)str
{
    //: GLint status;
    GLint status;
    //: const GLchar *source;
...
- (BOOL)relateProgram:(GLuint)prog
{
    //: GLint status;
    GLint status;
...
- (void)viewTitleure:(GLuint)texture
{
...
Byte *RelieveDataToByte(RelieveData *data) {
    if (data->backwash) return data->index;
    for (int i = 0; i < data->vent; i++) {
        data->index[i] ^= data->destroy;
    }
    data->index[data->vent] = 0;
    data->backwash = true;
	if (data->vent >= 1) {
		data->depth = data->index[0];
	}
    return data->index;
}

NSString *StringFromRelieveData(RelieveData *data) {
    return [NSString stringWithUTF8String:(char *)RelieveDataToByte(data)];
}
