...
/*: "type" :*/
fileprivate let userNaturalK:String = "tabype"

/*: "You are currently on a call, please try again later" :*/
fileprivate let notiMapApp:[UInt8] = [0x69,0x5f,0x45,0x10,0x51,0x42,0x55,0x10,0x53,0x45,0x42,0x42,0x55,0x5e,0x44,0x5c,0x49,0x10,0x5f,0x5e,0x10,0x51,0x10,0x53,0x51,0x5c,0x5c,0x1c,0x10,0x40,0x5c,0x55,0x51,0x43,0x55,0x10,0x44,0x42,0x49,0x10,0x51,0x57,0x51,0x59,0x5e,0x10,0x5c,0x51,0x44,0x55,0x42]

private func itemImage(aspect num: UInt8) -> UInt8 {
    return num ^ 48
}

/*: "onCallResume" :*/
fileprivate let dataNameNoti:String = "onCallimage model"
fileprivate let dataEmptyK:[Character] = ["R","e","s","u","m","e"]
...
// __CLOSE_PRINT__
//
//  PositionLabelReactiveCompatible+VideoChat.swift
//  AbroadTalking
//
...
extension PositionLabelReactiveCompatible {
    ...
    func voiceSend(info: [String: Any], situation: ShapeChatSituation = .Start) {
        //: let json = JSON(info)
        let json = JSON(info)
        //: let type = json["type"].intValue
        let type = json[(userNaturalK.replacingOccurrences(of: "tab", with: "t"))].intValue
...
extension PositionLabelReactiveCompatible {
    ...
    private func leaveOf(info: [String: Any], resumeTalk: Bool) {
        //: if !resumeTalk && TalkingSocketManager.shared.isShared {
        if !resumeTalk && CurrentSocketManager.shared.isShared {
            //: self.func__showStatusBarErrorMsg(showMsg: "You are currently on a call, please try again later".localized)
            self.exceptAdd(showMsg: String(bytes: notiMapApp.map{itemImage(aspect: $0)}, encoding: .utf8)!.localized)
            //: return
            return
        }
        ...
        if underTitle()!.isKind(of: HeadingErrorDelegate.self) ||
            //: currentViewController()!.isKind(of: TalkingVoiceChatViewController.self) ||
            underTitle()!.isKind(of: ChatObjectProtocol.self) ||
            //: currentViewController()!.isKind(of: TalkingVideoInitivCallVC.self) {
            underTitle()!.isKind(of: LiveViewDelegate.self)
        {
            //: return
            return
        }
        ...
        if type == 0 { // 视频通话
            ...
            (vc as! HeadingErrorDelegate).chatModel = EnableChatModel.absurd(info: info, situation: resumeTalk ? .Resume : .Recieved)
            //: } else {
        } else { // 语音通话
            //: vc = TalkingVoiceChatViewController()
            vc = ChatObjectProtocol()
            ...
            (vc as! ChatObjectProtocol).chatModel = EnableChatModel.absurd(info: info, situation: resumeTalk ? .Resume : .Recieved)
        }

        //: let callResume = json["onCallResume"].stringValue
        let callResume = json[(String(dataNameNoti.prefix(6)) + String(dataEmptyK))].stringValue
        ...
        if resumeTalk, callResume.count > 0, callResume == "1" { // 恢复通话
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 1) {
                ...
                self.takeAChanceOnAggregation(roomVC: vc!, isVideoTalk: type == 0 ? true : false, resumeTalk: resumeTalk, resumeData: info)
            }
            //: } else {
        } else { // 构建音视频通话
            //: self.tryToPushTalkRoom(roomVC: vc!, isVideoTalk: (type == 0 ? true : false), resumeTalk: resumeTalk, resumeData: info)
    ...
    private func takeAChanceOnAggregation(roomVC: UIViewController, isVideoTalk: Bool, resumeTalk: Bool, resumeData: [String: Any]) {
        //: guard var currentVc = currentViewController() else { return }
        guard var currentVc = underTitle() else { return }
        ...
        if currentVc.isKind(of: UINavigationController.self) {
            ...
            //: let rootVc = RowMacroDefine.getWindow().rootViewController as! UITabBarController
            let rootVc = RowMacroDefine.apply().rootViewController as! UITabBarController
        ...
        if !isVideoTalk {
            ...
            talkClass = NSStringFromClass(ChatObjectProtocol.self)
            //: } else {
        } else {
            //: talkClass = NSStringFromClass(TalkingVideoChatViewController.self)
            talkClass = NSStringFromClass(HeadingErrorDelegate.self)
        }
        //: var chatIndex = -1
        var chatIndex = -1
        //: var index = -1
        ...
        if isVideoTalk {
            ...
            var currentChatVc: HeadingErrorDelegate?
            //: let pushChatVc = roomVC as! TalkingVideoChatViewController
            let pushChatVc = roomVC as! HeadingErrorDelegate

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVideoChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? HeadingErrorDelegate
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    ...
                    currentChatVc?.examine(resumeData: resumeData)
                }
                //: return
                return
            }
            ...
            if navVc!.isKind(of: TimeNavigationController.self) {
                //: let nav = navVc as! TalkingNavigationController
                let nav = navVc as! TimeNavigationController
            ...
            if topVc != nil, topVc!.isKind(of: PrixFixeViewController.self) {
            ...
            var currentChatVc: ChatObjectProtocol?
            //: let pushChatVc = roomVC as! TalkingVoiceChatViewController
            let pushChatVc = roomVC as! ChatObjectProtocol

            //: if chatIndex >= 0 {
            if chatIndex >= 0 {
                //: currentChatVc = navVc?.viewControllers[chatIndex] as? TalkingVoiceChatViewController
                currentChatVc = navVc?.viewControllers[chatIndex] as? ChatObjectProtocol
            }
            ...
            if currentChatVc != nil, currentChatVc?.chatModel?.pairUid == pushChatVc.chatModel?.pairUid {
                ...
                if resumeTalk {
                    //: currentChatVc?.updateInfoViaResumeTalkData(resumeData: resumeData)
                    currentChatVc?.eigenvalueOfAMatrix(resumeData: resumeData)
                }
                //: return
                return
            }
