...
class MoreBotDataBase: NSObject {
    static let sharedInstance = MoreBotDataBase()
    private lazy var rootPath: URL = {
        ...
        let path = cachesDirectory.appendingPathComponent("/data.db")
    ...
    lazy var data: [String: [String: MoreBotDataModelBase]] = {
        let decoder = JSONDecoder()
        if let json = FileUtil.readFile(filePath: rootPath), let value = try? decoder.decode([String: [String: String]].self, from: json) {
            var result: [String: [String: MoreBotDataModelBase]] = [:]
            for (tableName, table) in value {
                result[tableName] = [:]
                for (primaryKey, model) in table {
                    if let cls = MoreBotTool.Phonationclass(from: primaryKey) as? MoreBotDataModelBase.Type,
...
class MoreBotDataModelBase: Codable, MoreBotDataProtocol {
