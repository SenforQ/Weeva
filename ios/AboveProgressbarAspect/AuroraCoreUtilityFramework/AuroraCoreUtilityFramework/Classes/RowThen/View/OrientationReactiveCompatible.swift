...
fileprivate let constTotalUser:[UInt8] = [0x21,0x26,0x21,0x3c,0x60,0x2b,0x27,0x2c,0x2d,0x3a,0x72,0x61,0x68,0x20,0x29,0x3b,0x68,0x26,0x27,0x3c,0x68,0x2a,0x2d,0x2d,0x26,0x68,0x21,0x25,0x38,0x24,0x2d,0x25,0x2d,0x26,0x3c,0x2d,0x2c]

private func blockDismiss(extra num: UInt8) -> UInt8 {
    return num ^ 72
}

/*: "Set New Password" :*/
fileprivate let showStreamConst:String = "report table model to interactionSet Ne"
fileprivate let appStyleKeyMain:String = "error withw Pas"

/*: "8075F5" :*/
fileprivate let showQueryedApp:String = "8075Fbag object"
fileprivate let showLayUser:String = "color"

/*: "F4F4F4" :*/
fileprivate let mainNameLogRawShow:String = "f4f4f"
fileprivate let userDataViewMain:String = "4"

/*: "Password" :*/
fileprivate let mainTableThenShow:String = "Passwordread equal data"

/*: "Please set a password of at least 6 digits, consisting of numbers, English case groups" :*/
fileprivate let userModeWithNoti:[UInt8] = [0xba,0x86,0x8f,0x8b,0x99,0x8f,0xca,0x99,0x8f,0x9e,0xca,0x8b,0xca,0x9a,0x8b,0x99,0x99,0x9d,0x85,0x98,0x8e,0xca,0x85,0x8c,0xca,0x8b,0x9e,0xca,0x86,0x8f,0x8b,0x99,0x9e,0xca,0xdc,0xca,0x8e,0x83,0x8d,0x83,0x9e,0x99,0xc6,0xca,0x89,0x85,0x84,0x99,0x83,0x99,0x9e,0x83,0x84,0x8d,0xca,0x85,0x8c,0xca,0x84,0x9f,0x87,0x88,0x8f,0x98,0x99,0xc6,0xca,0xaf,0x84,0x8d,0x86,0x83,0x99,0x82,0xca,0x89,0x8b,0x99,0x8f,0xca,0x8d,0x98,0x85,0x9f,0x9a,0x99]

private func objectScale(equal num: UInt8) -> UInt8 {
    return num ^ 234
}

/*: "Next" :*/
fileprivate let userModelData:[Character] = ["N","e","x","t"]

/*: "Skip" :*/
fileprivate let constLengthMain:String = "let instance ifSkip"

/*: "Resent Code" :*/
fileprivate let constSucceedViewInfoK:String = "Resentprogress type"

/*: "8C7AFF" :*/
fileprivate let dataFileConst:String = "8C7AFFreturn image"

/*: " ( :*/
fileprivate let showDataUser:[Character] = [" ","("]

/*: s)" :*/
fileprivate let notiTotalK:String = "temp receive filter normal lefts)"

/*: "Your Password: %@" :*/
fileprivate let dataAddImageMain:String = "Your Pain let show"
fileprivate let constLoadValueApp:[Character] = ["s","s","w","o","r","d",":"," ","%","@"]
...
class OrientationReactiveCompatible: UIView {
    ...
    private var type = PlusMirrorPath.PhoneLogin // 登录类型（手机号/邮箱）
    ...
    init(frame: CGRect, isNewPwd: Bool, loginType: PlusMirrorPath) {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constTotalUser.map{blockDismiss(extra: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var titleLab: UILabel = {
        ...
        lab.font = .viewPath(type: .Semibold, fontSize: 17)
        //: lab.textColor = .appTitleColor()
        lab.textColor = .read()
        //: lab.text = "Set New Password".localized
        lab.text = (String(showStreamConst.suffix(6)) + String(appStyleKeyMain.suffix(5)) + "sword").localized
        //: return lab
        return lab
        //: }()
    }()
    ...
    lazy var phoneLab: UILabel = {
        ...
        lab.font = .viewPath(type: .Medium, fontSize: 17)
        //: lab.textColor = .appTitleColor()
        lab.textColor = .read()
    ...
    private lazy var codeInputView: SpeedCacheThen = {
        //: let codeView = TalkingCodeInputView.init(frame: CGRect(x: 0, y: 0, width: ScreenWidth, height: 70), codeNumber: 6, labelTextColor: .white, mainColor: UIColor(hex: "8075F5")!, normalColor: UIColor(hex: "F4F4F4")!, margin: 12)
        let codeView = SpeedCacheThen(frame: CGRect(x: 0, y: 0, width: userDeviceKey, height: 70), codeNumber: 6, labelTextColor: .white, mainColor: UIColor(hex: (String(showQueryedApp.prefix(5)) + showLayUser.replacingOccurrences(of: "color", with: "5")))!, normalColor: UIColor(hex: (mainNameLogRawShow.uppercased() + userDataViewMain.capitalized))!, margin: 12)
    ...
    private lazy var pwdTitleLab: UILabel = {
        ...
        lab.text = (String(mainTableThenShow.prefix(8))).localized
        //: lab.textColor = .appTitleColor()
        lab.textColor = .read()
    ...
    private lazy var pwdInputView: UITextField = {
        ...
        textField.backgroundColor = UIColor(hex: (mainNameLogRawShow.uppercased() + userDataViewMain.capitalized))
    ...
    private lazy var desLab: UILabel = {
        ...
        lab.font = UIFont.viewPath(type: .Regular, fontSize: 12)
        ...
        lab.text = String(bytes: userModeWithNoti.map{objectScale(equal: $0)}, encoding: .utf8)!.localized
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var nextBtn: UIButton = {
        ...
        btn.setTitle((String(userModelData)).localized, for: .normal)
    ...
    private lazy var skipBtn: UIButton = {
        ...
        let attributes = [.foregroundColor: UIColor.requestCardTitle(), .font: UIFont.viewPath(type: .Medium, fontSize: 17), .underlineStyle: 1] as [NSAttributedString.Key: Any]
        //: let attributeString = NSAttributedString(string: "Skip".localized, attributes: attributes)
        let attributeString = NSAttributedString(string: (String(constLengthMain.suffix(4))).localized, attributes: attributes)
    ...
    private lazy var reSendBtn: UIButton = {
        ...
        btn.setTitle((String(constSucceedViewInfoK.prefix(6)) + " Code").localized, for: .normal)
...
extension OrientationReactiveCompatible {
    ...
    func timer() {
        ...
        codeInputView.equalBlock()
        //: codeInputView.startEdit()
        codeInputView.toEdit()
    ...
    private func at(countdown: Int) {
        //: reSendBtn.isEnabled = countdown <= 0
        reSendBtn.isEnabled = countdown <= 0
        //: let btnStr = "Resent Code".localized
        let btnStr = (String(constSucceedViewInfoK.prefix(6)) + " Code").localized
        ...
        var attributes = [.foregroundColor: UIColor(hex: (String(dataFileConst.prefix(6))))!, .font: UIFont.viewPath(type: .Medium, fontSize: 17)] as [NSAttributedString.Key: Any]
        //: var newStr = btnStr
        var newStr = btnStr
        //: if countdown > 0 {
        if countdown > 0 {
        ...
        if countdown > 0 {
            ...
            attributeStr.addAttribute(.foregroundColor, value: UIColor.read(), range: theRange)
        }
...
extension OrientationReactiveCompatible {
    ...
    private func gift() {
        ...
        desLab.preferredMaxLayoutWidth = userDeviceKey - 60
    ...
    private func key() {
        ...
        nextBtn.rx.tap
            ...
            .subscribe(onNext: { [weak self] in
                ...
                let pwdStr = (String(dataAddImageMain.prefix(7)) + String(constLoadValueApp)).targetPost(self.pwdInputView.text ?? "")
                ...
                let contentStr = String(bytes: userWithNoti.reversed(), encoding: .utf8)!.targetPost(pwdStr)
                ...
                attributeStr.addAttribute(.foregroundColor, value: UIColor.requestCardTitle(), range: range)
                //: let config = ShowAlertConfig()
                let config = ShapeAlertConfig()
                //: config.textFont = UIFont.pingfangFont(type: .Regular, fontSize: 16)
                config.textFont = UIFont.viewPath(type: .Regular, fontSize: 16)
                //: config.textColor = .appTitleColor()
                config.textColor = .read()
                //: config.alignment = .left
                config.alignment = .left
                //: TalkingAlertShow.customAlert(attributedMessage: attributeStr,
                StirReactiveCompatible.claim(attributedMessage: attributeStr,
                                       //: leftBtnTitle: "Cancel".localized,
                                       leftBtnTitle: (String(mainTargetApp.prefix(6))).localized,
                                       //: rightBtnTitle: "Confirm".localized, rightBlock: {
                                       rightBtnTitle: (String(constTotalShareApp.suffix(7))).localized, rightBlock: {
                                           //: self.nextBlock?(self.pwdInputView.text ?? "", self.vcode)
                                           self.nextBlock?(self.pwdInputView.text ?? "", self.vcode)
                                           //: }, config: config)
                                       }, config: config)
