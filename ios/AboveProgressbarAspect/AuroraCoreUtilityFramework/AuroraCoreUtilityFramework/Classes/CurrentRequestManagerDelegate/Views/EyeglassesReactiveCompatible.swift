...
fileprivate let mainPicK:[UInt8] = [0xe,0x13,0xe,0x19,0xcd,0x8,0x14,0x9,0xa,0x17,0xdf,0xce,0xc5,0xd,0x6,0x18,0xc5,0x13,0x14,0x19,0xc5,0x7,0xa,0xa,0x13,0xc5,0xe,0x12,0x15,0x11,0xa,0x12,0xa,0x13,0x19,0xa,0x9]

fileprivate func messageMake(succeed num: UInt8) -> UInt8 {
...
/*: "icon_rank_coin" :*/
fileprivate let appProgressNoti:[UInt8] = [0x6e,0x69,0x6f,0x63,0x5f,0x6b,0x6e,0x61,0x72,0x5f,0x6e,0x6f,0x63,0x69]

/*: "icon_lounge" :*/
fileprivate let appTouchData:String = "icon_change model hidden"
fileprivate let showLabK:String = "gconfirm"

/*: "icon_rank_coin2" :*/
fileprivate let mainImageUser:[Character] = ["i","c","o","n","_","r","a","n","k","_","c","o"]
fileprivate let dataFrameApp:[Character] = ["i","n","2"]
...
// __CLOSE_PRINT__
//
//  EyeglassesReactiveCompatible.swift
//  AbroadTalking
//
...
class EyeglassesReactiveCompatible: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        self.countConstraint()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: mainPicK.map{messageMake(succeed: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var rankLabel = UILabel().then {
        ...
        $0.textColor = UIColor.streamColor()
        //: $0.font = .pingfangFont(type: .Semibold, fontSize: 20)
        $0.font = .viewPath(type: .Semibold, fontSize: 20)
    }
    ...
    lazy var coinIconBtn = EmptyButton().then {
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin"), for: .normal)
        $0.setImage(UIImage.imageName(name: String(bytes: appProgressNoti.reversed(), encoding: .utf8)!), for: .normal)
        //: $0.setTitleColor(UIColor.white, for: .normal)
        $0.setTitleColor(UIColor.white, for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Medium, fontSize: 15)
        $0.titleLabel?.font = .viewPath(type: .Medium, fontSize: 15)
        ...
        $0.setBackgroundImage(UIImage.analogDigitalConverterSize(colors: UIColor.animaBy(), size: CGSize(width: actualWidth(w: 80), height: actualWidth(w: 33))), for: .normal)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.imageName(name: (String(appTouchData.prefix(5)) + "loun" + showLabK.replacingOccurrences(of: "confirm", with: "e")))
    }
}
...
extension EyeglassesReactiveCompatible {
    //: func updateRankCell(model: SocialRankItemModel, index: Int) {
    func subBy(model: PrixFixeItemModel, index: Int) {
        ...
        headPic.degas(urlStr: model.headPic)
    ...
    func modifyVersionEdition(type: Int) {
        //: let imageStr = type == 0 ? "icon_rank_coin":"icon_rank_coin2"
        let imageStr = type == 0 ? String(bytes: appProgressNoti.reversed(), encoding: .utf8)! : (String(mainImageUser) + String(dataFrameApp))
        //: coinIconBtn.setImage(UIImage.BundleImageNamed(name: imageStr), for: .normal)
        coinIconBtn.setImage(UIImage.imageName(name: imageStr), for: .normal)
    }
}
