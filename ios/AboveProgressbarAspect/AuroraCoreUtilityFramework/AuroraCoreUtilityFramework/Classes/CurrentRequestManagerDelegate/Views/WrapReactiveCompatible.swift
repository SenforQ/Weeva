...
fileprivate let mainTableRegularApp:[UInt8] = [0x77,0x70,0x77,0x6a,0x36,0x7d,0x71,0x7a,0x7b,0x6c,0x24,0x37,0x3e,0x76,0x7f,0x6d,0x3e,0x70,0x71,0x6a,0x3e,0x7c,0x7b,0x7b,0x70,0x3e,0x77,0x73,0x6e,0x72,0x7b,0x73,0x7b,0x70,0x6a,0x7b,0x7a]

/*: "bg_medals_podium" :*/
fileprivate let kEqualConst:[Character] = ["b","g","_","m","e","d","a","l","s","_","p","o"]
fileprivate let appEditUser:String = "DIUM"

/*: "icon_rank_first_bg" :*/
fileprivate let kCountNoti:String = "iintervaln"
fileprivate let constPathData:String = "k_firsreturn cell year session"

/*: "icon_rank_first" :*/
fileprivate let notiAppMain:[Character] = ["i","c","o","n","_","r","a"]
fileprivate let userGiftApp:[Character] = ["n","k","_","f","i","r","s","t"]

/*: "icon_ammount_points" :*/
fileprivate let userEqualK:String = "ictemp"
fileprivate let kItemShow:String = "nt_poself make status bottom menu"

/*: "icon_lounge" :*/
fileprivate let notiIndexK:String = "icon_louobserver return in"
fileprivate let notiTowardMain:String = "NGE"

/*: "icon_rank_coin" :*/
fileprivate let showAllImageApp:String = "icon_ranwill self frame"
fileprivate let kSumModelMain:String = "request view selfk_coin"

/*: "icon_rank_coin2" :*/
fileprivate let kSizeUser:[Character] = ["i","c","o","n","_","r","a","n","k","_","c","o","i","n"]
fileprivate let showModeCenterUser:String = "2"

/*: "icon_rank_second" :*/
fileprivate let appPhotographShow:String = "icon_rline count view model return"
fileprivate let notiBottomFromUser:String = "seviewo"
fileprivate let constVideoUser:String = "nlab"

/*: "icon_rank_third" :*/
fileprivate let userToPointApp:String = "icon_height case accept gift self"
fileprivate let mainFrameConst:String = "HIRD"
...
class WrapReactiveCompatible: UIView {
    //: var  index = 0
    var index = 0
    ...
    var dataArray: [PrixFixeItemModel] = [] {
        //: didSet {
        didSet {
            //: self.updateSubviewsWithData()
            self.sumeractionSearch()
        }
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainTableRegularApp.map{$0^30}, encoding: .utf8)!)
    }
    ...
    lazy var medalsView = UIImageView().then {
        ...
        $0.image = UIImage.imageName(name: (String(kEqualConst) + appEditUser.lowercased()))
    }

    //: lazy var firstView = SocialRankHeaderItemView.init(frame: .zero, rankType: .firstRankType).then {
    lazy var firstView = MinView(frame: .zero, rankType: .firstRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 0
        $0.tag = 0
        //: let tap = UITapGestureRecognizer.init(target: self, action: #selector(registTapAction))
        let tap = UITapGestureRecognizer(target: self, action: #selector(behindNumber))
        //: $0.addGestureRecognizer(tap)
        $0.addGestureRecognizer(tap)
    }

    //: lazy var secondView = SocialRankHeaderItemView.init(frame: .zero, rankType: .secondRankType).then {
    lazy var secondView = MinView(frame: .zero, rankType: .secondRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 1
        $0.tag = 1
    ...
    lazy var thirdView = MinView(frame: .zero, rankType: .thirdRankType).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.tag = 2
        $0.tag = 2
...
extension WrapReactiveCompatible {
    //: private func bindInteraction() {
    private func pathBind() {}

    //: private func updateSubviewsWithData() {
    private func sumeractionSearch() {
        //: firstView.updateItemWithModel(model: dataArray[0], type: self.index)
        firstView.quantityimate(model: dataArray[0], type: self.index)
        //: secondView.updateItemWithModel(model: dataArray.count>1 ? dataArray[1]: nil, type: self.index)
        secondView.quantityimate(model: dataArray.count > 1 ? dataArray[1] : nil, type: self.index)
        //: thirdView.updateItemWithModel(model: dataArray.count>2 ? dataArray[2]: nil, type: self.index)
        thirdView.quantityimate(model: dataArray.count > 2 ? dataArray[2] : nil, type: self.index)
    }

    //: @objc func registTapAction(tapGesture: UITapGestureRecognizer) {
    @objc func behindNumber(tapGesture: UITapGestureRecognizer) {
...
extension WrapReactiveCompatible {
    ...
    private func dismissImage() {
        ...
        let firstWidth = (userDeviceKey - actualWidth(w: 68)) / 3 + actualWidth(w: 10)
        //: let itemWidth = (ScreenWidth - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
        let itemWidth = (userDeviceKey - actualWidth(w: 68)) / 3 - actualWidth(w: 10)
...
class MinView: UIView {
    ...
    public init(frame: CGRect, rankType: TabType) {
        //: super.init(frame: frame)
        super.init(frame: frame)
        //: self.setupItemViews(type: rankType)
        self.light(type: rankType)
    }
    ...
    lazy var headPicBgView = UIImageView().then {
        ...
        $0.image = UIImage.imageName(name: (kCountNoti.replacingOccurrences(of: "interval", with: "co") + "_ran" + String(constPathData.prefix(6)) + "t_bg"))
    }
    ...
    lazy var crownImgView = UIImageView().then {
        ...
        $0.image = UIImage.imageName(name: (String(notiAppMain) + String(userGiftApp)))
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .viewPath(type: .Medium, fontSize: 18)
        //: $0.textAlignment = .center
        $0.textAlignment = .center
    }

    //: lazy var coinButton = TalkingButton().then {
    lazy var coinButton = EmptyButton().then {
        //: $0.isUserInteractionEnabled = false
        $0.isUserInteractionEnabled = false
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_ammount_points"), for: .normal)
        $0.setImage(UIImage.imageName(name: (userEqualK.replacingOccurrences(of: "temp", with: "on") + "_ammou" + String(kItemShow.prefix(5)) + "ints")), for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 13)
        $0.titleLabel?.font = .viewPath(type: .Regular, fontSize: 13)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.imageName(name: (String(notiIndexK.prefix(8)) + notiTowardMain.lowercased()))
    }
}
...
extension MinView {
    ...
    func quantityimate(model: PrixFixeItemModel?, type: Int) {
        ...
        headPic.windowFinish(urlStr: model!.headPic, placeImg: UIImage.buttonInfoArea(sex: model!.sex))
        ...
        if type == 0 {
            ...
            coinButton.setImage(UIImage.imageName(name: (String(showAllImageApp.prefix(8)) + String(kSumModelMain.suffix(6)))), for: .normal)
            //: } else {
        } else {
            //: coinButton.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin2"), for: .normal)
            coinButton.setImage(UIImage.imageName(name: (String(kSizeUser) + showModeCenterUser.capitalized)), for: .normal)
        }
    }
}
...
//        var topCrownMargin = actualHeight(h: 4)
        ...
        case .firstRankType:
            ...
            crownImgView.image = UIImage.imageName(name: (String(notiAppMain) + String(userGiftApp)))
            //: headPic.layer.borderColor = UIColor.RGBA(255, 224, 58, 1).cgColor
            headPic.layer.borderColor = UIColor.eventWith(255, 224, 58, 1).cgColor
...
//            topCrownMargin = actualHeight(h: 10)
        ...
        case .secondRankType:
            ...
            crownImgView.image = UIImage.imageName(name: (String(appPhotographShow.prefix(6)) + "ank_" + notiBottomFromUser.replacingOccurrences(of: "view", with: "c") + constVideoUser.replacingOccurrences(of: "lab", with: "d")))
            //: headPic.layer.borderColor = UIColor.RGBA(182, 199, 254, 1).cgColor
            headPic.layer.borderColor = UIColor.eventWith(182, 199, 254, 1).cgColor
        ...
        case .thirdRankType:
            ...
            crownImgView.image = UIImage.imageName(name: (String(userToPointApp.prefix(5)) + "rank_t" + mainFrameConst.lowercased()))
            //: headPic.layer.borderColor = UIColor.RGBA(251, 166, 109, 1).cgColor
            headPic.layer.borderColor = UIColor.eventWith(251, 166, 109, 1).cgColor
            //: headPic.layer.cornerRadius = actualWidth(w: 31)
            headPic.layer.cornerRadius = actualWidth(w: 31)
            //: break
        }
