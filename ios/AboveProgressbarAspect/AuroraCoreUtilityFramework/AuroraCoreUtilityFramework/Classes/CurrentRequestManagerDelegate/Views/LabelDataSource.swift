...
fileprivate let kColorConst:[UInt8] = [0x29,0x2e,0x29,0x34,0x68,0x23,0x2f,0x24,0x25,0x32,0x7a,0x69,0x60,0x28,0x21,0x33,0x60,0x2e,0x2f,0x34,0x60,0x22,0x25,0x25,0x2e,0x60,0x29,0x2d,0x30,0x2c,0x25,0x2d,0x25,0x2e,0x34,0x25,0x24]

private func subjectMatter(time num: UInt8) -> UInt8 {
    return num ^ 64
}

/*: "Not yet~" :*/
fileprivate let constRegularManagerData:String = "Not yresult info push change report"
fileprivate let kWithConst:String = "et~view new length base"

/*: "#8C7AFF" :*/
fileprivate let notiCardApp:String = "center text empty regular zone#8C7AFF"
...
class LabelDataSource: UIView {
	var startQuantity: Int = 62
	var searchedIconName: String = "block"
	var labelEachDictionary: [AnyHashable: String] = [:]
    ...
    override init(frame: CGRect) {
        ...
        super.init(frame: frame)
        //: self.setupSubviews()
        self.gestureInputArray()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: kColorConst.map{subjectMatter(time: $0)}, encoding: .utf8)!)
    }
    ...
    override func layoutSubviews() {
        ...
        ranktableView.frame = self.bounds
            ...
            self.startQuantity = fortTotal
            
            var rowGrant = type
            rowGrant *= 4
            if rowGrant > self.startQuantity {
                self.startQuantity = rowGrant
            }
            
            return self.startQuantity
            }
            rowGrant.chorusTitle = { [self] databaseContent in
            self.searchedIconName = databaseContent
            
            return self.searchedIconName
            }
            rowGrant.viewNameDictionary = { [self] nameDictionary in
            self.labelEachDictionary = nameDictionary
            
            guard var value = self.labelEachDictionary as? [String: String] else {
                return nil
            }
            return value
            }
    ...
    lazy var dataArray: [PrixFixeItemModel] = [] {
        //: didSet {
        didSet {
    ...
    lazy var ranktableView = UITableView(frame: self.bounds, style: .plain).then {
        ...
        $0.register(CompartmentResourceView.self, forCellReuseIdentifier: CompartmentResourceView.className())
        //: $0.register(SocialRankListViewCell.self, forCellReuseIdentifier: SocialRankListViewCell.className())
        $0.register(EyeglassesReactiveCompatible.self, forCellReuseIdentifier: EyeglassesReactiveCompatible.className())
    }
    ...
    lazy var tableHeaderView = WrapReactiveCompatible(frame: CGRect(x: 0, y: 0, width: userDeviceKey, height: actualHeight(h: 220 + 14))).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.index = self.type
        $0.index = self.type
    ...
    lazy var tableBackgrourdView = ShapeEmptyView().then {
        //: $0.backgroundColor = .white
        $0.backgroundColor = .white
        //: $0.desLab.text = "Not yet~".localized
        $0.desLab.text = (String(constRegularManagerData.prefix(5)) + String(kWithConst.prefix(3))).localized
        //: $0.desLab.textColor = UIColor(hex: "#8C7AFF")
        $0.desLab.textColor = UIColor(hex: (String(notiCardApp.suffix(7))))
...
extension LabelDataSource: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: CompartmentResourceView.className(), for: indexPath) as! CompartmentResourceView
            //: return cell
            return cell
        }
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: EyeglassesReactiveCompatible.className(), for: indexPath) as! EyeglassesReactiveCompatible
        //: let model = dataArray[indexPath.row+3]
        let model = dataArray[indexPath.row + 3]
        //: cell.updateRankCell(model: model, index: indexPath.row+3)
        cell.subBy(model: model, index: indexPath.row + 3)
        //: cell.updateIcon(type: self.type)
        cell.modifyVersionEdition(type: self.type)
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: CompartmentResourceView.className()) as! CompartmentResourceView
...
extension LabelDataSource: JXSegmentedListContainerViewListDelegate {
...
extension LabelDataSource {
    ...
    private func gestureInputArray() {
