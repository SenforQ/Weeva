...
fileprivate let notiMessageUser:[UInt8] = [0x20,0x27,0x20,0x3d,0x61,0x2a,0x26,0x2d,0x2c,0x3b,0x73,0x60,0x69,0x21,0x28,0x3a,0x69,0x27,0x26,0x3d,0x69,0x2b,0x2c,0x2c,0x27,0x69,0x20,0x24,0x39,0x25,0x2c,0x24,0x2c,0x27,0x3d,0x2c,0x2d]

private func countRelation(pic num: UInt8) -> UInt8 {
    return num ^ 73
}

/*: "female_bg_random" :*/
fileprivate let constReasonUser:[Character] = ["f","e","m","a","l","e","_"]
fileprivate let dataCollectionManagerNoti:String = "bg_ralet record type in view"

/*: "Random" :*/
fileprivate let userBackgroundStatusApp:String = "Randosize in for wrap content"
fileprivate let appPathK:String = "before"

/*: "get json error" :*/
fileprivate let notiSplitGiftUser:[Character] = ["g","e","t"," ","j","s","o","n"," ","e","r","r"]
fileprivate let notiExpressK:String = "oblock"

/*: "btn_female_randomvideo_nor" :*/
fileprivate let notiAppConst:[Character] = ["b","t","n","_","f","e","m"]
fileprivate let dataTempShow:String = "ale_open error button fatal"
fileprivate let notiListUser:String = "omvicolor"
fileprivate let notiPathLabData:[Character] = ["e","o","_","n","o","r"]

/*: "All" :*/
fileprivate let showShadowConst:String = "local bottom name shared positionAll"

/*: "Intimate" :*/
fileprivate let notiTouchShow:String = "Intimwindow view model path"
fileprivate let showKindConst:String = "aarray"

/*: "randomVideo" :*/
fileprivate let dataFormatTaskMain:[Character] = ["r","a","n","d","o","m","V"]
fileprivate let appKitMain:String = "appdeo"

/*: "#777777" :*/
fileprivate let showMakeMain:String = "#shareshare"
fileprivate let showModelApp:[Character] = ["7","7"]
...
@objc enum GainCountLiteral: Int {
    //: case All = 0
    case All = 0
    //: case intimate
    case intimate
...
@objc protocol FibonacciSequenceViewDelegate: NSObjectProtocol {
    ///
    //: @objc func changeTopItem(type: ChatListTopItemType)
    @objc func deleteColor(type: GainCountLiteral)
}

//: class TalkingChatListTopItemView: UIView {
class ResourceItemView: UIView {
    //: var selectButton = UIButton()
    var selectButton = UIButton()
    //: var resetButton = UIButton()
    var resetButton = UIButton()
    //: weak var delegate: TalkingChatListTopItemViewDelegate?
    weak var delegate: FibonacciSequenceViewDelegate?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiMessageUser.map{countRelation(pic: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var femaleRandomBtn: UIImageView = {
        ...
        let image = UIImage.imageName(name: (String(constReasonUser) + String(dataCollectionManagerNoti.prefix(5)) + "ndom"))
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(timeForEnd))
    ...
    private lazy var randomLab: UILabel = {
        ...
        lab.text = (String(userBackgroundStatusApp.prefix(5)) + appPathK.replacingOccurrences(of: "before", with: "m")).localized
        //: lab.textColor = .white
        lab.textColor = .white
    ...
    private lazy var svgaPlayer: SVGAPlayer = {
        ...
        let url = ShareImageThen.default.comeMagnitudeo(type: .Female_Randomvideo_Enter)
        //: do {
        do {
            ...
            printLog(message: (String(notiSplitGiftUser) + notiExpressK.replacingOccurrences(of: "block", with: "r")))
        }
    ...
    private lazy var stopRandomImgV: UIImageView = {
        ...
        let img = UIImageView(image: UIImage.imageName(name: (String(notiAppConst) + String(dataTempShow.prefix(4)) + "rand" + notiListUser.replacingOccurrences(of: "color", with: "d") + String(notiPathLabData))))
...
extension ResourceItemView {
    /// 进入随机视频页按钮事件
    //: @objc private func femaleRandomButtonClick() {
    @objc private func timeForEnd() {
        //: let vc = TalkingFemaleStaticRandomVideoVC()
        let vc = WearerViewController()
    ...
    func windowAcross() {
        ...
        phonation(sender: resetButton)
    }

    //: @objc private func buttonItemChange(sender: UIButton) {
    @objc private func phonation(sender: UIButton) {
        ...
        selectButton.titleLabel?.font = UIFont.viewPath(type: .Medium, fontSize: 16)
        //: selectButton.isSelected = false
        selectButton.isSelected = false
        //: sender.isSelected = true
        sender.isSelected = true
        //: sender.titleLabel?.font = UIFont.pingfangFont(type: .Medium, fontSize: 15)
        sender.titleLabel?.font = UIFont.viewPath(type: .Medium, fontSize: 15)
        //: selectButton = sender
        selectButton = sender
        //: updateSliderViewFrame(btn: sender)
        premier(btn: sender)
        //: delegate?.changeTopItem(type: ChatListTopItemType(rawValue: sender.tag)!)
        delegate?.deleteColor(type: GainCountLiteral(rawValue: sender.tag)!)
    }
}
...
extension ResourceItemView {
    ...
    private func iconOf() {
        ...
        let items = [(String(showShadowConst.suffix(3))).localized, (String(notiTouchShow.prefix(5)) + showKindConst.replacingOccurrences(of: "array", with: "te")).localized]
        //: var tag = ChatListTopItemType.All.rawValue
        var tag = GainCountLiteral.All.rawValue
        //: let buttonW = 70.0
        let buttonW = 70.0
        //: var buttonX: CGFloat = LanguageManager.shared.direction == .leftToRight ? -5:20
        var buttonX: CGFloat = ManagerReactiveCompatible.shared.direction == .leftToRight ? -5 : 20
        //: for (i, item) in items.enumerated() {
        for (i, item) in items.enumerated() {
            //: let itemButton = creatButton(item: item, type: tag)
            let itemButton = sinceModel(item: item, type: tag)
            //: addSubview(itemButton)
            addSubview(itemButton)
            //: let x = LanguageManager.shared.direction == .leftToRight ? buttonX:(RowMacroDefine.getScreenWidth()-buttonX-buttonW)
            let x = ManagerReactiveCompatible.shared.direction == .leftToRight ? buttonX : (RowMacroDefine.customAbout() - buttonX - buttonW)
            ...
            if i == 0 {
                ...
                phonation(sender: itemButton)
            }
        }
        //: addSubview(sliderView)
        addSubview(sliderView)
        //: updateSliderViewFrame(btn: resetButton)
        premier(btn: resetButton)
    ...
    @objc private func scale() {
        ...
        //: PositionLabelReactiveCompatible.share.appUserConfigMode.rx
        PositionLabelReactiveCompatible.share.appUserConfigMode.rx
            //: .observeWeakly(String.self, "randomVideo")
            .observeWeakly(String.self, (String(dataFormatTaskMain) + appKitMain.replacingOccurrences(of: "app", with: "i")))
    ...
    private func sinceModel(item: String, type: Int) -> UIButton {
        //: let btn = UIButton()
        let btn = UIButton()
        //: btn.setTitle(item, for: .normal)
        btn.setTitle(item, for: .normal)
        //: btn.setTitleColor(UIColor(hex: "#777777"), for: .normal)
        btn.setTitleColor(UIColor(hex: (showMakeMain.replacingOccurrences(of: "share", with: "77") + String(showModelApp))), for: .normal)
        //: btn.setTitleColor(UIColor.appTitleColor(), for: .selected)
        btn.setTitleColor(UIColor.read(), for: .selected)
        ...
        btn.addTarget(self, action: #selector(phonation(sender:)), for: .touchUpInside)
        //: return btn
        return btn
    }

    //: private func updateSliderViewFrame(btn: UIButton) {
    private func premier(btn: UIButton) {
