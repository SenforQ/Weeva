...
fileprivate let notiLabConst:[UInt8] = [0xdc,0xdb,0xdc,0xc1,0x9d,0xd6,0xda,0xd1,0xd0,0xc7,0x8f,0x9c,0x95,0xdd,0xd4,0xc6,0x95,0xdb,0xda,0xc1,0x95,0xd7,0xd0,0xd0,0xdb,0x95,0xdc,0xd8,0xc5,0xd9,0xd0,0xd8,0xd0,0xdb,0xc1,0xd0,0xd1]

private func beforeBar(back num: UInt8) -> UInt8 {
    return num ^ 181
}

/*: "bth_live_follow_pre" :*/
fileprivate let showAddActionApp:[Character] = ["b","t","h","_","l","i","v","e","_","f","o","l","l","o","w"]
fileprivate let dataCellConst:[Character] = ["_","p","r","e"]

/*: "btn_live_closed_nor" :*/
fileprivate let dataGiftMain:String = "btn_livself context"
fileprivate let constDataK:String = "user to true stope_cl"

/*: "removeAttentionUid" :*/
fileprivate let kCenterMainNoti:String = "rescaleov"
fileprivate let mainKitNoti:String = "style viewntionUid"

/*: "attentionUid" :*/
fileprivate let kSwitchicialConst:String = "atteviewtio"
fileprivate let constTitleK:String = "field path equalnUid"

/*: "source" :*/
fileprivate let constMonthAppShow:[Character] = ["s","o","u","r","c","e"]

/*: "num" :*/
fileprivate let userButtonK:[Character] = ["n","u","m"]

/*: "99+" :*/
fileprivate let showTitleK:[UInt8] = [0x5a,0x5a,0x48]

/*: "UID: :*/
fileprivate let userWhiteData:[Character] = ["U","I","D",":"]
...
class PhotoReactiveCompatible: UIView {
    ...
    private var partyModel = CacheUIMeasurable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiLabConst.map{beforeBar(back: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var iconBgBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(echogram), for: .touchUpInside)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var followBtn: UIButton = {
        ...
        btn.setImage(UIImage.imageName(name: (String(showAddActionApp) + String(dataCellConst))), for: .normal)
        //: btn.addTarget(self, action: #selector(followButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(strain), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var miniBtn: UIButton = {
        ...
        btn.setImage(UIImage.imageName(name: (String(dataGiftMain.prefix(7)) + String(constDataK.suffix(4)) + "osed_nor")), for: .normal)
        //: btn.addTarget(self, action: #selector(miniButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(suiteOn), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var onlineNumBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(enableline), for: .touchUpInside)
    ...
    private lazy var onlineListView: RunningListView = {
        //: let v = TalkingVoiceRoomOnlineListView()
        let v = RunningListView()
...
extension PhotoReactiveCompatible {
    ...
    @objc func likeGift(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(kCenterMainNoti.replacingOccurrences(of: "scale", with: "m") + "eAtte" + String(mainKitNoti.suffix(8)))] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(kSwitchicialConst.replacingOccurrences(of: "view", with: "n") + String(constTitleK.suffix(4)))] = uid
            //: dict["source"] = "2"
            dict[(String(constMonthAppShow))] = "2"
        }
        //: TitleRequestTool.req_atationTool(isAttention: !isAttention, params: dict) { succeed, _, _ in
...
extension PhotoReactiveCompatible {
    ...
    @objc private func fillMagnitude() {
        //: TalkingVoiceRoomReqTool.req_partyOnlineNum(roomId: partyModel.roomId) { succeed, result, _ in
        DetailReqTool.receiveCompletion(roomId: partyModel.roomId) { succeed, result, _ in
            //: guard succeed else { return }
            guard succeed else { return }
            //: let json = JSON(result ?? [String: Any]())
            let json = JSON(result ?? [String: Any]())
            //: self.onlineNum = json["num"].intValue
            self.onlineNum = json[(String(userButtonK))].intValue
    ...
    private func startData() {
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 60, target: self, selector: #selector(fillMagnitude), userInfo: nil, repeats: true)
        //: onlineNunTimer?.fire()
        onlineNunTimer?.fire()
    }
...
extension PhotoReactiveCompatible {
    ...
    func restoreToSightView() {
        ...
        icon.windowFinish(urlStr: partyModel.streamerInfo.headPic, placeImg: UIImage.buttonInfoArea(sex: partyModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        //: let isAnchor = (String(partyModel.streamerInfo.uid) == PositionLabelReactiveCompatible.share.loginUid)
        let isAnchor = (String(partyModel.streamerInfo.uid) == PositionLabelReactiveCompatible.share.loginUid)
        //: if isAnchor {
        if isAnchor {
            //: refreshFollow(isAttention: true)
            isClick(isAttention: true)
            //: } else {
        } else {
            //: refreshFollow(isAttention: partyModel.streamerInfo.isAttention)
            isClick(isAttention: partyModel.streamerInfo.isAttention)
        }
        // 定时更新在线人数
        //: startTimer()
        startData()
    ...
    @objc private func strain() {
        //: req_followRequest(uid: partyModel.streamerInfo.uid, isAttention: partyModel.streamerInfo.isAttention) { [weak self] succeed in
        likeGift(uid: partyModel.streamerInfo.uid, isAttention: partyModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.isClick(isAttention: self.partyModel.streamerInfo.isAttention)
        }
    }

    /// 最小化按钮点击事件
    //: @objc private func miniButtonClick() {
    @objc private func suiteOn() {
        //: superview?.endEditing(true)
        superview?.endEditing(true)
        //: let view = TalkingVoiceRoomCloseView()
        let view = SequenceQuantityThen()
        //: view.show()
        view.gestureImageValue()
    }

    /// 刷新关注按钮
    //: func refreshFollow(isAttention: Bool) {
    func isClick(isAttention: Bool) {
...
extension PhotoReactiveCompatible {
    ...
    private func soccerPlayerConstraint() {
        //: frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: StatusBarHeight+actualWidth(w: 43))
        frame = CGRect(x: 0, y: 0, width: userDeviceKey, height: showDomainUrl + actualWidth(w: 43))
