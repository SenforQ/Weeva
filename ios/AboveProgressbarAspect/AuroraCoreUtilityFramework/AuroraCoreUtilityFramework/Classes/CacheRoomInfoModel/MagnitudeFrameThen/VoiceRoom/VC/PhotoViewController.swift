...
/*: "bg_party" :*/
fileprivate let kGiftActionData:[Character] = ["b","g","_","p","a","r","t","y"]

/*: "You have been muted" :*/
fileprivate let dataRecordTextApp:String = "You var gift false block nor"
fileprivate let kPickShow:[Character] = ["n"," "]
fileprivate let userOpenK:[Character] = ["m","u","t","e","d"]

/*: "PartyGift_ :*/
fileprivate let appRandomMain:[Character] = ["P","a","r","t","y","G","i","f","t","_"]
...
let mainAvailableId = CXCallObserver()
//: class TalkingVoiceRoomViewController: TalkingBaseViewController {
class PhotoViewController: EmptyRecognizerDelegate {
    ...
    private lazy var bgView: UIImageView = {
        ...
        img.image = UIImage.imageName(name: (String(kGiftActionData)))
    ...
    lazy var bottomView: SurnameReactiveCompatible = {
        //: let view = TalkingVoiceRoomBottomView()
        let view = SurnameReactiveCompatible()
        //: view.delegate = self
        view.delegate = self
        //: return view
        return view
    ...
    private lazy var gradientLayer: CAGradientLayer = {
        ...
        layer.frame = CGRect(x: 0, y: 0, width: userDeviceKey, height: showDomainUrl + actualWidth(w: 85))
    ...
    private lazy var topView: PhotoReactiveCompatible = {
        //: let view = TalkingVoiceRoomTopView()
        let view = PhotoReactiveCompatible()
        //: view.showUserCardBlock = { [weak self] uid in
        view.showUserCardBlock = { [weak self] uid in
            //: guard let self = self else { return }
            guard let self = self else { return }
...
extension PhotoViewController {
    ...
    func glide() {
        ...
        topView.applicationTimer()
        //: danmuView.stopTimer()
        danmuView.bigData()
        //: TalkingDanmuManager.qiutGroupDanmu(groupId: TalkingVoiceRoomManager.shared().partyModel.chatGroupId)
        ToponymThen.qiutAndSettleQuick(groupId: LoungeEventHandler.imageShared().partyModel.chatGroupId)
...
extension PhotoViewController {
    ...
    @objc private func swanBy() {
        //: updateViewConstraints(offset: ScreenHeight-HalfViewTopMargin-10)
        cream(offset: appNameMessage - mainSelectImageName - 10)
    }
    ...
    @objc private func eventMessage() {
...
extension PhotoViewController: CXCallObserverDelegate {
    ...
    func callObserver(_: CXCallObserver, callChanged call: CXCall) {
        ...
        if call.hasConnected == true { // 通话中
            ...
            guard LoungeEventHandler.imageShared().isParty else { return }
            //: TalkingVoiceRoomManager.shared().voiceRoom_releaseAllResource()
            LoungeEventHandler.imageShared().toResource()
        }
    }
...
// MARK: - : ScienceLabThen【弹幕代理】

//: extension TalkingVoiceRoomViewController: TalkingDanmuManagerDelegate {
extension PhotoViewController: ScienceLabThen {
...
extension PhotoViewController: EventThen {
    ...
    func command(msgStr: String, atUid: String?) {
        ...
        ToponymThen.currentList(groupId: LoungeEventHandler.imageShared().partyModel.chatGroupId, message: msgStr, toUid: atUid)
    }
...
// MARK: - DeleteObjectProtocol【底部评论按钮代理】

//: extension TalkingVoiceRoomViewController: TalkingVoiceRoomBottomViewDelegate {
extension PhotoViewController: DeleteObjectProtocol {
    //: func bottom_commentBtnClick() {
    func areaPop() {
        // 拉起弹幕评论
        //: if Int(Date().timeIntervalSince1970) < TalkingVoiceRoomManager.shared().partyModel.muteExpireAt {
        if Int(Date().timeIntervalSince1970) < LoungeEventHandler.imageShared().partyModel.muteExpireAt {
            //: func__showStatusBarErrorMsg(showMsg: "You have been muted".localized)
            exceptAdd(showMsg: (String(dataRecordTextApp.prefix(4)) + "have bee" + String(kPickShow) + String(userOpenK)).localized)
            //: return
            return
        }
...
extension PhotoViewController: EnableViewDelegate {
    ...
    func lineIcon(uid: String, mikeStatus: Int, position: Int) {
        ...
        LoungeEventHandler.imageShared().liveView(uid: uid.intValue, type: mikeStatus == 2 ? 5 : 4, position: position)
    }

    //: func func_showUserCardView(pushUid: String?) {
    func arc(pushUid: String?) {
        //: if let uidStr = pushUid, !uidStr.isEmptyString {
        if let uidStr = pushUid, !uidStr.isEmptyString {
            //: let view = TalkingVoiceRoomCardView(frame: self.view.frame, uid: uidStr)
            let view = TitleCardView(frame: self.view.frame, uid: uidStr)
            //: view.delegate = self
            view.delegate = self
            //: view.show()
            view.mortificationTo()
        }
    }
    ...
    func tabApp(uid: String, nickname: String) {
        ...
        nick(nickName: nickname, atUid: uid)
    }

    //: func func__followUserSucess(uid: String) {
    func titleDismiss(uid _: String) {
        //: TalkingVoiceRoomManager.shared().partyModel.streamerInfo.isAttention = true
        LoungeEventHandler.imageShared().partyModel.streamerInfo.isAttention = true
        //: topView.refreshFollow(isAttention: true)
        topView.isClick(isAttention: true)
    }
}
...
extension PhotoViewController {
    ...
    func sight() {
        ...
        let toUid = (String(appRandomMain)) + "\(LoungeEventHandler.imageShared().partyModel.streamerInfo.uid)"
    ...
    func voiceMsg(Msg: ThoughtImageDanmuModel) {
...
extension PhotoViewController {
    ...
    private func outMagnitudeerchange() {
        ...
        mikePositionView.snp.makeConstraints { make in
            ...
            make.height.equalTo(userEnvironmentTabValue.height * 2)
        }
    ...
    private func phone() {
        ...
        mainAvailableId.setDelegate(self, queue: DispatchQueue.main)
    }
}
