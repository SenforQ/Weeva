...
fileprivate let dataFrameShow:[UInt8] = [0x17,0x10,0x17,0xa,0x56,0x1d,0x11,0x1a,0x1b,0xc,0x44,0x57,0x5e,0x16,0x1f,0xd,0x5e,0x10,0x11,0xa,0x5e,0x1c,0x1b,0x1b,0x10,0x5e,0x17,0x13,0xe,0x12,0x1b,0x13,0x1b,0x10,0xa,0x1b,0x1a]

private func bagMe(source num: UInt8) -> UInt8 {
    return num ^ 126
}

/*: "#171717" :*/
fileprivate let kModelUser:String = "#toto"
fileprivate let mainShootNoti:[Character] = ["1","7"]

/*: "icon_cameraoff" :*/
fileprivate let appColorShow:String = "icon_device add photo bottom"
...
class NonnegativeThen: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.backgroundColor = UIColor.eventWith(51, 51, 51, 1)
        ...
        modelConstraint()
        //: bindInteraction()
        labyrinthQuantityeraction()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: dataFrameShow.map{bagMe(source: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var bgView: UIImageView = {
        ...
        alphaV.backgroundColor = UIColor(hex: (kModelUser.replacingOccurrences(of: "to", with: "17") + String(mainShootNoti)))
        //: alphaV.alpha = 0.6
        alphaV.alpha = 0.6
        //: v.addSubview(alphaV)
        v.addSubview(alphaV)
    ...
    private lazy var icon: UIImageView = {
        ...
        v.image = UIImage.imageName(name: (String(appColorShow.prefix(5)) + "cameraoff"))
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
...
extension NonnegativeThen: UIGestureRecognizerDelegate {
    /// 拖拽位移手势
    /// - Parameter pan: 手势
    //: @objc private func dragWindow(pan: UIPanGestureRecognizer) {
    @objc private func analogDigitalConverterView(pan: UIPanGestureRecognizer) {
        ...
        newCenterX = min(newCenterX, userDeviceKey - self.frame.size.width * 0.5)
        //: newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        //: newCenterY = min(newCenterY, ScreenHeight - self.frame.size.height * 0.5)
        newCenterY = min(newCenterY, appNameMessage - self.frame.size.height * 0.5)
...
extension NonnegativeThen {
    ...
    private func labyrinthQuantityeraction() {
        // 切换视角手势
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(exchangeRenderContainer))
        let tap = UITapGestureRecognizer(target: self, action: #selector(imageComponent))
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(analogDigitalConverterView(pan:)))
        //: pan.delegate = self
        pan.delegate = self
