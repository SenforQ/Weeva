...
fileprivate let constPriceData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
@objc protocol CurrentShareObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func recommence()

    //: @objc optional func endScroll()
    @objc optional func addScroll()
}

//: class TalkingVideoDanmuMsgListTableView: UIView {
class UserTableView: UIView {
	var giftSum: Int = 39
	var liveContent: String = "lab"
	var telecastingDictionary: [AnyHashable: String] = [:]
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(inform))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constPriceData.reversed(), encoding: .utf8)!)
    }
    ...
    override func layoutSubviews() {
        ...
        super.layoutSubviews()
            ...
            if (msgTableView.preservesSuperviewLayoutMargins) && (msgTableView.autoresizingMask == .flexibleLeftMargin) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let playGift = GiftView(frame: msgTableView.frame.insetBy(dx: CGFloat(382.93), dy: CGFloat(91)))
            playGift.bindOff = self.inPending
            playGift.menuNumber = reloadTimeSpan

            
            
            playGift.textSum = { [self] fortTotal in
            self.giftSum = fortTotal
            
            var playGift = RoomMsgScroViewTag
            playGift ^= playGift ^ 5
            if playGift > self.giftSum {
                self.giftSum = playGift
            }
            
            return self.giftSum
            }
            playGift.chorusTitle = { [self] databaseContent in
            self.liveContent = databaseContent
            
            return self.liveContent
            }
            playGift.viewNameDictionary = { [self] nameDictionary in
            self.telecastingDictionary = nameDictionary
            
            guard var value = self.telecastingDictionary as? [String: String] else {
                return nil
            }
            return value
            }
                msgTableView.addSubview(playGift)
            }
...
extension UserTableView {
    ...
    func nameUp() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(aboutEvent), userInfo: nil, repeats: true)
    }
...
extension UserTableView {
    ...
    func evoke(msgModel: PhotoDanmuModel?) {
    ...
    func sexualPerversion() {
        ...
        if AllHeight > showTagUrl {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < showTagUrl {
                ...
                self.msgTableView.height = showTagUrl
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = showTagUrl - AllHeight
        ...
        alongStart(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func alongStart(animated: Bool) {
...
extension UserTableView: UITableViewDelegate, UITableViewDataSource {
    ...
    func pairAcross(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.addScroll?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(appStatusFormat)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = ViewBassCell.supplement(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension UserTableView {
    ...
    func setupZone() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: notiGiftFormat, height: showTagUrl)
    }
}
