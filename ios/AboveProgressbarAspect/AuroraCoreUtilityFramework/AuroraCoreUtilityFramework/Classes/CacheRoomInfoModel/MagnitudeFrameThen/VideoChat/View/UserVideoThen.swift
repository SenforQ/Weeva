...
fileprivate let appRawViewData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
class UserVideoThen: UIView {
	var mpOn: Bool = false
	var swaddlingClothesMagnitude: Int = 30

    //: private var giftSendItem = [TalkingGiftNumArrModel()]
    private var giftSendItem = [SpeedMeasurable()]
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appRawViewData.reversed(), encoding: .utf8)!)
    }
    ...
    func attributed(attributedText: NSAttributedString) {
        ...
        shadow.shadowColor = UIColor.equalComplexionBackground()
        ...
        gradientLabel.attributedText = attributedText
            ...
            if (gradientLabel.layer.anchorPoint.y != 0.5) && (!gradientLabel.isOpaque) {
            ...
            self.mpOn = titleDoing
            
            return self.mpOn
            }
            eyeBlockCell.talkLabInterval = { [self] bringHomeCount in
            self.swaddlingClothesMagnitude = bringHomeCount
            
            return self.swaddlingClothesMagnitude
            }
                gradientLabel.addSubview(eyeBlockCell)
            }
    ...
    private lazy var gradientLayer: CAGradientLayer = {
        ...
        lay.colors = [UIColor.eventWith(255, 232, 72, 1).cgColor, // 浅黄
                      //: UIColor.RGBA(253, 170, 52, 1).cgColor
                      UIColor.eventWith(253, 170, 52, 1).cgColor, // 深黄
        ]
...
extension UserVideoThen {
    // 添加视图
    //: private func setupSubviews() {
    private func popEnable() {
