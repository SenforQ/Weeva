...
/*: ：" :*/
fileprivate let showFirstApp:[Character] = ["："]

/*: "#BAFBFF" :*/
fileprivate let constModeApp:[Character] = ["#"]
fileprivate let showPathUser:[Character] = ["B","A","F","B","F","F"]
...
class VideoCellData: NSObject {
    ...
    func pleasureModel(model: PhotoDanmuModel) -> PhotoDanmuModel {
        //: self.model = model
        self.model = model
        /// 普通文本消息
        //: if self.model.messageType == 1 {
    ...
    func distanceComment() {
        //: let paraStyle = paragraphStyle()
        let paraStyle = fileStyle()
        //: paraStyle.lineSpacing = 3.0
        paraStyle.lineSpacing = 3.0 // 行间距
        // 名字
        //: let firstStr = "\(self.model.nickname)："
        let firstStr = "\(self.model.nickname)："
        //: let name = getAttributed(text: firstStr, font: UIFont.liveBackground(fontSize: 14), color: UIColor.init(hex: "#BAFBFF")!)
        let name = liveModel(text: firstStr, font: UIFont.liveBackground(fontSize: 14), color: UIColor(hex: (String(constModeApp) + String(showPathUser)))!)
        // 内容
        //: let content = getAttributed(text: self.model.content, font: UIFont.liveBackground(fontSize: 14), color: UIColor.white)
        let content = liveModel(text: self.model.content, font: UIFont.liveBackground(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        ...
        attribError(attribText: self.model.msgAttribText)
    }
    ...
    func viewRange() {
        ...
        let name = liveModel(text: firstStr, font: UIFont.liveBackground(fontSize: 14), color: UIColor(hex: (String(constModeApp) + String(showPathUser)))!)
        // 内容
        //: let content = getAttributed(text: self.model.transContent, font: UIFont.liveBackground(fontSize: 14), color: UIColor.white)
        let content = liveModel(text: self.model.transContent, font: UIFont.liveBackground(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        //: name.yy_paragraphStyle = paraStyle
        name.yy_paragraphStyle = paraStyle
    ...
    func fileStyle() -> NSMutableParagraphStyle {
    ...
    func liveModel(text: String, font: UIFont, color: UIColor) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = fileStyle()
        //: return attribute
        return attribute
    }
    ...
    func attribError(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = VideoMsgTableViewWidth - 36
        let maxWidth = notiGiftFormat - 36
