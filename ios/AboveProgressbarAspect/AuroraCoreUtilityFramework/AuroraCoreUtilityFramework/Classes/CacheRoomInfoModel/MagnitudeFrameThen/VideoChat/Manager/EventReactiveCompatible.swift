...
/*: "Speaker On" :*/
fileprivate let showReasonJumpConst:String = "pan player image guard colorSpeake"
fileprivate let appMyK:[Character] = ["r"," ","O","n"]

/*: "Speaker Off" :*/
fileprivate let kUserNoti:[Character] = ["S","p","e","a","k","e","r"," ","O"]
fileprivate let mainImageViewNoti:[Character] = ["f","f"]

/*: "s- :*/
fileprivate let dataTopApp:[Character] = ["s","-"]

/*: - :*/
fileprivate let appLuckShow:String = "-"

/*: "TRTC 音频通话销毁. logId： :*/
fileprivate let appAllowLeadingInfoK:String = "indexRindexC"
fileprivate let kCornerNoti:String = "毁. lmanager video leading self frame"
fileprivate let kQuantityervalConst:String = "true page self var frameogId："

/*: ." :*/
fileprivate let userWithData:String = "color"

/*: "TRTC 音频通话错误. errCode: :*/
fileprivate let showInfoUser:[Character] = ["T","R","T","C"," ","音"]
fileprivate let mainEraseSucceedUser:String = "频\u{901a}\u{8bdd}错"
fileprivate let userMakeAtData:String = "rCode:custom view"

/*: , errMsg: :*/
fileprivate let constSectionMain:String = "hidden to send, errMs"
fileprivate let constClickInfoShow:String = "g:for frame from model"

/*: "TRTC 音频通话SDK收到警告. warningCode: :*/
fileprivate let kManagerShow:String = "TRTC 音push input end"
fileprivate let userSizeData:String = "K收到警告string style else make equal"
fileprivate let showToData:[Character] = ["n"]
fileprivate let notiModelNetwork:String = "layer row var var shadowgCode:"

/*: , warningMsg: :*/
fileprivate let kViewWithApp:String = "for true, warn"
fileprivate let mainLabConst:[Character] = ["s","g",":"]

/*: "TRTC 音频通话进入房间. result: :*/
fileprivate let appInstanceMain:[Character] = ["T","R","T","C"," ","音","\u{9891}","通","话","进","入","房","\u{95f4}","."," ","r","e","s"]
fileprivate let userModelMain:String = "ult:accept let request view"

/*: , logId： :*/
fileprivate let showWhiteK:String = "video cur transaction, lo"

/*:  加入了房间" :*/
fileprivate let mainUserK:String = " 加入了\u{623f}间"

/*:  离开了房间" :*/
fileprivate let kListData:String = " 离开了房间"
...
private class PhotoChatConfig: NSObject {
...
class EventReactiveCompatible: NSObject {
    ...
    private var config = PhotoChatConfig() // 音频渲染配置类
    //: private var logId = 0
...
extension EventReactiveCompatible {
    ...
    func makeId(roomId: Int) {
        //: self.handsfreeBtn?.isEnabled = !hasHeadSet()
        self.handsfreeBtn?.isEnabled = !rubric()
    ...
    func emplaceInsideColorHandsfree(isHandsfree: Bool) {
        //: guard hasHeadSet() == false else { return }
        guard rubric() == false else { return }
        ...
        self.handsfreeDesLab?.text = isHandsfree ? (String(showReasonJumpConst.suffix(6)) + String(appMyK)).localized : (String(kUserNoti) + String(mainImageViewNoti)).localized
        //: updateWhenPlayDeviceChanged(isHandsfree: isHandsfree)
        transactionHandsfree(isHandsfree: isHandsfree)
    }
}
...
extension EventReactiveCompatible {
    ...
    func instancePath() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(withApp(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(sectionNotif(notif:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(dateChange(notif:)),
    ...
    @objc private func withApp(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func allow(notif: Notification?) {
        //: if notif == nil {
        if notif == nil {
    ...
    @objc private func sectionNotif(notif: Notification) {
    ...
    @objc func dateChange(notif: Notification) {
        ...
        case .newDeviceAvailable: // 听筒
            ...
            transactionHandsfree(isHandsfree: false)
        ...
        case .oldDeviceUnavailable: // 免提
            ...
            self.handsfreeDesLab?.text = (String(showReasonJumpConst.suffix(6)) + String(appMyK)).localized
            //: updateWhenPlayDeviceChanged(isHandsfree: self.config.isHandsfree)
            transactionHandsfree(isHandsfree: self.config.isHandsfree)
...
extension EventReactiveCompatible {
    ...
    private func transactionHandsfree(isHandsfree: Bool) {
    ...
    private func tidalRiver() {
        //: UploadLogTool.writeLog(msg: "TRTC 音频通话销毁. logId：\(logId).")
        NameReactiveCompatible.trademarkIndex(msg: (appAllowLeadingInfoK.replacingOccurrences(of: "index", with: "T") + " 音频通话\u{9500}" + String(kCornerNoti.prefix(4)) + String(kQuantityervalConst.suffix(5))) + "\(logId).")
        ...
        if self.config.isHandsfree == false { // 恢复系统扬声器播放声音
            ...
            transactionHandsfree(isHandsfree: true)
        }
...
extension EventReactiveCompatible: TRTCCloudDelegate {
    ...
    func onError(_ errCode: TXLiteAVError, errMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        NameReactiveCompatible.trademarkIndex(msg: (String(showInfoUser) + mainEraseSucceedUser + "误. er" + String(userMakeAtData.prefix(6))) + "\(errCode)" + (String(constSectionMain.suffix(7)) + String(constClickInfoShow.prefix(2))) + "\(String(describing: errMsg)).")
        //: if errMsg != nil {
        if errMsg != nil {
    ...
    func onWarning(_ warningCode: TXLiteAVWarning, warningMsg: String?, extInfo _: [AnyHashable: Any]?) {
        ...
        NameReactiveCompatible.trademarkIndex(msg: (String(kManagerShow.prefix(6)) + "频通\u{8bdd}SD" + String(userSizeData.prefix(5)) + ". warni" + String(showToData) + String(notiModelNetwork.suffix(6))) + "\(warningCode)" + (String(kViewWithApp.suffix(6)) + "ingM" + String(mainLabConst)) + "\(String(describing: warningMsg)).")
    }

    //: func onEnterRoom(_ result: Int) {
    func onEnterRoom(_ result: Int) {
        //: UploadLogTool.writeLog(msg: "TRTC 音频通话进入房间. result:\(result), logId：\(logId).")
        NameReactiveCompatible.trademarkIndex(msg: (String(appInstanceMain) + String(userModelMain.prefix(4))) + "\(result)" + (String(showWhiteK.suffix(4)) + "gId：") + "\(logId).")
        //: if result < 0 {
        if result < 0 { // 加入失败
