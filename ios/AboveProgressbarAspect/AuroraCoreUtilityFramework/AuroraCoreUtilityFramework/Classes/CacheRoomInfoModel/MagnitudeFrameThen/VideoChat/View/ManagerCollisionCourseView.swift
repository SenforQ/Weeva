...
fileprivate let kMagnitudeApp:[UInt8] = [0x7b,0x7c,0x7b,0x66,0x3a,0x71,0x7d,0x76,0x77,0x60,0x28,0x3b,0x32,0x7a,0x73,0x61,0x32,0x7c,0x7d,0x66,0x32,0x70,0x77,0x77,0x7c,0x32,0x7b,0x7f,0x62,0x7e,0x77,0x7f,0x77,0x7c,0x66,0x77,0x76]

private func picHidden(err num: UInt8) -> UInt8 {
    return num ^ 18
}

/*: "#FFD13A" :*/
fileprivate let dataQuantityimateShow:[Character] = ["#","F","F","D","1","3","A"]

/*: "gift_gb_umnad_1" :*/
fileprivate let userLayerStatusConst:String = "lab if maingift_"
fileprivate let appAtNextK:String = "index up warnmnad_"
fileprivate let showFilterData:String = "1"

/*: "icon_circle_pic_default" :*/
fileprivate let notiLayerShow:String = "var original size viewicon_c"
fileprivate let kFillHeadConst:String = "e_picextension attribute player origin moment"
fileprivate let constModelAppForUser:String = "color view gift aspect_default"

/*: "Open lucky blind-box" :*/
fileprivate let kAppearData:String = "file center ofOpe"
fileprivate let userHomeNoti:String = "n lucview model"
fileprivate let constMakeShow:String = "var row to selfnd-box"

/*: "Sent " :*/
fileprivate let notiMessageShow:String = "Sent title make file"

/*: "PingFangSC-Semibold" :*/
fileprivate let notiViewUser:String = "PingFastreet message change return"
fileprivate let dataTotalInfoApp:[Character] = ["m","i","b","o","l","d"]

/*: "%d" :*/
fileprivate let notiMainUser:String = "%dpath other let import"

/*: "Bombard" :*/
fileprivate let mainLabelViewLetNoti:String = "Bombardrequest path view"
...
// __CLOSE_PRINT__
//
//  ManagerCollisionCourseView.swift
//  AbroadTalking
//
...
class ManagerCollisionCourseView: UIView {
    //: var giftMsgModel = TalkingGiftMsgAnimatModel()
    var giftMsgModel = ShareMeasurable()
    //: var giftSendItem = [TalkingGiftNumArrModel()]
    var giftSendItem = [SpeedMeasurable()]
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kMagnitudeApp.map{picHidden(err: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var nickLb: UILabel = {
        ...
        lb.font = .startFontSize(fontSize: 14)
...
//        lb.numberOfLines = 1
        ...
        lb.textColor = UIColor(hex: (String(dataQuantityimateShow)))
        //: lb.font = UIFont.pingfangRugularFont(fontSize: 13)
        lb.font = UIFont.startFontSize(fontSize: 13)
        //: return lb
        return lb
        //: }()
    }()
    ...
    private lazy var pluralTip: UserVideoThen = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = UserVideoThen()
        //: lab.isHidden = true
        lab.isHidden = true
    ...
    private lazy var countLb: UserVideoThen = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = UserVideoThen()
...
extension ManagerCollisionCourseView {
    //: func setGiftMsgModel(model: TalkingGiftMsgAnimatModel) {
    func resignMake(model: ShareMeasurable) {
        ...
        self.bgImage.image = UIImage.imageName(name: (String(userLayerStatusConst.suffix(5)) + "gb_u" + String(appAtNextK.suffix(5)) + showFilterData.capitalized))

        //: let placeholder = UIImage.BundleImageNamed(name: "icon_circle_pic_default")
        let placeholder = UIImage.imageName(name: (String(notiLayerShow.suffix(6)) + "ircl" + String(kFillHeadConst.prefix(5)) + String(constModelAppForUser.suffix(8))))

        //: if (model.showType == ChatGiftAnimatType.myStery.rawValue ) {
        if model.showType == ContentVarArg.myStery.rawValue {
            //: self.nickLb.text = "Open lucky blind-box".localized
            self.nickLb.text = (String(kAppearData.suffix(3)) + String(userHomeNoti.prefix(5)) + "ky bli" + String(constMakeShow.suffix(6))).localized
            //: }else {
        } else {
        ...
        self.avatarImage.windowFinish(urlStr: giftMsgModel.fromHeadPic, placeImg: placeholder)
        //: self.giftImage.setUrlImage(urlStr: giftMsgModel.giftPic, placeImg: placeholder)
        self.giftImage.windowFinish(urlStr: giftMsgModel.giftPic, placeImg: placeholder)

        //: self.giftName.text = "Sent ".localized + giftMsgModel.pname
        self.giftName.text = (String(notiMessageShow.prefix(5))).localized + giftMsgModel.pname
        ...
        if sendNumber > 1 {
            ...
            var font1 = UIFont(name: (String(notiViewUser.prefix(6)) + "ngSC-Se" + String(dataTotalInfoApp)), size: 22)
            //: font1 = italicFontWith(angle: 15, fontSize: 18, fontName: font1!.fontName)
            font1 = imageContent(angle: 15, fontSize: 18, fontName: font1!.fontName)
            //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 26)
            var font2 = UIFont(name: (String(notiViewUser.prefix(6)) + "ngSC-Se" + String(dataTotalInfoApp)), size: 26)
            //: font2 = italicFontWith(angle: 15, fontSize: 22, fontName: font2!.fontName)
            font2 = imageContent(angle: 15, fontSize: 22, fontName: font2!.fontName)
            ...
            numText = numText.snapLine(texts: strs, colors: colors, fonts: fonts)
        ...
        titleOn(giftCount: giftMsgModel.comboNum)
    }

    //: func refreshGiftCount(giftCount: Int) {
    func titleOn(giftCount: Int) {
        //: var font1 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font1 = UIFont(name: (String(notiViewUser.prefix(6)) + "ngSC-Se" + String(dataTotalInfoApp)), size: 45)
        //: font1 = italicFontWith(angle: 15, fontSize: 26, fontName: font1!.fontName)
        font1 = imageContent(angle: 15, fontSize: 26, fontName: font1!.fontName)
        //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font2 = UIFont(name: (String(notiViewUser.prefix(6)) + "ngSC-Se" + String(dataTotalInfoApp)), size: 45)
        //: font2 = italicFontWith(angle: 15, fontSize: 38, fontName: font2!.fontName)
        font2 = imageContent(angle: 15, fontSize: 38, fontName: font2!.fontName)
        ...
        let colors = [UIColor.eventWith(150, 77, 20, 1), UIColor.white, UIColor.eventWith(150, 77, 20, 1)]
        ...
        attText = attText.snapLine(texts: strs, colors: colors, fonts: fonts)
        ...
        countLb.attributed(attributedText: attText)
        ...
        countLb.titleBy(frame: CGRect(x: x, y: countLb.origin.y, width: width, height: countLb.size.height))
    }

    /// 连击
    //: func comboHitWith(giftMsgModel: TalkingGiftMsgAnimatModel) {
    func addModel(giftMsgModel: ShareMeasurable) {
        ...
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(popHide), object: nil)
        ...
        self.perform(#selector(popHide), afterDelay: 3.5)
    }

    /// 展示
    //: func showGiftAnimate(model: TalkingGiftMsgAnimatModel, animate: Bool) {
    func manageAnimate(model _: ShareMeasurable, animate _: Bool) {
...
extension ManagerCollisionCourseView {
    ...
    func imageContent(angle: CGFloat, fontSize: CGFloat, fontName: String) -> (UIFont) {
...
extension ManagerCollisionCourseView {
    ...
    private func addColor() {
        ...
        font = imageContent(angle: 15, fontSize: 20, fontName: font.fontName)
        ...
        let attText = attributedString.snapLine(texts: [(String(mainLabelViewLetNoti.prefix(7))).localized], colors: [UIColor.eventWith(255, 232, 72, 1)], fonts: [font])
        //: pluralTip.setFrame(frame: CGRect.init(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height-20))
        pluralTip.titleBy(frame: CGRect(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height - 20))

        //: pluralTip.setAttributedText(attributedText: attText)
        pluralTip.attributed(attributedText: attText)
        //: contentView.addSubview(pluralTip)
        contentView.addSubview(pluralTip)

        //: countLb.setFrame(frame: CGRect.init(x: giftImage.right, y: 2, width: 70, height: height))
        countLb.titleBy(frame: CGRect(x: giftImage.right, y: 2, width: 70, height: height))
