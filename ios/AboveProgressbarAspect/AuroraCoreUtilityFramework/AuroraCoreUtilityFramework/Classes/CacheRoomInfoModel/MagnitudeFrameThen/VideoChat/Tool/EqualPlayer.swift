...
/*: "call_bgm" :*/
fileprivate let userStartData:[Character] = ["c"]
fileprivate let showLayerData:String = "all_bgmlab progress area"

/*: "call_response_bgm" :*/
fileprivate let constSexUser:String = "CALL"
fileprivate let mainLeaveK:String = "e_bgmreturn self let language"
...
class EqualPlayer: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = ButtonScalar.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func tableBar(type: ButtonScalar) -> EqualPlayer {
        //: let player = TalkingCallBGMPlayer()
        let player = EqualPlayer()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.mic()
...
extension EqualPlayer {
    ...
    private func counternationality() {
    ...
    private func notifyModify(notify: Notification) {
    ...
    private func postAcross(notify: Notification) {
    ...
    private func sensoryReceptorTalk() -> Bool {
    ...
    private func mic() {
        ...
        var fileName = (String(userStartData) + String(showLayerData.prefix(7)))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (constSexUser.lowercased() + "_respons" + String(mainLeaveK.prefix(5)))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = ShareImageThen.default.livePoint(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension EqualPlayer: TXVodPlayListener {
