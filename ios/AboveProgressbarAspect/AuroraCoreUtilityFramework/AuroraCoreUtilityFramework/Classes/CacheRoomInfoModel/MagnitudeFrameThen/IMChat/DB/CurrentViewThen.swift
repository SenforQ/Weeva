...
/*: "serialQueue" :*/
fileprivate let constTapShow:[Character] = ["s","e","r","i","a","l"]
fileprivate let showModelUser:[Character] = ["Q","u","e","u","e"]

/*: "fromUids" :*/
fileprivate let mainInputMenuUser:[UInt8] = [0x73,0x64,0x69,0x55,0x6d,0x6f,0x72,0x66]

//: Declare String End

// __DEBUG__
// __CLOSE_PRINT__
//  CurrentViewThen.swift
//  AbroadTalking
//
...
public class CurrentViewThen: NSObject {
    ...
    @objc private func actionRecord() {
        //: DBUserInfoManager.destroy()
        CurrentViewThen.pathDate()
    }
}
...
extension CurrentViewThen {
    /// 获取用户消息
    /// - Parameters:
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: class func cache_getUserInfo(with userIdsArr: [String], completion: @escaping ([EventMeasurable]?, Error?) -> Void) {
    class func beyondStatus(with userIdsArr: [String], completion: @escaping ([EventMeasurable]?, Error?) -> Void) {
        // 数量过多，wcdb的where语句会因过长而报错，所以做分页
        //: var newUserIdsArr = userIdsArr
        var newUserIdsArr = userIdsArr
        //: if DBUserInfoManager.shared().needReqUidsSet != nil {
        if CurrentViewThen.containerShared().needReqUidsSet != nil { // 合并去重
            //: newUserIdsArr = Array(DBUserInfoManager.shared().needReqUidsSet!.union(userIdsArr))
            newUserIdsArr = Array(CurrentViewThen.containerShared().needReqUidsSet!.union(userIdsArr))
            //: DBUserInfoManager.shared().needReqUidsSet = nil
            CurrentViewThen.containerShared().needReqUidsSet = nil
        }
        ...
        repeat {
            ...
            keyEmpty(with: subArr, completion: completion)
            //: location += len
            location += len
    ...
    class func takeCare(with userArr: [String], completion: @escaping ([[String: Any]]?, Error?) -> Void) {
        ...
        let serialQueue = DispatchQueue(label: (String(constTapShow) + String(showModelUser)))
        ...
        serialQueue.async {
            ...
            for item in finalArray {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                    //: TitleRequestTool.req_queryStatus(toUids: requestUidsStr) { _, list in
                    TitleRequestTool.finishCompletion(toUids: requestUidsStr) { _, list in
                        //: defer { sema.signal() }
                        defer { sema.signal() }
    ...
    public class func getMessage(message: V2TIMMessage) -> AudileReactiveCompatible {
        //: let key = TUISocialChatManager.checkPrimaryKey(withMsgModel: message)
        let key = SocietalChatManager.buttonDestroy(withMsgModel: message)
        //: var cacheWrap = WCDBVoiceMsgTable.db_getVoiceMsg(with: key)
        var cacheWrap = AudileReactiveCompatible.atWith(with: key)
        //: if cacheWrap == nil {
        if cacheWrap == nil {
            //: let info = TUISocialChatManager.packageDict(withTXMessage: message)
            let info = SocietalChatManager.atImage(withTXMessage: message)
            //: cacheWrap = WCDBVoiceMsgTable.db_inserVoiceMsgToDB(info)
            cacheWrap = AudileReactiveCompatible.dismissDb(info)
        }
        //: return cacheWrap!
        return cacheWrap!
    }
...
extension CurrentViewThen {
    ...
    //: @discardableResult
    @discardableResult
    //: class func cache_getCachedUserInfo(targetId: String) -> EventMeasurable? {
    class func eventId(targetId: String) -> EventMeasurable? {
        //: return WCDBUserInfoTable.db_getUserInfo(with: targetId)
        return TitleInfoTable.recordSuite(with: targetId)
    }

    /// 更新数据库【用户模型】，并发送通知
    /// - Parameter userInfo: 用户信息model
    //: class func cache_updateMsgUserInfoAndPostNotif(with userInfo: EventMeasurable?) {
    class func readoutVideo(with userInfo: EventMeasurable?) {
        ...
        let model = CurrentViewThen.eventId(targetId: userInfo.uid)
        // 自定义字段需要赋值，防止被覆盖
        ...
        TitleInfoTable.userReloadInfo(userInfo)
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: NotificationCenter.default.post(name: USER_INFO_MSG_LIST_NOTIFICATION, object: [userInfo.uid: userInfo])
            NotificationCenter.default.post(name: notiErrorName, object: [userInfo.uid: userInfo])
        }
    }
    ...
    class func formatTime(with targetId: String, intimate: Int, currTime: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = CurrentViewThen.eventId(targetId: targetId) else { return }
    ...
    class func deal(with targetId: String, time: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = CurrentViewThen.eventId(targetId: targetId) else { return }
    ...
    class func illegitimacy(with targetUid: String) {
        //: WCDBUserInfoTable.db_deleteUserInfo(with: targetUid)
        TitleInfoTable.atAll(with: targetUid)
        //: DBUserInfoManager.shared().reqedUidsSet.remove(targetUid)
        CurrentViewThen.containerShared().reqedUidsSet.remove(targetUid)
    }
}
...
extension CurrentViewThen {
    ...
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: private class func inner_getUserInfo(with userIdsArr: [String], completion: @escaping ([EventMeasurable]?, Error?) -> Void) {
    private class func keyEmpty(with userIdsArr: [String], completion: @escaping ([EventMeasurable]?, Error?) -> Void) {
        //: let userModels = WCDBUserInfoTable.db_getUserInfos(with: userIdsArr)
        let userModels = TitleInfoTable.terms(with: userIdsArr)
        ...
        for uid in userIdsArr where !userModels.contains(where: { $0.uid == uid }) {
            ...
            if CurrentViewThen.containerShared().reqedUidsSet.contains(uid) == false {
        ...
        if !noDBUidArr.isEmpty {
            ...
            CurrentViewThen.endUserVideo(with: noDBUidArr)
        }
    }
    ...
    private class func endUserVideo(with userIdsArr: [String]) {
        ...
        CurrentViewThen.containerShared().reqedUidsSet = CurrentViewThen.containerShared().reqedUidsSet.union(userIdsArr)

        // 请求用户消息列表
        //: TitleRequestTool.req_refreshMsgUserListInfo(param: ["fromUids": requestUidsStr]) { succeed, dataArr in
        TitleRequestTool.subDown(param: [String(bytes: mainInputMenuUser.reversed(), encoding: .utf8)!: requestUidsStr]) { succeed, dataArr in
            //: guard succeed else {
            guard succeed else { // 失败还原数据
                //: DBUserInfoManager.shared().reqedUidsSet.subtract(userIdsArr)
                CurrentViewThen.containerShared().reqedUidsSet.subtract(userIdsArr)
                //: return
                return
            }
            // 更新数据库
            //: DBUserInfoManager.updateMsgUserInfoTable(userInfoArr: dataArr)
            CurrentViewThen.createPlace(userInfoArr: dataArr)
            // 发送通知
            //: var retInfoDict: [String: EventMeasurable] = [:]
            var retInfoDict: [String: EventMeasurable] = [:]
            ...
            if !retInfoDict.isEmpty {
                ...
                DispatchQueue.main.async {
                    ...
                    NotificationCenter.default.post(name: notiErrorName, object: retInfoDict)
                }
            }
        }
    }
    ...
    //: private class func updateMsgUserInfoTable(userInfoArr: [EventMeasurable]) {
    private class func createPlace(userInfoArr: [EventMeasurable]) {
        //: for aWrap in userInfoArr {
        for aWrap in userInfoArr {
            //: WCDBUserInfoTable.db_updateUserInfo(aWrap)
            TitleInfoTable.userReloadInfo(aWrap)
        }
    }
}
