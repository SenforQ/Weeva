...
fileprivate let notiEmptyApp:[UInt8] = [0xa1,0xa6,0xa1,0xbc,0xe0,0xab,0xa7,0xac,0xad,0xba,0xf2,0xe1,0xe8,0xa0,0xa9,0xbb,0xe8,0xa6,0xa7,0xbc,0xe8,0xaa,0xad,0xad,0xa6,0xe8,0xa1,0xa5,0xb8,0xa4,0xad,0xa5,0xad,0xa6,0xbc,0xad,0xac]

private func bagView(app num: UInt8) -> UInt8 {
    return num ^ 200
}

/*: "0/ :*/
fileprivate let dataExplainConst:[Character] = ["0","/"]

/*: "#AAAAAA" :*/
fileprivate let notiPopNumberK:String = "input world path progress#AAAAAA"

/*: "btn_chatsettings_unchoice" :*/
fileprivate let constSucceedShow:String = "make result add if iconbtn_ch"
fileprivate let appMainNoti:String = "INGS"
fileprivate let mainNameNoti:String = "_unchoicein time"

/*: "btn_chatsettings_choiced" :*/
fileprivate let constNameApp:[Character] = ["b","t","n","_","c","h","a","t","s","e","t","t","i","n"]
fileprivate let appLessK:String = "size every ting make ofgs_ch"

/*: "Top" :*/
fileprivate let userMediumConst:[Character] = ["T","o","p"]

/*: "Save" :*/
fileprivate let kArrayToMain:String = "Savein name"

/*: "Cancel" :*/
fileprivate let notiModelShow:String = "chance temp outCancel"

/*: / :*/
fileprivate let kTitleApp:[Character] = ["/"]
...
class PrivateViewDelegate: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiEmptyApp.map{bagView(app: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIView = {
        ...
        let height = actualWidth(w: 475) + showNameData
        //: let v = UIView(frame: CGRect(x: 0, y: ScreenHeight-height, width: ScreenWidth, height: height))
        let v = UIView(frame: CGRect(x: 0, y: appNameMessage - height, width: userDeviceKey, height: height))
        //: v.backgroundColor = .white
        v.backgroundColor = .white
        //: v.Corner(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        v.willRadii(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var textBgView: UIView = {
        ...
        v.backgroundColor = UIColor.loadSize()
    ...
    private lazy var limitLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(notiPopNumberK.suffix(7))))
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var topBtn: EmptyButton = {
        ...
        btn.setImage(UIImage.imageName(name: (String(constSucceedShow.suffix(6)) + "atsett" + appMainNoti.lowercased() + String(mainNameNoti.prefix(9)))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btn_chatsettings_choiced"), for: .selected)
        btn.setImage(UIImage.imageName(name: (String(constNameApp) + String(appLessK.suffix(5)) + "oiced")), for: .selected)
        ...
        btn.setTitle((String(userMediumConst)).localized, for: .normal)
        //: btn.spaceBetweenTitleAndImage = 6
        btn.spaceBetweenTitleAndImage = 6
        //: btn.addTarget(self, action: #selector(topButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(fromContent), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var saveBtn: UIButton = {
        ...
        btn.setTitle((String(kArrayToMain.prefix(4))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(resume), for: .touchUpInside)
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((String(notiModelShow.suffix(6))).localized, for: .normal)
...
extension PrivateViewDelegate: YYTextViewDelegate {
...
extension PrivateViewDelegate {
    ...
    @objc private func resume() {
        ...
        let isTop = topBtn.isSelected ? 1 : 2
        //: TitleRequestTool.req_remark(content: textView.text, isTop: isTop, toUid: targetId) {
        TitleRequestTool.resign(content: textView.text, isTop: isTop, toUid: targetId) {
            //: succeed, _, _ in
            succeed, _, _ in
            ...
            self.dismissBlock?(self.textView.text, self.topBtn.isSelected ? 1 : 2)
            //: self.dismiss()
            self.equalCounteraction()
        }
    }

    /// 置顶 / 取消置顶按钮点击事件
    //: @objc private func topButtonClick() {
    @objc private func fromContent() {
    ...
    func itemMini(_ content: String, isTop: Int, toUid: String) {
        //: targetId = toUid
        targetId = toUid
