...
fileprivate let mainAspectData:[UInt8] = [0xf7,0xf0,0xf7,0xea,0xb6,0xfd,0xf1,0xfa,0xfb,0xec,0xa4,0xb7,0xbe,0xf6,0xff,0xed,0xbe,0xf0,0xf1,0xea,0xbe,0xfc,0xfb,0xfb,0xf0,0xbe,0xf7,0xf3,0xee,0xf2,0xfb,0xf3,0xfb,0xf0,0xea,0xfb,0xfa]

private func makeUser(content num: UInt8) -> UInt8 {
    return num ^ 158
}

/*: "icon_chats_banner_subtract" :*/
fileprivate let dataLoadK:[Character] = ["i","c","o","n","_","c","h","a","t","s","_","b","a","n","n","e","r","_","s","u","b","t","r","a","c","t"]

/*: "icon_chats_banner_subtract_pre" :*/
fileprivate let dataGroupEnvironmentApp:[Character] = ["i","c","o","n","_","c","h","a","t","s","_","b","a","n","n","e","r","_","s","u","b","t","r","a","c","t","_","p","r","e"]

/*: "bannerList" :*/
fileprivate let userNameModeShow:[Character] = ["b","a","n","n","e","r","L"]
fileprivate let mainCenterConst:String = "IST"

/*: "JXBannerCellID" :*/
fileprivate let notiMaximumViewData:String = "JXBantop frame self guard"

/*: "#FFFFFF" :*/
fileprivate let showRemoveConst:[Character] = ["#","F","F","F","F","F","F"]
...
class CurrentBannerView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainAspectData.map{makeUser(content: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bannerListData: [ErrorMeasurable] = //: return Array<ErrorMeasurable>()
        .init()
    //: }()
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.imageName(name: (String(dataLoadK))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chats_banner_subtract_pre"), for: .selected)
        btn.setImage(UIImage.imageName(name: (String(dataGroupEnvironmentApp))), for: .selected)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(nationality), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension CurrentBannerView {
    /// banner
    //: func reqBanner() {
    func gold() {
        //: SocialRequestManager.init().req_adBanner(position: 3) { [weak self] succeed, result, errorModel in
        TimeRequestManager().broadsideButtonCompletion(position: 3) { [weak self] _, result, _ in
            ...
            let array = JSON(result[(String(userNameModeShow) + mainCenterConst.lowercased())] ?? []).arrayValue
...
extension CurrentBannerView {
    ...
    @objc private func nationality() {
        ...
        self.delegate?.outpost(isMiniView: isMiniView)
    }

    //: func resetView(isMiniView: Bool) {
    func towardTo(isMiniView: Bool) {
        //: if isMiniView {
        if isMiniView {
...
extension CurrentBannerView: JXBannerDelegate, JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (String(notiMaximumViewData.prefix(5)) + "nerCellID"))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        tempCell.imageView.windowFinish(urlStr: url, placeImg: UIImage.changeSquare())
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(showRemoveConst)))!
