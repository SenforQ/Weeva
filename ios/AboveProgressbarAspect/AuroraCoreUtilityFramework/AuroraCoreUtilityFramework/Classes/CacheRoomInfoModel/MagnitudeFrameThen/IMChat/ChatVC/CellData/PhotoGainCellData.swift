...
fileprivate let dataStartNoti:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "View read receipts?" :*/
fileprivate let showSearchedData:String = "View user direction cell image model"
fileprivate let constDataMain:String = "rlocationa"
fileprivate let dataModelShow:String = "d retrue let push self pic"

/*: "text" :*/
fileprivate let notiTapShow:String = "responsext"

/*: "color" :*/
fileprivate let userMinMain:[UInt8] = [0x27,0x2b,0x28,0x2b,0x36]

/*: "<br/>" :*/
fileprivate let appStyleData:String = "frame mode select main<br/>"

/*: "\n" :*/
fileprivate let constEqualNoti:String = "\n"

/*: "<br />" :*/
fileprivate let constMakeApp:[Character] = ["<","b","r"," ","/"]
fileprivate let showPathNameWithoutConst:[Character] = [">"]

/*: "<xml>%@</xml>" :*/
fileprivate let showViewConst:[Character] = ["<","x","m","l",">","%","@"]
fileprivate let notiOffSearchedUser:String = "guide"
fileprivate let kGiftMain:String = "with model/xml>"

/*: "href" :*/
fileprivate let notiTagArrayData:String = "hrlabel"

/*: "type" :*/
fileprivate let constPositionCellTransformShow:[UInt8] = [0x65,0x70,0x79,0x74]

/*: "txt" :*/
fileprivate let appViewData:String = "terror"

/*: "padding" :*/
fileprivate let constModeData:String = "paddbackg"

/*: "size" :*/
fileprivate let userHiddenShow:[Character] = ["s","i","z","e"]

/*: "maxFont" :*/
fileprivate let kTitleConst:String = "maxFontextension to tap gift"

/*: "link" :*/
fileprivate let userStatusK:[Character] = ["l","i","n","k"]

/*: "PingFangSC-Regular" :*/
fileprivate let dataNextApp:[Character] = ["P","i","n","g","F","a","n","g","S","C","-","R","e","g","u","l","a","r"]

/*: "content" :*/
fileprivate let kFrameMain:[UInt8] = [0x90,0x9c,0x9d,0x87,0x96,0x9d,0x87]

private func outOfSight(count num: UInt8) -> UInt8 {
    return num ^ 243
}

/*: "%@" :*/
fileprivate let appModelNoti:String = "%@"
...
@objcMembers public class PhotoGainCellData: GainReactiveCompatible {
    ...
    override public func littler() -> CGSize {
        ...
        if self.msgModel.tips.content == (String(showSearchedData.prefix(5)) + constDataMain.replacingOccurrences(of: "location", with: "e") + String(dataModelShow.prefix(4)) + "ceipts?").localized {
            //: return CGSize(width: ScreenWidth, height: 30)
            return CGSize(width: userDeviceKey, height: 30)
        }
        ...
        //: let rect = self.attributedString.boundingRect(with: CGSize.init(width: RowMacroDefine.getScreenWidth()-50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil )
        let rect = self.attributedString.boundingRect(with: CGSize(width: RowMacroDefine.customAbout() - 50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil)
    ...
    public lazy var attributedString: NSMutableAttributedString = {
        ...
        var attr = receiveTitle(info: self.msgModel.tips)
        //: return attr
        return attr
        //: }()
    }()
...
extension PhotoGainCellData {
    //: func formatMessageToStringM(info: AbTalkingChatMsgTipsModel) -> NSMutableAttributedString {
    func receiveTitle(info: ChatModel) -> NSMutableAttributedString {
...
//        let dic = NSDictionary(jsonString: extra ?? "")
        ...
        return noWith(tips: content)
    }

    //: func designTextFream(tips: String) -> NSMutableAttributedString {
    func noWith(tips: String) -> NSMutableAttributedString {
        ...
        _ = stochastic(msg: tips, info: attrbutedArray)
        //: self.yyLayout = designYYTextLayout(array: attrbutedArray, displayMaxWidth: RowMacroDefine.getScreenWidth()-50, para: para)
        self.yyLayout = addPara(array: attrbutedArray, displayMaxWidth: RowMacroDefine.customAbout() - 50, para: para)
    ...
    func stochastic(msg: String, info: NSMutableArray) -> String {
        ...
        let pure_msg = userAttrb(source: msg, attrbAM: contentAttrArray)
        //: for  textInfo in contentAttrArray {
        for textInfo in contentAttrArray {
            //: let textDic = textInfo as![String: Any]
            let textDic = textInfo as! [String: Any]
            //: let text: String = textDic["text"] as! String
            let text: String = textDic[(notiTapShow.replacingOccurrences(of: "response", with: "te"))] as! String

            //: var textColor: UIColor?
            var textColor: UIColor?

            //: var obj = textDic["color"]
            var obj = textDic[String(bytes: userMinMain.map{$0^68}, encoding: .utf8)!]
            //: if obj is UIColor {
            if obj is UIColor {
                //: obj = obj as? UIColor
                obj = obj as? UIColor
            ...
            contentDict.removeObjects(forKeys: [String(bytes: userMinMain.map{$0^68}, encoding: .utf8)!, (notiTapShow.replacingOccurrences(of: "response", with: "te"))])

            //: let attrDict = ctDictionaryForText(content: text, color: textColor)
            let attrDict = information(content: text, color: textColor)
    ...
    func userAttrb(source: String, attrbAM: NSMutableArray) -> String {
        ...
        do {
            ...
            let document = try ONOXMLDocument(string: String(format: (String(showViewConst) + notiOffSearchedUser.replacingOccurrences(of: "guide", with: "<") + String(kGiftMain.suffix(5))), newSource), encoding: CFStringConvertEncodingToNSStringEncoding(CFStringBuiltInEncodings.UTF8.rawValue))
            //: let element = document.rootElement
            let element = document.rootElement
            //: parseXMLElement(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: ["color": UIColor.white])
            makeAttrib(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: [String(bytes: userMinMain.map{$0^68}, encoding: .utf8)!: UIColor.white])
            ...
            attrbAM.add([(notiTapShow.replacingOccurrences(of: "response", with: "te")): newSource, String(bytes: userMinMain.map{$0^68}, encoding: .utf8)!: UIColor.white])
            //: return newSource
            return newSource
        }
    }
    ...
    func makeAttrib(element: ONOXMLElement, arrM: NSMutableArray, attrib: NSDictionary) {
        ...
        if children.count > 0 {
            ...
            if element.tag == "u" { // 下滑线
                ...
                let url: String = element.attributes[(notiTagArrayData.replacingOccurrences(of: "label", with: "ef"))] as! String
                //: if url.count > 0 {
                if url.count > 0 {
                    //: parentInfo["href"] = url
                    parentInfo[(notiTagArrayData.replacingOccurrences(of: "label", with: "ef"))] = url
                }
            }
            //: for  subElement in children {
            for subElement in children {
                //: parseXMLElement(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
                makeAttrib(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
            }
            ...
            if text != nil {
                ...
                //: info["text"] = text
                info[(notiTapShow.replacingOccurrences(of: "response", with: "te"))] = text
                //: arrM.add(info)
                arrM.add(info)
            }
        }
    ...
    func addPara(array: NSArray, displayMaxWidth: CGFloat, para: NSParagraphStyle?) -> YYTextLayout {
        //: let attrString = designAttribStr(array: array, paragraphStyle: para)
        let attrString = designSend(array: array, paragraphStyle: para)
    ...
    func designSend(array: NSArray, paragraphStyle: NSParagraphStyle?) -> NSAttributedString {
        ...
        array.enumerateObjects { obj, _, _ in
            ...
            let type = newObj[String(bytes: constPositionCellTransformShow.reversed(), encoding: .utf8)!] as? String
            //: var tmp = NSAttributedString.init()
            var tmp = NSAttributedString()
            //: if type  == "txt" {
            if type == (appViewData.replacingOccurrences(of: "error", with: "xt")) {
                //: tmp = designAttribStrByTextDic(dict: newObj as NSDictionary)
                tmp = bagTitleSession(dict: newObj as NSDictionary)
                //: } else if type == "padding" {
            } else if type == (constModeData.replacingOccurrences(of: "back", with: "in")) {
                //: let imageView = UIImageView.init()
                let imageView = UIImageView()
                //: imageView.size = (newObj["size"] as! NSValue).cgSizeValue
                imageView.size = (newObj[(String(userHiddenShow))] as! NSValue).cgSizeValue
                ...
                tmp = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: newObj[(String(kTitleConst.prefix(7)))] as! UIFont, alignment: .center)
            }
            ...
            if underline {
                ...
                if len > 0 {
                    ...
                    _ = YYTextDecoration(style: .single, width: 1.5, color: newObj[String(bytes: userMinMain.map{$0^68}, encoding: .utf8)!] as? UIColor)
    ...
    func courseOfActionDict(dict: NSDictionary) -> NSAttributedString {
        //: let imageView = UIImageView.init()
        let imageView = UIImageView()
        //: imageView.size = (dict["size"]as! NSValue).cgSizeValue
        imageView.size = (dict[(String(userHiddenShow))] as! NSValue).cgSizeValue
        ...
        let attachText = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: dict[(String(kTitleConst.prefix(7)))] as! UIFont, alignment: .center)
        //: return attachText
        return attachText
    }

    //: func designAttribStrByTextDic(dict: NSDictionary) ->NSMutableAttributedString {
    func bagTitleSession(dict: NSDictionary) -> NSMutableAttributedString {
        //: let type = dict["type"] as! String
        let type = dict[String(bytes: constPositionCellTransformShow.reversed(), encoding: .utf8)!] as! String
        //: if type != "link" && type != "txt" {
        if type != (String(userStatusK)) && type != (appViewData.replacingOccurrences(of: "error", with: "xt")) {
            //: return NSMutableAttributedString.init()
            return NSMutableAttributedString()
        }
        //: let attributes = attributesWithConfig()
        let attributes = hiddenConfig()
        // set color
        //: let color = dict["color"] as? UIColor
        let color = dict[String(bytes: userMinMain.map{$0^68}, encoding: .utf8)!] as? UIColor
        //: if color != nil {
        if color != nil {
        ...
        let fontSize = (dict[(String(userHiddenShow))] as? NSNumber)?.floatValue
        //: if (fontSize ?? 0 > 0) {
        if fontSize ?? 0 > 0 {
            //: let fontRef = CTFontCreateWithName("PingFangSC-Regular" as CFString, CGFloat(fontSize!), nil)
            let fontRef = CTFontCreateWithName((String(dataNextApp)) as CFString, CGFloat(fontSize!), nil)
            //: attributes[kCTFontAttributeName] = fontRef
            attributes[kCTFontAttributeName] = fontRef
        }
        //: var content = (dict["content"] != nil) ? dict["content"] : ""
        var content = (dict[String(bytes: kFrameMain.map{outOfSight(count: $0)}, encoding: .utf8)!] != nil) ? dict[String(bytes: kFrameMain.map{outOfSight(count: $0)}, encoding: .utf8)!] : ""
    ...
    func hiddenConfig() -> NSMutableDictionary {
        //: let font = UIFont.pingfangRugularFont(fontSize: 15)
        let font = UIFont.startFontSize(fontSize: 15)
    ...
    func information(content: String, color: UIColor?) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
        ...
        if color == nil {
            ...
            return addUp(content: content)
        }
        //: return ["type": "txt",
        return [String(bytes: constPositionCellTransformShow.reversed(), encoding: .utf8)!: (appViewData.replacingOccurrences(of: "error", with: "xt")),
                //: "content": content,
                String(bytes: kFrameMain.map{outOfSight(count: $0)}, encoding: .utf8)!: content,
                //: "color": color ?? .black]
                String(bytes: userMinMain.map{$0^68}, encoding: .utf8)!: color ?? .black]
    }

    //: func ctDictionaryForText(content: String) ->NSDictionary {
    func addUp(content: String) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
            //: return NSDictionary.init()
            return NSDictionary()
