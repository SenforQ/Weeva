...
fileprivate let dataEqualApp:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_chat_start" :*/
fileprivate let userQueryPathShow:String = "equal render topicon_"
fileprivate let kHiddenStateData:String = "sstorearstore"

/*: "icon_chat_lock" :*/
fileprivate let dataUpConst:[Character] = ["i","c","o","n","_","c","h","a","t","_","l","o","c","k"]

/*: "icon_chat_burned" :*/
fileprivate let notiAboutK:[Character] = ["i","c","o","n","_","c","h","a","t","_"]
fileprivate let appAdjustConst:[Character] = ["b","u","r","n","e","d"]

/*: "icon_chat_burned_shan" :*/
fileprivate let mainToData:String = "action content typeicon_c"
fileprivate let showTitleK:String = "range file viewhat_b"
fileprivate let showViewNoti:String = "name make self pic falsed_shan"

/*: "Already burned" :*/
fileprivate let dataSizeShow:[Character] = ["A","l","r","e","a","d","y"," ","b","u","r","n","e","d"]
...
class OnMsgCell: ErrorViewCell {
	var statusOpen: Bool = true
	var addMagnitude: Int = 6

    //: var videoData: CradleThen?
    var videoData: CradleThen?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataEqualApp.reversed(), encoding: .utf8)!)
    }
    ...
    override func awakeFromNib() {
		...
		if var cellValue = videoData?.messageType { 
			if var electValue = videoData?.selected { 
				if var nameValue = self.videoData?.translatedContent { 
			            if (burnedImgView.convert(CGPoint(x: 0, y: CGFloat(0)), to: burnedImgView.superview).y == 20.66) && (burnedImgView.undoManager != nil) {
			                //: SWIFT_CUSTOM_DANGER_File_Call
			                let buttonLoadLet = TelephoneReceiverView(frame: burnedImgView.bounds.integral)
			
			            buttonLoadLet.dataModelText = nameValue
			            
			            buttonLoadLet.viewOff = { [self] titleDoing in
			            self.statusOpen = titleDoing
			            
			            var buttonLoadLet = electValue
			            buttonLoadLet = true
			            if buttonLoadLet != self.statusOpen {
			                self.statusOpen = buttonLoadLet
			            }
			            
			            return self.statusOpen
			            }
			            buttonLoadLet.talkLabInterval = { [self] bringHomeCount in
			            self.addMagnitude = bringHomeCount
			            
			            var buttonLoadLet = cellValue
			            buttonLoadLet &= 3
			            if buttonLoadLet > self.addMagnitude {
			                self.addMagnitude = buttonLoadLet
			            }
			            
			            return self.addMagnitude
			            }
			                burnedImgView.addSubview(buttonLoadLet)
			            }
    ...
    private lazy var playImgView: UIImageView = {
        ...
        imgV.image = UIImage.imageName(name: (String(userQueryPathShow.suffix(5)) + "chat_" + kHiddenStateData.replacingOccurrences(of: "store", with: "t")))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var lockImgView: UIImageView = {
        ...
        imgV.image = UIImage.imageName(name: (String(dataUpConst)))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var burnedImgView: EmptyButton = {
        ...
        imgV.setBackgroundImage(UIImage.imageName(name: (String(notiAboutK) + String(appAdjustConst))), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.imageName(name: (String(mainToData.suffix(6)) + String(showTitleK.suffix(5)) + "urne" + String(showViewNoti.suffix(6)))), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(dataSizeShow)).localized, for: .normal)
...
extension OnMsgCell {
    ...
    override func equalWith(with data: NumerousnessCellData) {
        ...
        //: self.videoData = data as? CradleThen
        self.videoData = data as? CradleThen
        ...
        self.dutationLab.text = NSDate.responseShowButton(totalTime: duration)
        ...
        self.mosaic(urlStr: videoModel.coverImg, isMosaic: imgIsMosaic)
    }
    ...
    func mosaic(urlStr: String, isMosaic: Bool) {
        ...
        if !coverImgPath.isEmptyString, isExit { // 加载本地
            ...
            //: if isMosaic {
            if isMosaic {
                //: self.picImgView.image = UIImage.duringSend(image, license: 90)
                self.picImgView.image = UIImage.duringSend(image, license: 90)
                //: } else {
            } else {
            ...
            self.picImgView.colorCarefulAdd(urlStr: urlStr) { [weak self] image in
