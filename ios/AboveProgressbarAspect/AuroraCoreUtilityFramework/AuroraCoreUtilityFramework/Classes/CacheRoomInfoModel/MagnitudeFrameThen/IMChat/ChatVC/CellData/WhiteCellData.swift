...
fileprivate let dataPlayAddApp:[UInt8] = [0xd0,0xd7,0xd0,0xcd,0x91,0xda,0xd6,0xdd,0xdc,0xcb,0x83,0x90,0x99,0xd1,0xd8,0xca,0x99,0xd7,0xd6,0xcd,0x99,0xdb,0xdc,0xdc,0xd7,0x99,0xd0,0xd4,0xc9,0xd5,0xdc,0xd4,0xdc,0xd7,0xcd,0xdc,0xdd]

private func aCard(color num: UInt8) -> UInt8 {
    return num ^ 185
}

/*: "[未知消息]" :*/
fileprivate let appMiniUser:String = "[\u{672a}知消息]"

/*: "extra" :*/
fileprivate let mainModeUser:String = "rawxtra"

/*: "</at> " :*/
fileprivate let mainCountData:[Character] = ["<","/","a","t",">"," "]

/*: "#0084FF" :*/
fileprivate let showImageData:String = "to privacy add list render#0084FF"

/*: "#FFEE4B" :*/
fileprivate let appOnUser:String = "#FFEE4commit return make me hidden"
fileprivate let userDomainToShow:String = "time"

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let userResourceData:String = "equal manager manager<at>"
fileprivate let userAddConst:String = "s]+?selected cell var self"

/*: "jumpKey" :*/
fileprivate let mainInsideConst:String = "jumpKeyname the cell var"

/*: "user" :*/
fileprivate let constMessageUser:String = "USER"

/*: "uid" :*/
fileprivate let dataFinishK:[UInt8] = [0x64,0x69,0x75]

/*: "#16D073" :*/
fileprivate let dataMainUser:String = "#16D073self and self"

/*: "#F95644" :*/
fileprivate let mainGameManagerShow:[Character] = ["#","F","9","5","6","4","4"]
...
//: @objcMembers public class WhiteCellData: TalkingChatMsgBaseCellData {
@objcMembers public class WhiteCellData: GainReactiveCompatible {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataPlayAddApp.map{aCard(color: $0)}, encoding: .utf8)!)
    }
    ...
    public lazy var contentAttrString: NSMutableAttributedString = {
        ...
        let attr = countModel(msgInfoModel: self.msgModel.msgInfo)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func countModel(msgInfoModel: LoadImageModel) -> NSMutableAttributedString {
        ...
        guard dic[(mainModeUser.replacingOccurrences(of: "raw", with: "e"))] is [String: Any] else {
            //: return NSMutableAttributedString.init(string: "")
            return NSMutableAttributedString(string: "")
        }
        ...
        let textColor = (self.direction == .MsgDirectionIncoming) ? UIColor.read() : .white
        //: let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: "#0084FF")! : UIColor(hex: "#FFEE4B")!
        let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: (String(showImageData.suffix(7))))! : UIColor(hex: (String(appOnUser.prefix(6)) + userDomainToShow.replacingOccurrences(of: "time", with: "B")))!
        ...
        let regex = try! NSRegularExpression(pattern: (String(userResourceData.suffix(4)) + "@[\\S\\" + String(userAddConst.prefix(4)) + "</at>"), options: [])
        ...
        for (index, value) in rangeArr.enumerated() {
            ...
            if jumpInfo[(String(mainInsideConst.prefix(7)))].stringValue == (constMessageUser.lowercased()) {
                //: uid = jumpInfo["uid"].intValue
                uid = jumpInfo[String(bytes: dataFinishK.reversed(), encoding: .utf8)!].intValue
            }
            //: if uid != -1 {
            if uid != -1 {
                ...
                attr.yy_setTextHighlight(range, color: atColor, backgroundColor: .clear, userInfo: [String(bytes: dataFinishK.reversed(), encoding: .utf8)!: uid]) { [weak self] _, _, _, _ in
    ...
    func photoFormat() -> NSMutableAttributedString {
        ...
        var color = UIColor(hex: (String(dataMainUser.prefix(7))))!
        //: if self.translateStatus == .MsgTranslate_fail {
        if self.translateStatus == .MsgTranslate_fail {
            //: color = UIColor(hex: "#F95644")!
            color = UIColor(hex: (String(mainGameManagerShow)))!
        }
