...
@objcMembers public class NumerousnessCellData: NSObject {
    //: public var reuseId: String?
    public var reuseId: String?
    
    //: public func heightOfWidth(_ width: CGFloat) -> CGFloat {
    public func monthPop(_ width: CGFloat) -> CGFloat {
        //: return 44
        return 44
    }
}
...
@objcMembers public class PhotoView: UITableViewCell {
	var headDictionary: [AnyHashable: String] = [:]

    
    //: private(set) public var data: TCommonCellData?
    private(set) public var data: NumerousnessCellData?
    ...
    @objc dynamic public func equalWith(with data: NumerousnessCellData) {
        //: self.data = data
        self.data = data
    
            if (self.clipsToBounds) && (self.layer.isHidden != false) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let rowView = UtilizerView()
            rowView.cycloramaSwitch = self.changeColorWhenTouched

            
            rowView.mobileItemDictionary = { [self] serviceDictionary in
            self.headDictionary = serviceDictionary
