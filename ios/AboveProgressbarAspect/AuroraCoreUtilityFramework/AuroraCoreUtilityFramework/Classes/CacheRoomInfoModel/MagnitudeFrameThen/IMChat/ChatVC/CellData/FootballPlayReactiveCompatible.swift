...
/*: "title" :*/
fileprivate let mainSocialApp:String = "titsysteme"

/*: "content" :*/
fileprivate let kViewFeeShow:String = "conyouenyou"

/*: "init(coder:) has not been implemented" :*/
fileprivate let showKeyConst:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "img" :*/
fileprivate let notiListData:[Character] = ["i","m","g"]

/*: "jumpKey" :*/
fileprivate let userTargetShow:[Character] = ["j"]
fileprivate let constSizeApp:String = "observer lab whiteumpKey"
...
// __CLOSE_PRINT__
//
//  FootballPlayReactiveCompatible.swift
//  AbroadTalking
//
...
@objcMembers public class FootballPlayReactiveCompatible: GainReactiveCompatible {
    // 数据
    //: var extraJson: JSON {
    var extraJson: JSON {
    ...
    public lazy var titleAttrStr: NSMutableAttributedString = {
        ...
        let attributes = [.foregroundColor: UIColor.read(), .font: UIFont.viewPath(type: .Medium, fontSize: 17)] as [NSAttributedString.Key: Any]
        //: let attributeStr = NSMutableAttributedString(string: extraJson["title"].stringValue)
        let attributeStr = NSMutableAttributedString(string: extraJson[(mainSocialApp.replacingOccurrences(of: "system", with: "l"))].stringValue)
    ...
    public lazy var contentAttrStr: NSMutableAttributedString = {
        ...
        let attributes = [.foregroundColor: UIColor.equalComplexionBackground(), .font: UIFont.viewPath(type: .Regular, fontSize: 14)] as [NSAttributedString.Key: Any]
        //: let attributeStr = NSMutableAttributedString(string: extraJson["content"].stringValue)
        let attributeStr = NSMutableAttributedString(string: extraJson[(kViewFeeShow.replacingOccurrences(of: "you", with: "t"))].stringValue)
        //: attributeStr.addAttributes(attributes, range: NSRange(location: 0, length: attributeStr.length))
        attributeStr.addAttributes(attributes, range: NSRange(location: 0, length: attributeStr.length))
        //: return attributeStr
        return attributeStr
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showKeyConst.reversed(), encoding: .utf8)!)
    }
    ...
    override public func monthPop(_: CGFloat) -> CGFloat {
        ...
        if extraJson[(String(notiListData))].stringValue.count > 0 {
        ...
        if extraJson[(mainSocialApp.replacingOccurrences(of: "system", with: "l"))].stringValue.count > 0 {
        ...
        if extraJson[(kViewFeeShow.replacingOccurrences(of: "you", with: "t"))].stringValue.count > 0 {
            //: height += 4
            height += 4
        ...
        if extraJson[(String(userTargetShow) + String(constSizeApp.suffix(6)))].stringValue.count > 0 {
            //: jumpHeight = 35
            jumpHeight = 35
            //: height += jumpHeight
            height += jumpHeight
