...
fileprivate let constCenterMain:[UInt8] = [0xca,0xcd,0xca,0xd7,0x8b,0xc0,0xcc,0xc7,0xc6,0xd1,0x99,0x8a,0x83,0xcb,0xc2,0xd0,0x83,0xcd,0xcc,0xd7,0x83,0xc1,0xc6,0xc6,0xcd,0x83,0xca,0xce,0xd3,0xcf,0xc6,0xce,0xc6,0xcd,0xd7,0xc6,0xc7]

private func varPage(greet num: UInt8) -> UInt8 {
    return num ^ 163
}

/*: "icon_chat_translate_nor" :*/
fileprivate let constUserData:[Character] = ["i","c","o","n","_","c","h","a","t","_","t","r","a","n","s","l","a","t","e","_","n"]
fileprivate let notiTouchApp:String = "ocrop"

/*: "icon_chat_translate_pre" :*/
fileprivate let appLayerNoti:[Character] = ["i","c","o","n","_","c","h","a","t"]
fileprivate let mainMakeApp:String = "list view of_tra"
fileprivate let showNameConst:[Character] = ["n","s","l","a","t","e","_","p","r","e"]

/*: "icon_chat_miss" :*/
fileprivate let appRemoveNoti:String = "beyond title action managericon_c"
fileprivate let userMyConst:String = "hat_missfalse let spec global"

/*: "icon_chat_yes" :*/
fileprivate let userMakeShow:String = "icon_chequal title show return"
fileprivate let showTingData:String = "view height max message cancelat_yes"
...
class HadithThen: ErrorViewCell {
    //: var transBlock: (() -> Void)?
    var transBlock: (() -> Void)?

    //: var textData: WhiteCellData?
    var textData: WhiteCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: constCenterMain.map{varPage(greet: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var transBtn: UIButton = {
        ...
        btn.setImage(UIImage.imageName(name: (String(constUserData) + notiTouchApp.replacingOccurrences(of: "crop", with: "r"))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chat_translate_pre"), for: .highlighted)
        btn.setImage(UIImage.imageName(name: (String(appLayerNoti) + String(mainMakeApp.suffix(4)) + String(showNameConst))), for: .highlighted)
...
extension HadithThen {
    ...
    override func equalWith(with data: NumerousnessCellData) {
        ...
        //: let maxWidth = RowMacroDefine.getScreenWidth() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        let maxWidth = RowMacroDefine.customAbout() - (15 + (self.cellData?.cellLayout.avatarSize.width ?? 45) + 10) * 2
        // 翻译赋值相关
        ...
        if textData.translateStatus == .MsgTranslate_translating {
            ...
            self.transStatusImagV.image = UIImage.imageName(name: (String(appRemoveNoti.suffix(6)) + String(userMyConst.prefix(8))))
            ...
            self.transStatusImagV.image = UIImage.imageName(name: (String(userMakeShow.prefix(7)) + String(showTingData.suffix(6))))
        }
    ...
    func be() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(buttonTap(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func LognCellMessageAction(tap: UILongPressGestureRecognizer) {
    @objc func buttonTap(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
