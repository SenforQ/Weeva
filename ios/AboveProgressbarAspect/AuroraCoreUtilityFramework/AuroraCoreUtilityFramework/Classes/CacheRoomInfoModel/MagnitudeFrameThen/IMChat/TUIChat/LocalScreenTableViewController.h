 ...
 *
 *  ValueKeepDelegate 协议和 LocalScreenTableViewController 类。
 *  LocalScreenTableViewController 是聊天界面的核心类，负责聊天界面大部分核心业务逻辑的处理与运算。
 *  该类的详细属性与功能，请参展本文件中 LocalScreenTableViewController 的具体声明。
 *
 ******************************************************************************/
...
@class LocalScreenTableViewController;
//: @class DetailReactiveCompatible;
@class DetailReactiveCompatible;
//: @class MessageThen;
@class MessageThen;
//: @class V2TIMMessage;
...
//: typedef NS_ENUM(NSInteger, CacheIMChatType) {
typedef NS_ENUM(NSInteger, CacheIMChatType) {
    //: CacheIMChatType_unDefined = 0,
    CacheIMChatType_unDefined = 0,
    //: CacheIMChatType_private = 1,
    CacheIMChatType_private = 1,
    //: CacheIMChatType_group = 2,
    CacheIMChatType_group = 2,
    //: CacheIMChatType_system = 3,
    CacheIMChatType_system = 3,
    //: CacheIMChatType_service = 4, 
    CacheIMChatType_service = 4, // 官方客服
    //: CacheIMChatType_meeting = 5, 
    CacheIMChatType_meeting = 5, // 公共聊天室
    //: CacheIMChatType_moreMsg = 6, 
    CacheIMChatType_moreMsg = 6, // 更多消息（超过5天未回复）
//: };
};
...
@protocol ValueKeepDelegate <NSObject>
...
- (void)imageTable:(LocalScreenTableViewController *)controller;
...
- (void)counterestOutput:(LocalScreenTableViewController *)controller;
...
- (BOOL)giftObject:(LocalScreenTableViewController *)controller add:(UIView *)view;
 ...
 */
//: - (MessageThen *)messageController:(TUIMessageController *)controller onNewMessage:(V2TIMMessage *)data;
- (MessageThen *)quote:(LocalScreenTableViewController *)controller run:(V2TIMMessage *)data;
 ...
 */
//: - (DetailReactiveCompatible *)messageController:(TUIMessageController *)controller onShowMessageData:(MessageThen *)data;
- (DetailReactiveCompatible *)circumferenceSession:(LocalScreenTableViewController *)controller roundNumber:(MessageThen *)data;
 ...
 *  @param cell 所点击的消息单元
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageAvatar:(DetailReactiveCompatible *)cell;
- (void)value:(LocalScreenTableViewController *)controller length:(DetailReactiveCompatible *)cell;
 ...
 *  @param cell 所点击的消息单元
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageContent:(DetailReactiveCompatible *)cell;
- (void)handle:(LocalScreenTableViewController *)controller packetOrAggregation:(DetailReactiveCompatible *)cell;
 ...
 */
//: - (void)messageController:(TUIMessageController *)controller onSelectMessageMenu:(NSInteger)menuType withData:(MessageThen *)data;
- (void)data:(LocalScreenTableViewController *)controller production:(NSInteger)menuType radiogram:(MessageThen *)data;
/**
...
/*重发*/
//: - (void)messageController:(TUIMessageController *)controller onRetryMessageContent:(DetailReactiveCompatible *)cell;
- (void)message:(LocalScreenTableViewController *)controller title:(DetailReactiveCompatible *)cell;

/*引用*/
//: - (void)messageController:(TUIMessageController *)controller onReplyMessage:(MessageThen *)cellData;
- (void)alongOf:(LocalScreenTableViewController *)controller giftAlong:(MessageThen *)cellData;

/*点击引用*/

//: - (void)messageController:(TUIMessageController *)controller onClickReplyMessage:(MessageThen *)cellData;
- (void)upwardly:(LocalScreenTableViewController *)controller motionFor:(MessageThen *)cellData;
...
/////////////////////////////////////////////////////////////////////////////////
//
//                         LocalScreenTableViewController
//
/////////////////////////////////////////////////////////////////////////////////

/**
 * 腾讯云 TUIKit
 *  【模块名称】LocalScreenTableViewController
...
@interface LocalScreenTableViewController : UITableViewController

/**
 *  执行 ValueKeepDelegate 协议的委托
 */
//: @property (nonatomic, weak) id<TMessageControllerDelegate> delegate;
@property (nonatomic, weak) id<ValueKeepDelegate> delegate;
 ...
 * @param uiMsgs 待删除的数据
 */
//- (void)deleteMessages:(NSArray<MessageThen *> *)uiMsgs;
...
///当前对话类型
//: @property (nonatomic, assign) CacheIMChatType currentChatType;
@property (nonatomic, assign) CacheIMChatType currentChatType;
