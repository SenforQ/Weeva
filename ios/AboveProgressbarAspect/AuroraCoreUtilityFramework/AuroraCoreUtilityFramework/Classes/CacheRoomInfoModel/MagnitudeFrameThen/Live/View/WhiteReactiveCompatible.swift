...
/*: s" :*/
fileprivate let dataMakeNoti:String = "S"

/*: "init(coder:) has not been implemented" :*/
fileprivate let kClickApp:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "bth_live_follow_pre" :*/
fileprivate let notiTimeShow:String = "make record user shared collectbth_l"
fileprivate let dataShadowApp:String = "last letllow_p"
fileprivate let appCurShow:[Character] = ["r","e"]

/*: "btn_live_mini_nor" :*/
fileprivate let kShareNoti:String = "btn_lab right"
fileprivate let notiMinimumConst:String = "var conversation super root_min"
fileprivate let notiModelApp:String = "i_norself of self video"

/*: "#2DF2FF" :*/
fileprivate let mainMethodManagerK:[Character] = ["#","2","D","F","2","F","F"]

/*: "Live" :*/
fileprivate let kImageConst:[Character] = ["L","i","v","e"]

/*: "#FF506D" :*/
fileprivate let constActualMain:String = "status import#FF506D"

/*: "  :*/
fileprivate let notiCenterApp:[Character] = [" "]

/*: "removeAttentionUid" :*/
fileprivate let kTopConst:String = "icon leading content modelremoveAt"
fileprivate let mainExecuteConst:[Character] = ["t","e","n"]
fileprivate let dataUserNoti:String = "push content self ad videotionUid"

/*: "attentionUid" :*/
fileprivate let constViewLeadingApp:[Character] = ["a","t","t","e","n","t","i","o","n"]
fileprivate let appColumnConst:String = "Uidfor card cling key make"

/*: "source" :*/
fileprivate let notiTableIndexUser:String = "SOURCE"

/*: "num" :*/
fileprivate let kLaunchData:[Character] = ["n","u","m"]

/*: "UID: :*/
fileprivate let appGenderShow:[Character] = ["U","I","D",":"]

/*: "btn_live_closed_nor" :*/
fileprivate let appPathShow:String = "btn_lishare end can selection name"
fileprivate let userSumConst:[Character] = ["e","d","_","n","o","r"]
...
class WhiteReactiveCompatible: UIView {
    ...
    weak var weakVC: EmptyRecognizerDelegate?
    // 右侧按钮点击Block
    ...
    var noFaceTime: Int = 0 {
        //: didSet {
        didSet {
            //: if noFaceTime >= PositionLabelReactiveCompatible.share.appConfigMode.countdown1 {
            if noFaceTime >= PositionLabelReactiveCompatible.share.appConfigMode.countdown1 { // 展示倒计时
                //: self.liveStatusImgV.isHidden = true
                self.liveStatusImgV.isHidden = true
                //: self.countdownView.isHidden = false
                self.countdownView.isHidden = false
                //: self.countdownLab.text = "\(MAX_NOFACETIME-noFaceTime)s"
                self.countdownLab.text = "\(appLikeName - noFaceTime)s"
                // svga
                //: self.svgaView.isHidden = false
                self.svgaView.isHidden = false
                //: self.svgaView.refreshView(countdown: "\(MAX_NOFACETIME-noFaceTime)")
                self.svgaView.customerWith(countdown: "\(appLikeName - noFaceTime)")
                //: } else {
            } else {
    ...
    private var liveModel = GiftTransformable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kClickApp.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var iconBgBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(sampleLabel), for: .touchUpInside)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var nickNameLab: UILabel = {
        ...
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.font = UIFont.liveBackground(fontSize: 11)
        lab.font = UIFont.liveBackground(fontSize: 11)
        //: return lab
        return lab
        //: }()
    ...
    private lazy var followBtn: UIButton = {
        ...
        btn.setImage(UIImage.imageName(name: (String(notiTimeShow.suffix(5)) + "ive_fo" + String(dataShadowApp.suffix(6)) + String(appCurShow))), for: .normal)
    ...
    private lazy var miniBtn: UIButton = {
        ...
        btn.setImage(UIImage.imageName(name: (String(kShareNoti.prefix(4)) + "live" + String(notiMinimumConst.suffix(4)) + String(notiModelApp.prefix(5)))), for: .normal)
        //: btn.addTarget(self, action: #selector(miniButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(numberry), for: .touchUpInside)
    ...
    private lazy var liveStatusImgV: EmptyButton = {
        ...
        let image = UIImage.requestSize(color: UIColor(hex: (String(mainMethodManagerK)))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.on(), for: .normal)
        img.setImage(image.on(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(kImageConst)).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: return img
        return img
    ...
    private lazy var countdownView: UIView = {
        ...
        icon.backgroundColor = UIColor(hex: (String(constActualMain.suffix(7))))
    ...
    private lazy var countdownLab: UILabel = {
        //: let lab = UILabel()
        let lab = UILabel()
        //: lab.font = UIFont.liveBackground(fontSize: 12)
        lab.font = UIFont.liveBackground(fontSize: 12)
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.text = " \(PositionLabelReactiveCompatible.share.appConfigMode.countdown2)s"
        lab.text = " \(PositionLabelReactiveCompatible.share.appConfigMode.countdown2)s"
    ...
    lazy var svgaView: LiveTogetherFrameSumdownView = {
        //: let v = TalkingLiveCountdownView()
        let v = LiveTogetherFrameSumdownView()
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
    ...
    private lazy var onlineNumBtn: UIButton = {
        ...
        btn.showCorner(color: UIColor.black.withAlphaComponent(0.25), forState: .normal)
        //: btn.addTarget(self, action: #selector(onlineNumBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(withTo), for: .touchUpInside)
    ...
    private lazy var onlineListView: GiftListView = {
...
extension WhiteReactiveCompatible {
    ...
    @objc func followOutput(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(String(kTopConst.suffix(8)) + String(mainExecuteConst) + String(dataUserNoti.suffix(7)))] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(String(constViewLeadingApp) + String(appColumnConst.prefix(3)))] = uid
            //: dict["source"] = "1"
            dict[(notiTableIndexUser.lowercased())] = "1"
        }
        //: TitleRequestTool.req_atationTool(isAttention: !isAttention, params: dict) { succeed, result, errorModel in
        TitleRequestTool.sexOfCompletion(isAttention: !isAttention, params: dict) { succeed, _, _ in
            //: completion?(succeed)
            completion?(succeed)
        }
    }
...
extension WhiteReactiveCompatible {
    ...
    private func scorePost() {
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 10, target: self, selector: #selector(dataDoing), userInfo: nil, repeats: true)
    }
...
extension WhiteReactiveCompatible {
    ...
    func textSend(_ liveModel: GiftTransformable) {
        ...
        uidLab.text = (String(appGenderShow)) + "\(liveModel.streamerInfo.uid)"
        ...
        icon.windowFinish(urlStr: liveModel.streamerInfo.headPic, placeImg: UIImage.buttonInfoArea(sex: liveModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        ...
        if isAnchor {
            ...
            miniBtn.setImage(UIImage.imageName(name: (String(kShareNoti.prefix(4)) + "live" + String(notiMinimumConst.suffix(4)) + String(notiModelApp.prefix(5)))), for: .normal)
            //: refreshFollow(isAttention: true)
            listAttention(isAttention: true)
            //: } else {
        } else {
            //: miniBtn.setImage(UIImage.BundleImageNamed(name: "btn_live_closed_nor"), for: .normal)
            miniBtn.setImage(UIImage.imageName(name: (String(appPathShow.prefix(6)) + "ve_clos" + String(userSumConst))), for: .normal)
            //: refreshFollow(isAttention: self.liveModel.streamerInfo.isAttention)
            listAttention(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }
    ...
    @objc func numberry() {
        ...
        if isAnchor { // 改为小窗口
            ...
            MortalLiveManager.viewShared().eraseTop(smallWindow: true)
            //: currentViewController()?.navigationController?.popViewController(animated: false)
            underTitle()?.navigationController?.popViewController(animated: false)
        }
    }

    //: @objc private func iconBgButtonClick() {
    @objc private func sampleLabel() {
        //: showUserCard(uid: String(TalkingLiveManager.shared().liveRoomModel.streamerInfo.uid))
        cypher(uid: String(MortalLiveManager.viewShared().liveRoomModel.streamerInfo.uid))
    }

    /// 展示个人资料卡
    //: private func showUserCard(uid: String) {
    private func cypher(uid: String) {
        //: let userCard = TalkingLiveUserCardView(frame: .zero, uid: uid)
        let userCard = SourceViewDelegate(frame: .zero, uid: uid)
        //: userCard.delegate = weakVC as? any TalkingLiveUserCardViewDelegate
        userCard.delegate = weakVC as? any DrawViewDelegate
        //: userCard.show()
        userCard.withName()
    }
    ...
    private func listAttention(isAttention: Bool) {
    ...
    private func ofGift() {
        ...
        self.liveStatusImgV.isHidden = !MortalLiveManager.viewShared().isLive
    ...
    @objc private func script() {
        //: req_followRequest(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
        followOutput(uid: liveModel.streamerInfo.uid, isAttention: liveModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.listAttention(isAttention: self.liveModel.streamerInfo.isAttention)
        }
    }
}
...
extension WhiteReactiveCompatible {
    ...
    private func counterchange() {
        ...
        self.frame = CGRect(x: 0, y: 0, width: userDeviceKey, height: showDomainUrl + actualWidth(w: 68))
