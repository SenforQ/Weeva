...
/*: "99+" :*/
fileprivate let userServerData:String = "levellevel+"

/*: "init(coder:) has not been implemented" :*/
fileprivate let userEventData:[UInt8] = [0x1d,0x1a,0x1d,0x0,0x5c,0x17,0x1b,0x10,0x11,0x6,0x4e,0x5d,0x54,0x1c,0x15,0x7,0x54,0x1a,0x1b,0x0,0x54,0x16,0x11,0x11,0x1a,0x54,0x1d,0x19,0x4,0x18,0x11,0x19,0x11,0x1a,0x0,0x11,0x10]

private func modeTurn(tag num: UInt8) -> UInt8 {
    return num ^ 116
}

/*: "#2DF2FF" :*/
fileprivate let mainTaskNoti:[Character] = ["#","2","D","F","2","F","F"]

/*: "Live" :*/
fileprivate let constWindowShow:[Character] = ["L","i","v","e"]

/*: "img_home_shadow_icon" :*/
fileprivate let constWithShow:String = "img_hneed image video make"
fileprivate let showValueMain:String = "ome_manager string log image"
fileprivate let dataPathToApp:String = "ow_iconi add string"

/*: "get json error" :*/
fileprivate let notiEnableK:[Character] = ["g","e","t"," ","j","s","o","n"," ","e","r","r","o","r"]

/*: "#FF2348" :*/
fileprivate let appCellLogData:String = "model start log model color#FF2348"

/*: "#35C759" :*/
fileprivate let showAddNoti:[Character] = ["#","3"]
fileprivate let mainSlideByData:String = "succeed cover5C759"

/*: "mfBean" :*/
fileprivate let kPopMain:String = "mfBeanimport label equal"

/*: "%.1f" :*/
fileprivate let showDetailNoti:[Character] = ["%",".","1","f"]

/*: "points\n≈ $" :*/
fileprivate let appEndShow:String = "poikey"
fileprivate let notiBelowData:[Character] = ["$"]

/*:   :*/
fileprivate let appValueShow:String = "model"
...
public var appLikeName: Int {
    //: return PositionLabelReactiveCompatible.share.appConfigMode.countdown1 + PositionLabelReactiveCompatible.share.appConfigMode.countdown2
    return PositionLabelReactiveCompatible.share.appConfigMode.countdown1 + PositionLabelReactiveCompatible.share.appConfigMode.countdown2
}

//: class TalkingLiveMiniView: TalkingBaseMiniView {
class PerspectiveMiniView: LiveOfReactiveCompatible {
    // 未检测出人脸时长（间隔1秒）
    ...
    var noFaceTime: Int = 0 {
        ...
        didSet {
            ...
            if noFaceTime >= PositionLabelReactiveCompatible.share.appConfigMode.countdown1 { // 展示倒计时
                ...
                svgaNumLab.text = "\(appLikeName - noFaceTime)"
                //: } else {
            } else {
                //: refreshMiniView()
                equalMakeViewAction()
    ...
    override init(frame: CGRect) {
        ...
        //: if Double(PositionLabelReactiveCompatible.share.loginUserMode.mf_bean) > 0 {
        if Double(PositionLabelReactiveCompatible.share.loginUserMode.mf_bean) > 0 {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userEventData.map{modeTurn(tag: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var liveStatusImgV: EmptyButton = {
        ...
        let image = UIImage.requestSize(color: UIColor(hex: (String(mainTaskNoti)))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.on(), for: .normal)
        img.setImage(image.on(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(constWindowShow)).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: self.addSubview(img)
        self.addSubview(img)
    ...
    private lazy var shadowImg: UIImageView = {
        ...
        img.image = UIImage.imageName(name: (String(constWithShow.prefix(5)) + String(showValueMain.prefix(4)) + "shad" + String(dataPathToApp.prefix(7))))
        //: img.layer.cornerRadius = 8
        img.layer.cornerRadius = 8
        //: img.layer.masksToBounds = true
        img.layer.masksToBounds = true
    ...
    private lazy var svgaPlayer: SVGAPlayer = {
        ...
        do {
            ...
            printLog(message: (String(notiEnableK)))
        }
        //: return player
        return player
        //: }()
    ...
    private lazy var svgaNumLab: UILabel = {
        //: let lab = UILabel()
        let lab = UILabel()
        //: lab.font = UIFont.liveBackground(fontSize: 20)
        lab.font = UIFont.liveBackground(fontSize: 20)
        //: lab.textAlignment = .center
        lab.textAlignment = .center
        //: lab.textColor = UIColor(hex: "#FF2348")
        lab.textColor = UIColor(hex: (String(appCellLogData.suffix(7))))
        //: self.addSubview(lab)
        self.addSubview(lab)
        //: return lab
        return lab
    ...
    private lazy var msgCountLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(showAddNoti) + String(mainSlideByData.suffix(5))))
        //: lab.font = UIFont.pingfangFont(type: .Medium, fontSize: 11)
        lab.font = UIFont.viewPath(type: .Medium, fontSize: 11)
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.textAlignment = .center
        lab.textAlignment = .center
...
extension PerspectiveMiniView {
    /// 构建视频小窗口
    //: static func buildLiveMiniView() -> TalkingLiveMiniView {
    static func terminate() -> PerspectiveMiniView {
        //: let view = TalkingLiveMiniView(frame: .zero)
        let view = PerspectiveMiniView(frame: .zero)
        //: let position = TalkingSocketManager.shared.liveMiniPosition
        let position = CurrentSocketManager.shared.liveMiniPosition
    ...
    func equalMakeViewAction() {
        //: liveStatusImgV.isHidden = !TalkingLiveManager.shared().isLive
        liveStatusImgV.isHidden = !MortalLiveManager.viewShared().isLive
...
extension PerspectiveMiniView {
    ...
    private func upNotification() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(titleNotification(notification:)),
    ...
    @objc private func titleNotification(notification: NSNotification) {
        //: let userinfo = notification.userInfo as! [String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: guard let mfBean = userinfo["mfBean"] as? Double else { return }
        guard let mfBean = userinfo[(String(kPopMain.prefix(6)))] as? Double else { return }
        //: setPointText(mfBean: mfBean)
        sharedIn(mfBean: mfBean)
    }

    //: func setPointText(mfBean: Double) {
    func sharedIn(mfBean: Double) {
        //: let nowBean = PositionLabelReactiveCompatible.share.loginUserMode.mf_bean + mfBean
        let nowBean = PositionLabelReactiveCompatible.share.loginUserMode.mf_bean + mfBean
        //: PositionLabelReactiveCompatible.share.loginUserMode.mf_bean = nowBean
        PositionLabelReactiveCompatible.share.loginUserMode.mf_bean = nowBean
        ...
        mfBeanLB.text = "\(Int(nowBean))\((appEndShow.replacingOccurrences(of: "key", with: "n") + "ts\n≈ " + String(notiBelowData)).localized) \(divisor)"
