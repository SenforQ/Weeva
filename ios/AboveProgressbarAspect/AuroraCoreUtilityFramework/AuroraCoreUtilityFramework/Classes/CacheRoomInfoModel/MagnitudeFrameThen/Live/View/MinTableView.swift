...
fileprivate let notiTopK:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]
...
enum CurvatureCustomReflectable: Int {
    //: case TimeGap = 0
...
@objc protocol ManagerObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func startPost()

    //: @objc optional func endScroll()
    @objc optional func appearScroll()
}

//: class TalkingDanmuMsgListTableView: UIView {
class MinTableView: UIView {
	var femaleSum: Double = 33.6
	var sweepingContent: String = "view"
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiTopK.reversed(), encoding: .utf8)!)
    }
    ...
    override func layoutSubviews() {
        ...
        nameItem()
            ...
            if (self.frame.size.height == 194.62) && (self.layer.isHidden != false) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let personLet = ComponentView()
            personLet.beautyUpInterval = { [self] questionNumber in
            self.femaleSum = questionNumber
            
            var personLet = reloadTimeSpan
            personLet = 0
            if personLet < self.femaleSum {
                self.femaleSum = personLet
            }
            
            return self.femaleSum
            }
            personLet.wildcatContent = { [self] playerCollectionName in
            self.sweepingContent = playerCollectionName
            
            return self.sweepingContent
            }
                self.addSubview(personLet)
            }
...
extension MinTableView {
    ...
    func getNameStub() {
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(miscue), userInfo: nil, repeats: true)
    }
...
extension MinTableView {
    ...
    func secureModel(msgModel: ThoughtImageDanmuModel?) {
    ...
    func bindHome() {
        ...
        if AllHeight > dataRecordBarId {
            //: if self.msgTableView.height < MsgTableViewHeight {
            if self.msgTableView.height < dataRecordBarId {
                ...
                self.msgTableView.height = dataRecordBarId
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = dataRecordBarId - AllHeight
        ...
        pointMagnitudeeraction(animated: false)
    }

    /** 执行插入动画并滚动 */
    //: func scrollToBottom(animated: Bool) {
    func pointMagnitudeeraction(animated: Bool) {
    ...
    func groundAppearUnfinished(inPending: Bool) {
...
extension MinTableView: UITableViewDelegate, UITableViewDataSource {
    ...
    func counterest(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.appearScroll?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(userMaleTitle)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = BackgroundBassCell.overdoSize(tableView: tableView, msg: msgModel, indexPath: indexPath)
