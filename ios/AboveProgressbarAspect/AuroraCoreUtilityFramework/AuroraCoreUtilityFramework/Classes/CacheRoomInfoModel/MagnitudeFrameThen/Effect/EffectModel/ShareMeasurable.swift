...
/*: "id" :*/
fileprivate let showNorData:String = "enter"

/*: "num" :*/
fileprivate let appNameResultNoti:String = "modelum"
...
@objc public enum ContentVarArg: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct ShareMeasurable: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func member(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct ContentAnimatModel: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (appNameResultNoti.replacingOccurrences(of: "model", with: "n"))
    }
}
