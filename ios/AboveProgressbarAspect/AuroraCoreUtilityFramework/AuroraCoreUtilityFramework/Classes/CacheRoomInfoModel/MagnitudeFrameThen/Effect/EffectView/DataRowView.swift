...
fileprivate let mainManagerNoti:[UInt8] = [0x72,0x75,0x72,0x6f,0x33,0x78,0x74,0x7f,0x7e,0x69,0x21,0x32,0x3b,0x73,0x7a,0x68,0x3b,0x75,0x74,0x6f,0x3b,0x79,0x7e,0x7e,0x75,0x3b,0x72,0x76,0x6b,0x77,0x7e,0x76,0x7e,0x75,0x6f,0x7e,0x7f]

private func inputCan(value num: UInt8) -> UInt8 {
    return num ^ 27
}

/*: "iosSvgaEffect" :*/
fileprivate let kAdultMainNoti:[Character] = ["i","o","s","S","v","g","a"]
fileprivate let kAwakeData:[Character] = ["E","f","f","e","c","t"]

/*: "iosEffect" :*/
fileprivate let userPointConst:String = "iosEffecmid view"
fileprivate let userGiftFilterK:[Character] = ["t"]

/*: "entityId" :*/
fileprivate let appBirthdayConst:[Character] = ["e","n","t","i","t","y","I","d"]
...
class DataRowView: UIView {
    ...
    var currentEffectMsgModel: TimingHandyJSON?
    ...
    var animatedView: ScaleValueThen?

    //: open weak var delegate: AnimatViewStepProtocol?
    open weak var delegate: ToThen?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainManagerNoti.map{inputCan(value: $0)}, encoding: .utf8)!)
    }
...
extension DataRowView {
    ...
    func see(infoDic: [String: Any]) {
        //: let dict = infoDic
        let dict = infoDic
        //: if dict.keys.isEmpty == false {
        if dict.keys.isEmpty == false {
            //: var iosEffect: String = dict["iosSvgaEffect"] as! String
            var iosEffect: String = dict[(String(kAdultMainNoti) + String(kAwakeData))] as! String
            //: if iosEffect.isEmptyString {
            if iosEffect.isEmptyString {
                //: iosEffect = dict["iosEffect"] as! String
                iosEffect = dict[(String(userPointConst.prefix(8)) + String(userGiftFilterK))] as! String
            }

            //: let entityId = dict["entityId"] as! Int
            let entityId = dict[(String(appBirthdayConst))] as! Int
            ...
            if iosEffect.isEmptyString == false {
                ...
                let successs = birdSEye(msgModel: effectMsgModel)
            }
        }
    }
    ...
    func birdSEye(msgModel: Any?) -> Bool {
        ...
        if msgModel is TimingHandyJSON {
            //: effectMsgModel = msgModel as? TalkingAnimatMsgModel
            effectMsgModel = msgModel as? TimingHandyJSON
            //: } else if msgModel is TalkingGiftMsgAnimatModel {
        } else if msgModel is ShareMeasurable {
            //: let giftMsgModel = msgModel as! TalkingGiftMsgAnimatModel
            let giftMsgModel = msgModel as! ShareMeasurable
            ...
            if giftMsgModel.animationTimes > 1 {
                ...
                tmpGiftMsgModel.member(giftNum: 1) // 礼物个数设置为1，每次动效显示
                ...
                var repeatEfftect = TimingHandyJSON()
            ...
            effectMsgModel = TimingHandyJSON()
            ...
            if giftMsgModel.showType == ContentVarArg.myStery.rawValue || giftMsgModel.showType == ContentVarArg.king.rawValue {
        ...
        if self.currentEffectMsgModel != nil || paused {
            ...
            if effectMsgModel?.top ?? false && self.effectMsgModelArray.count > 0 {
                ...
                for i in 0 ..< self.effectMsgModelArray.count {
                    ...
                    let aModel: TimingHandyJSON = self.effectMsgModelArray[i] as! TimingHandyJSON
                    //: if aModel.top == false {
                    if aModel.top == false {
                        //: index = i
                        index = i
                ...
                if index == -1 {
                    ...
                    self.effectMsgModelArray.add(effectMsgModel ?? TimingHandyJSON())
                    //: } else {
                } else {
                    //: self.effectMsgModelArray.insert(effectMsgModel ?? TalkingAnimatMsgModel.init(), at: index)
                    self.effectMsgModelArray.insert(effectMsgModel ?? TimingHandyJSON(), at: index)
                }
            ...
            successPicture(effectMsgModel: effectMsgModel!)
        }
...
extension DataRowView {
    ...
    func toEqual() {
        ...
        self.animatedView?.demonstrateGift()
    }
    ...
    func levelBottom() {
        ...
        if self.currentEffectMsgModel == nil {
            ...
            if self.effectMsgModelArray.count > 0 {
                ...
                guard let removeModel = blockData() else { return }
                //: firstShow(effectMsgModel: removeModel)
                successPicture(effectMsgModel: removeModel)
            }
            //: } else {
        } else {
            //: self.animatedView?.resumeAnimation()
            self.animatedView?.colorBy()
        }
    ...
    func blockData() -> TimingHandyJSON? {
        //: if (self.effectMsgModelArray.count == 0) {
        if self.effectMsgModelArray.count == 0 {
            //: return TalkingAnimatMsgModel.init()
            return TimingHandyJSON()
        }
        ...
        if effectMsgModel is TimingHandyJSON {
            //: let lastEffectMsgModel = effectMsgModel as! TalkingAnimatMsgModel
            let lastEffectMsgModel = effectMsgModel as! TimingHandyJSON
        ...
        return effectMsgModel as? TimingHandyJSON
    }
}
...
extension DataRowView {
    ...
    func successPicture(effectMsgModel: TimingHandyJSON) {
        ...
        self.delegate?.playAcross(effectView: self, obj: effectMsgModel)

        //: TalkingGiftAnimatTool.shared.loadEffectData(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
        MagnitudeFrameThen.shared.lineEventBottom(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
            ...
            self.onModel(effectMsgModel: effectMsgModel, success: success, effectModel: giftModel ?? StateAnimatModel())
            //: })
        })
    }
    ...
    func onModel(effectMsgModel: TimingHandyJSON, success: Bool, effectModel: StateAnimatModel) {
        // 资源下载完成
        //: self.delegate?.animatViewWithDidEndLoadingResource(effectView: self, obj: effectMsgModel, success: success)
        self.delegate?.loadEqual(effectView: self, obj: effectMsgModel, success: success)
        ...
        if success {
            ...
            if self.paused {
                ...
                if self.currentEffectMsgModel?.eid == effectMsgModel.eid, self.currentEffectMsgModel?.effectUrl == effectMsgModel.effectUrl {
                    ...
                    faceOnAllowDiagnosticProcedure(effectMsgModel: effectMsgModel, model: effectModel)
                    //: return
                    return
                }
            }
    ...
    func sizeModel(effectMsgModel: TimingHandyJSON, effectModel: StateAnimatModel) {
        //: if self.animatedView != nil {
        if self.animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.borderStatus()
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.omit()
        ...
        className = MagnitudeFrameThen.shared.giftTo(oldName: className ?? "")
        ...
        let realClass = AnimatedViewClass as! ScaleValueThen.Type
...
extension DataRowView: DataReactiveCompatible {
    ...
    func viewWith(effectItemView: ScaleValueThen, success: Bool) {
        ...
        self.delegate?.messageClick(effectView: self, obj: effectItemView.effectMsgModel ?? TimingHandyJSON(), success: success)
    }
    ...
    func lineItem(effectItemView: ScaleValueThen) {
        ...
        self.delegate?.active(effectView: self, obj: effectMsgModel!)
    }
}
...
extension DataRowView {
    ...
    func referencePoint() {
        ...
        if animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.borderStatus() // stop animation
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.omit() // clean animation data
