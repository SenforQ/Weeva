...
/*: "gifts" :*/
fileprivate let userTitleK:String = "gionts"

/*: "vipGifts" :*/
fileprivate let notiBottomK:[Character] = ["v","i","p","G","i"]
fileprivate let dataFirstMain:String = "resultts"

/*: "packGifts" :*/
fileprivate let showPathK:String = "main status countpackGif"
fileprivate let showEqualK:String = "thost"

/*: "giftSendItem" :*/
fileprivate let constGenderApp:String = "giftSeleft transform to ting"
fileprivate let showSocialSourceUser:[Character] = ["n","d","I","t","e","m"]
...
class SpecsReactiveCompatible: NSObject {
    /// 是否展示背包红点
    ...
    private var cacheDict = [String: [RequestAdditiveArithmetic: [Any]]]()
    // 数据锁
    ...
    private var giftType = ImageOutputStreamable.normal

    //: static let share = TalkingChatGiftManager()
    static let share = SpecsReactiveCompatible()
...
extension SpecsReactiveCompatible {
    /// 发送礼物面板数据
    /// - Parameters:
    ...
    func pastAttention(type: ImageOutputStreamable = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        SpecsReactiveCompatible.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[RequestAdditiveArithmetic.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            SpecsReactiveCompatible.share.sendPush(completion: { isSucceed in
            ...
            SpecsReactiveCompatible.share.actionWith(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            SpecsReactiveCompatible.share.haveEqual(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            SpecsReactiveCompatible.share.noInner(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func noInner(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = SpecsReactiveCompatible.share.giftType
        //: TitleRequestTool.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        TitleRequestTool.donationVideoTheme(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [RequestAdditiveArithmetic: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(userTitleK.replacingOccurrences(of: "on", with: "f"))] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<GiftMeasurable>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[RequestAdditiveArithmetic.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(notiBottomK) + dataFirstMain.replacingOccurrences(of: "result", with: "f"))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<GiftMeasurable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[RequestAdditiveArithmetic.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(showPathK.suffix(7)) + showEqualK.replacingOccurrences(of: "host", with: "s"))] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<GiftMeasurable>.deserialize(from: array as? Array) {
                    ...
                    dataDict[RequestAdditiveArithmetic.Bags] = bagsArr
                }
                ...
                var firstModel = SpeedMeasurable()
                //: firstModel.num = 1
                firstModel.num = 1
                ...
                let sendItem: Array = data[(String(constGenderApp.prefix(6)) + String(showSocialSourceUser))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<SpeedMeasurable>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func sendPush(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = SpecsReactiveCompatible.share.giftType
        //: TitleRequestTool.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        TitleRequestTool.nature(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func startAt(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: TitleRequestTool.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        TitleRequestTool.momentOfCell(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<OfMainHandyJSON>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func actionWith(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = ImageOutputStreamable.live
        //: TitleRequestTool.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        TitleRequestTool.fillUp(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    private func haveEqual(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.party
        let type = ImageOutputStreamable.party
        //: TitleRequestTool.req_partyGiftList(type: type.rawValue) { succeed, result, errorModel in
        TitleRequestTool.afterCompletion(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func bagSear(perPageCount: Int, dataType: RequestAdditiveArithmetic) -> Int {
    ...
    func removeType(dataType: RequestAdditiveArithmetic) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func appVoice() -> [Any] {
        ...
        if SpecsReactiveCompatible.share.giftType == .groupChat {
            //: return chatRoomSendItems
            return chatRoomSendItems
            //: } else if TalkingChatGiftManager.share.giftType == .live || TalkingChatGiftManager.share.giftType == .party {
        } else if SpecsReactiveCompatible.share.giftType == .live || SpecsReactiveCompatible.share.giftType == .party {
    ...
    func errorDestroy(dataType: RequestAdditiveArithmetic, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension SpecsReactiveCompatible {
    ...
    private func addEqual() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(haveForPackage),
    ...
    @objc private func haveForPackage() {
        //: gift_clearMemoryCache(showRed: true)
        propaganda(showRed: true)
    }
}
...
extension SpecsReactiveCompatible {
    ...
    func propaganda(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        SpecsReactiveCompatible.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
