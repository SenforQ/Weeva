...
fileprivate let showInfoMain:[Character] = ["设","\u{7f6e}","\u{666e}","通","状","\u{6001}","的","文","字","颜","\u{8272}","时"," ","请","使","用","R"]
fileprivate let notiUpApp:[Character] = ["G","B","\u{7a7a}"]
fileprivate let kShowUser:String = "\u{95f4}的颜\u{8272}\u{503c}"

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let notiToData:String = "设\u{7f6e}选中\u{72b6}"
fileprivate let showNowConst:String = "色时 \u{8bf7}使\u{7528}"
fileprivate let constTopMainShow:String = "RGB空raw color true group self"

/*: "init(coder:) has not been implemented" :*/
fileprivate let notiWithUser:[UInt8] = [0x7d,0x7a,0x7d,0x60,0x3c,0x77,0x7b,0x70,0x71,0x66,0x2e,0x3d,0x34,0x7c,0x75,0x67,0x34,0x7a,0x7b,0x60,0x34,0x76,0x71,0x71,0x7a,0x34,0x7d,0x79,0x64,0x78,0x71,0x79,0x71,0x7a,0x60,0x71,0x70]

private func rowImage(accept num: UInt8) -> UInt8 {
    return num ^ 20
}

/*: "设置的下标不合法!!" :*/
fileprivate let dataViewK:String = "设\u{7f6e}\u{7684}下标不合法!"
fileprivate let dataControlConst:String = "nor"

/*: "Bags" :*/
fileprivate let dataExtraK:[Character] = ["B","a","g","s"]
...
open class CapacityView: UIView {
	var messageTotal: Int = 84
	var strengthSum: Double = -34.6
	var liveBlockDictionary: [AnyHashable: String] = [:]
	var bagSum: Int = 81
	var magnitudeSum: Double = -74.0
	var noteDictionary: [AnyHashable: String] = [:]

    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: DetailSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.adhereRgb(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(showInfoMain) + String(notiUpApp) + kShowUser))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.adhereRgb(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((notiToData + "态的\u{6587}字颜" + showNowConst + String(constTopMainShow.prefix(4)) + "\u{95f4}的\u{989c}色值"))
        }
    ...
    fileprivate func adhereRgb(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
	...
	if let scrollLine = scrollLine {
            ...
            if (scrollLine.isExclusiveTouch) && (!scrollLine.transform.isIdentity) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let toolSection = UpDoingView()
            toolSection.titleOn = segmentStyle.showLine

            toolSection.beforeMagnitude = { [self] tableRowInterval in
            self.bagSum = tableRowInterval
            
            var toolSection = numOfComponents
            toolSection &= 2
            if toolSection > self.bagSum {
                self.bagSum = toolSection
            }
            
            return self.bagSum
            }
            toolSection.labelSaveMagnitude = { [self] gestureMagnitude in
            self.magnitudeSum = gestureMagnitude
            
            self.magnitudeSum -= 1
            return self.magnitudeSum
            }
            toolSection.rowTextDictionary = { [self] userDictionary in
            self.noteDictionary = userDictionary
            
            guard var value = self.noteDictionary as? [String: String] else {
                return nil
            }
            return value
            }
                scrollLine.addSubview(toolSection)
            }
    ...
    public init(frame: CGRect, segmentStyle: DetailSegmentStyle, titles: [String]) {
    ...
    @objc func nameer(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? ViewLabel else { return }
        ...
        profileOn(true)
            ...
            if (scrollView.isExclusiveTouch) && (!scrollView.transform.isIdentity) {
                //: SWIFT_CUSTOM_DANGER_File_Call
                let toolSection = UpDoingView()
            toolSection.titleOn = self.segmentStyle.scrollTitle

            toolSection.beforeMagnitude = { [self] tableRowInterval in
            self.messageTotal = tableRowInterval
            
            var toolSection = currentIndex
            toolSection &= 2
            if toolSection > self.messageTotal {
                self.messageTotal = toolSection
            }
            
            return self.messageTotal
            }
            toolSection.labelSaveMagnitude = { [self] gestureMagnitude in
            self.strengthSum = gestureMagnitude
            
            self.strengthSum -= 1
            return self.strengthSum
            }
            toolSection.rowTextDictionary = { [self] userDictionary in
            self.liveBlockDictionary = userDictionary
            
            guard var value = self.liveBlockDictionary as? [String: String] else {
                return nil
            }
            return value
            }
                scrollView.addSubview(toolSection)
            }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiWithUser.map{rowImage(accept: $0)}, encoding: .utf8)!)
    }
}
...
public extension CapacityView {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func dent(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (dataViewK + dataControlConst.replacingOccurrences(of: "nor", with: "!")))
    ...
    func restoreBottomSize(_ titles: [String]) {
        // 移除所有的scrollView子视图
...
extension CapacityView {
    ...
    fileprivate func viewBind() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = ViewLabel(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.nameer(_:)))
            ...
            if title == (String(dataExtraK)).localized {
    ...
    fileprivate func even() {
        ...
        if let firstLabel = labelsArray[0] as? ViewLabel {
    ...
    public func computerMenu(_ currentIndex: Int) {
