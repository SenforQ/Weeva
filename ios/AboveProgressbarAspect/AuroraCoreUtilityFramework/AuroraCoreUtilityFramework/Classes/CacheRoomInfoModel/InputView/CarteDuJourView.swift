...
fileprivate let dataAllowUser:[UInt8] = [0xd1,0xd6,0xd1,0xcc,0x90,0xdb,0xd7,0xdc,0xdd,0xca,0x82,0x91,0x98,0xd0,0xd9,0xcb,0x98,0xd6,0xd7,0xcc,0x98,0xda,0xdd,0xdd,0xd6,0x98,0xd1,0xd5,0xc8,0xd4,0xdd,0xd5,0xdd,0xd6,0xcc,0xdd,0xdc]

private func individualAspect(layer num: UInt8) -> UInt8 {
    return num ^ 184
}

/*: "icon_coinxxx" :*/
fileprivate let userHideApp:String = "begin type nameicon_c"
fileprivate let notiInfoData:String = "upinxxx"

/*: "icon_gift_num_bg" :*/
fileprivate let mainTitleNoti:String = "icon_gview type window"
fileprivate let constColorMain:String = "height target true with equalm_bg"

/*: "x1" :*/
fileprivate let mainNameData:String = "xmessage"

/*: " x :*/
fileprivate let kMainNoti:String = " xto table else view result"
...
class CarteDuJourView: UICollectionViewCell {
    //: var model = TalkingRoomGiftModel()
    var model = GiftMeasurable()
    //: let checked = false
    let checked = false
    //: let giftNum = 0
    let giftNum = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataAllowUser.map{individualAspect(layer: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nameLB: UILabel = {
        ...
        lb.font = UIFont.viewPath(type: .Regular, fontSize: 10)
        //: lb.textColor = .white
        lb.textColor = .white
    ...
    lazy var priceBtn: EmptyButton = {
        //: let btn = TalkingButton.init()
        let btn = EmptyButton()
        //: btn.setTitleColor(UIColor.white, for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Regular, fontSize: 10)
        btn.titleLabel?.font = UIFont.viewPath(type: .Regular, fontSize: 10)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_coinxxx"), for: .normal)
        btn.setImage(UIImage.imageName(name: (String(userHideApp.suffix(6)) + notiInfoData.replacingOccurrences(of: "up", with: "o"))), for: .normal)
    ...
    lazy var numBgView: UIImageView = {
        ...
        let imag = UIImage.imageName(name: (String(mainTitleNoti.prefix(6)) + "ift_nu" + String(constColorMain.suffix(4))))
        //: let sizex = imag.size
        let sizex = imag.size
    ...
    lazy var numLb: UILabel = {
        ...
        lb.font = UIFont.viewPath(type: .Regular, fontSize: 9)
        //: return lb
        return lb
        //: }()
    }()
...
extension CarteDuJourView {
    /// 更改选中状态
    //: func setChecked(checked: Bool) {
    func toVisible(checked: Bool) {
        ...
        if checked {
            ...
            bgView.layer.borderColor = UIColor.requestCardTitle().cgColor
    ...
    func indexBind(currenmodel: GiftMeasurable, giftType: RequestAdditiveArithmetic) {
        //: model = currenmodel
        model = currenmodel
        //: giftIcon.setUrlImage(urlStr: model.imgPreview ?? "")
        giftIcon.degas(urlStr: model.imgPreview ?? "")
        //: priceBtn.setTitle("\(model.price)", for: .normal)
        priceBtn.setTitle("\(model.price)", for: .normal)
        //: selectedImageView.setUrlImage(urlStr: model.cornerMark, placeImg: nil)
        selectedImageView.degas(urlStr: model.cornerMark, placeImg: nil)
        ...
        let attributes = [NSAttributedString.Key.font: UIFont.viewPath(type: .Regular, fontSize: 11),
...
extension CarteDuJourView {
    ...
    func towardComment() {
        ...
        bgView.backgroundColor = UIColor.repossessWithinChromaticColor()
