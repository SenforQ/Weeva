...
// __CLOSE_PRINT__
//
//  TimeNavigationController.swift
//  AbroadTalking
//
...
class TimeNavigationController: UINavigationController {

    //: var tabBarType: TabBarItemType?
    var tabBarType: OffFrameCustomReflectable?
    //: var pushing = false                             
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count >= 1 {
            ...
            if viewController.navigationItem.leftBarButtonItem == nil {
                ...
                viewController.navigationItem.leftBarButtonItem = viewController.anQuantityimate()
            }
        }
    ...
    func playNavibarAppearance() {
        ...
        if #available(iOS 15.0, *) {
            ...
            barApp.backgroundImage = UIImage.observerColor(color: UIColor.white)
            //: barApp.shadowImage = UIImage.init()
            barApp.shadowImage = UIImage.init()
            //: barApp.shadowColor = UIColor.appBgColor()
            barApp.shadowColor = UIColor.loadSize()
            ...
            barApp.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.read(), NSAttributedString.Key.font: UIFont.viewPath(type: .Medium, fontSize: 17)]
        ...
        //: } else {
            ...
            navigationBar.setBackgroundImage(UIImage.observerColor(color: UIColor.white), for: .any, barMetrics: .default)
            ...
            navigationBar.titleTextAttributes = [NSAttributedString.Key.foregroundColor: UIColor.read(), NSAttributedString.Key.font: UIFont.viewPath(type: .Medium, fontSize: 17)]
        }
        //: UINavigationBar.appearance().tintColor = UIColor.appTitleColor()
        UINavigationBar.appearance().tintColor = UIColor.read()
...
extension TimeNavigationController: UINavigationControllerDelegate {
...
extension TimeNavigationController: UIGestureRecognizerDelegate {
