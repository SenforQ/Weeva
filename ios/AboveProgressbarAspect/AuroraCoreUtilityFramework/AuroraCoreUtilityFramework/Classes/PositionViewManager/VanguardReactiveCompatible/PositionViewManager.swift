...
/*: "position.x" :*/
fileprivate let constScaleK:[Character] = ["p","o","s","i","t","i","o","n",".","x"]

/*: "slideIn" :*/
fileprivate let kMainShow:String = "with button succeed sizeslideIn"

/*: "slideOut" :*/
fileprivate let constDataK:String = "view moment empty viewslideOu"
fileprivate let mainRequestConst:[Character] = ["t"]
...
class PositionViewManager: NSObject {
    //: static let shared = GiftFloatViewManager()
    static let shared = PositionViewManager()
    //: private var giftArr = [SocialBroadcastModel]()
    private var giftArr = [MenuHandyJSON]()
    ...
    private lazy var giftView: VanguardReactiveCompatible = {
        //: let v = GiftFloatView()
        let v = VanguardReactiveCompatible()
        //: RowMacroDefine.getWindow().addSubview(v)
        RowMacroDefine.apply().addSubview(v)
        //: return v
        return v
        //: }()
    }()
...
extension PositionViewManager {
    /// 添加礼物
    //: private func addGift(gift: SocialBroadcastModel) {
    private func addNamePortion(gift: MenuHandyJSON) {
        //: self.synchronized(self) {
        self.liveForRelease(self) {
    ...
    public func liveForRelease<T>(_ lock: AnyObject, _ body: () throws -> T) rethrows -> T {
    ...
    private func adjustAnimation(_ model: MenuHandyJSON) {
        ...
        let isRTL = ManagerReactiveCompatible.shared.direction == .rightToLeft
        // 起始位置：屏幕外
        //: let startCenterX = isRTL ? -centerX : ScreenWidth + centerX
        let startCenterX = isRTL ? -centerX : userDeviceKey + centerX
        // 目标位置：屏幕中间
        //: let targetCenterX = ScreenWidth / 2
        let targetCenterX = userDeviceKey / 2
        // 结束位置：屏幕另一侧
        //: let endCenterX = isRTL ? ScreenWidth + centerX : -centerX
        let endCenterX = isRTL ? userDeviceKey + centerX : -centerX
        ...
        let slideIn = CABasicAnimation(keyPath: (String(constScaleK)))
        ...
        self.giftView.layer.add(slideIn, forKey: (String(kMainShow.suffix(7))))
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) { [weak self] in
            ...
            let slideOut = CABasicAnimation(keyPath: (String(constScaleK)))
            ...
            self.giftView.layer.add(slideOut, forKey: (String(constDataK.suffix(7)) + String(mainRequestConst)))
...
extension PositionViewManager {
    ...
    func bindNotifications() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.appNoti(noti:)),
                                               //: name: USER_RECIEVED_GIFT_NOTIFICATION,
                                               name: appRecordStatusAcceptUrl,
                                               //: object: nil)
                                               object: nil)
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.isometricalBackground),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.atForeground),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.currentServer),
    ...
    @objc private func appNoti(noti: Notification) {
        ...
        guard let data = noti.object,
              ...
              let gift = JSONDeserializer<MenuHandyJSON>.deserializeFrom(dict: data as? [String: Any], designatedPath: nil),
              //: gift.broadType != BroadcastType.Unknow.rawValue
              gift.broadType != DescriptorEquatable.Unknow.rawValue
        //: else {
        else {
            //: return
            return
        }
        //: let currUserType = PositionLabelReactiveCompatible.share.appUserConfigMode.userType
        let currUserType = PositionLabelReactiveCompatible.share.appUserConfigMode.userType
        // 过滤接口异常
        ...
        // 非特殊用户 不展示 不同地区用户礼物
        //: if currUserType != 0, gift.userCountryType != PositionLabelReactiveCompatible.share.appUserConfigMode.userCountryType {
        if currUserType != 0, gift.userCountryType != PositionLabelReactiveCompatible.share.appUserConfigMode.userCountryType {
            //: return
            return
        }
        ...
        self.addNamePortion(gift: gift)
    }

    /// 应用退到后台
    //: @objc private func enterBackground() {
    @objc private func isometricalBackground() {
    ...
    @objc private func atForeground() {
        //: self.activeWhenShow()
        self.atShow()
    }
    ...
    @objc private func currentServer() {
